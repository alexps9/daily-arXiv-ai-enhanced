...
- [Sec](#Sec) [Total: 80]
- [Training](#Training) [Total: 56]
- [Model](#Model) [Total: 52]
- [Inference](#Inference) [Total: 136]
...
TL;DR: 开放权重医学语言模型在朴素攻击下，关于5种受保护行为的总体攻击成功率为38.0%，其中‘医学委员会考试’和‘医生权威’框架显著提高成功率，而指令覆盖前缀无效；鲁棒性受主题主导，药物相互作用防线几乎失效（83.2%），紧急转移防线较强（4.7%）。


<details>
  <summary>Details</summary>
Motivation: 模型卡片描述了期望行为而非鲁棒行为，需要量化在无技术门槛攻击下受保护行为的实际违反率。

Method: 构建5×50×6×3的全因子基准（4500个生成），通过Ollama本地服务MedGemma-4B-it，使用三个独立评判器（LLM、正则表达式、NLI蕴含）编码拒绝/回避/遵从，并应用多种统计方法分析。

Result: LLM评判下整体ASR为38.0%；‘医学委员会考试’框架提高24.0个百分点至53.1%，“医生权威”框架提高14.7个百分点至43.7%；药物相互作用ASR为83.2%，紧急转移ASR为4.7%。
...
TL;DR: 提出一种基于超平面排列交偏序集算法，用于在矩形矩阵中查找零子式，从而解决椭圆曲线离散对数问题。


<details>
  <summary>Details</summary>
Motivation: 延续前期工作，将椭圆曲线离散对数问题归约为矩阵零子式查找，并改进该子问题的解决效率。

Method: 利用超平面排列的交偏序结构设计算法，寻找矩形矩阵中的零子式，并分析其复杂度与成功概率。

Result: 给出了算法实现细节与仿真结果，验证了零子式查找的有效性及独立应用价值。

Conclusion: 该算法以初等方法实现零子式查找，为椭圆曲线离散对数问题提供了可行方案，且零子式问题本身具有通用意义。
...
TL;DR: 提出GATAS，一种在黑盒设置下通过音素级潜在空间优化生成自然对抗样本的方法，在ASR系统上实现98%攻击成功率且保持高感知质量。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒攻击方法生成的对抗样本通常缺乏自然性，难以在保持感知逼真的同时诱导ASR系统错误。

Method: 在文本转语音模型的音素级潜在空间中进行插值，将对抗生成建模为多目标优化问题，平衡语义偏差与感知质量，无需梯度信息。

Result: 实验表明GATAS对白盒与黑盒基线方法均具竞争力，攻击成功率98%，失真更低且感知质量更高（经人类评估验证）。

Conclusion: 潜在空间表示对齐与感知保持比访问模型内部结构更关键，该方法能高效生成自然有效的ASR测试用例。
...
TL;DR: 提出一种结合量子密码、混沌理论和储层计算的混合图像加密系统，抵御量子攻击，实现高效安全加密。


<details>
  <summary>Details</summary>
Motivation: 传统加密方法（如RSA、DHKE）易受量子算法（Shor、Grover）攻击，需开发抗量子加密方案。

Method: 采用E91量子协议生成安全密钥，结合Lorenz超混沌系统增加不可预测性，并用储层计算提升运算效率。

Result: 128x128图像加密时间0.0296s、解密0.0164s；MAE在300-600节点网络中降低；NPCR>99.6%，UACI>0.49。

Conclusion: 该混合系统在量子计算时代兼顾了高安全性与实际可行性，适用于图像加密。
...
TL;DR: AI驱动的语音钓鱼攻击因自动化而具经济可行性，最高36%的受试者可能上当，总体合规率16.5%。


<details>
  <summary>Details</summary>
Motivation: 传统语音钓鱼依赖人工，AI语音合成和LLM的出现使其可规模化自动化，需评估其威胁。

Method: 对4100名美国成年人进行大规模调查实验和12人定性访谈，使用多个AI语音模型生成钓鱼场景并与人类基线对比。

Result: 最高36%的受试者可能对‘亲属困境’类请求妥协；整体合规率16.5%；‘说服力’是主要预测因素；部分AI模型（如Sesame）评分接近或略超人类。

Conclusion: 当前AI语音钓鱼的主要风险在于自动化带来的经济可行性，而非更高级的欺骗技术，需关注AI系统设计、消费者保护和模型发布政策。
...
TL;DR: 提示词包装仅格式不同即可显著改变模型得分，甚至颠覆排行榜结论。本研究在令牌控制协议下引入格式敏感指数（FSI）和可解析敏感指数（PSI），发现模型间FSI均值差异超30倍，主要由合规失败解释。回归分析表明可解析性在控制任务、模型和包装后仍是准确率的强预测因子。


<details>
  <summary>Details</summary>
Motivation: 提示词包装的格式差异可能导致模型性能波动，影响评测结论的可靠性，但缺乏系统性量化研究。

Method: 在令牌控制协议下，定义FSI和PSI指标，对7个QA任务、5个包装族、4个指令模型（7B-72B参数）共14万次OpenRouter生成进行分析，并采用固定效应回归模型。

Result: 模型间FSI均值差异超30倍，可解析性在控制任务、模型和包装后仍是准确率的显著预测变量。

Conclusion: 仅报告准确率而不考虑包装方差和合规性在统计上不可靠，建议在基准评测和结构化输出部署中采纳实用建议。
...
### [7] [SafeGuard: A Lightweight Client-Server Architecture for Real-Time Endpoint Threat Detection and Response](https://arxiv.org/abs/2607.10027)
*Gideon Francis Oghie,DivineDavid Shittu Abolanle*

Main category: Sec

TL;DR: SafeGuard是一个轻量级三层客户端-服务器架构，用于实时端点监控和响应，采用开源技术栈，在50个端点下命令延迟低于2秒，适用于资源受限组织。


<details>
  <summary>Details</summary>
Motivation: 商业EDR平台对小型组织成本过高且操作复杂，需要轻量、低开销的替代方案。

Method: 基于Flutter/Kotlin的端点代理、Node.js中央服务器和管理仪表盘，通过签名检测、WSS通信、JWT认证和HMAC完整性验证实现威胁检测与安全通信。

Result: 50个并发端点平均命令延迟约1.5秒，负载下低于2秒；认证令牌、SQL注入和重放攻击被有效阻止。

Conclusion: 开源技术栈可提供实时端点可见性和协调响应，但局限于静态签名、Android实现和受控环境评估。
...
### [8] [Federated Cybersecurity Testbed as a Service (FCTaaS): A framework to federate cybersecurity testbeds](https://arxiv.org/abs/2607.10061)
...
TL;DR: 提出FCTaaS框架，通过VPN连接异构网络安全测试床，支持跨地域联合实验，在攻击场景下网络利用率≤49%，开销仅1%，延迟5.63-147毫秒。


<details>
  <summary>Details</summary>
Motivation: 现有网络安全测试床更新成本高、利用率低，且受地理限制难以协作，亟需联邦化框架以共享资源和开展跨域实验。

Method: 设计FCTaaS架构，基于VPN实现测试床发现、实验编排和协调控制，支持远程访问与数据交互。

Result: 在拒绝服务和入侵检测案例中，成功实现跨床实验，攻击流量和IDS告警可见性完整；高负载下网络利用率49%、开销1%，本地延迟5.63 ms，跨域延迟147 ms。

Conclusion: FCTaaS可有效整合异构测试床资源，兼顾性能与开销，为分布式安全实验提供可行方案。
...
### [9] [A Survey on LLM Watermarking: Theory and Deployment](https://arxiv.org/abs/2607.10103)
*Huy Phan,Kieu Dang,Ojaswi Dulal,Aiham AL Shukairi,Abby Shine,Chase Garner,Phung Lai*

Main category: Sec

TL;DR: 本文系统梳理了LLM水印技术，从嵌入方式、检测权限、假设条件、威胁模型等维度构建分类体系，分析各类技术的安全-效用权衡，并给出部署建议。


<details>
  <summary>Details</summary>
Motivation: LLM生成文本的流畅性导致来源模糊、滥用和内容洗钱风险加剧，现有水印研究分类混乱，难以横向比较和工程落地。

Method: 采用面向部署的分类框架，按水印嵌入阶段（生成/训练时）、检测权限（公开/私密）、系统假设（logits、采样控制、密钥等）和威胁模型（改写、翻译、摘要等）组织技术，并评估安全性、效用与鲁棒性。

Result: 归纳出采样偏置、编码方案、表征/训练嵌入等主流方法，揭示了检测率、鲁棒性与生成质量之间的三难权衡，并总结了攻击策略与评估指标。
...
### [10] [Minionese: Comprehensive Benchmark and Mechanistic Study of Multilingual LLM Safety](https://arxiv.org/abs/2607.10112)
*Chigozirim Ifebi,Brent Kong,Ayushi Mehrotra*

Main category: Sec

TL;DR: 多语言环境下大语言模型的安全对齐存在脆性，非英语和低资源语言可通过不同扰动类型绕过安全机制，且低资源攻击通过几何错位子空间规避拒绝触发。


<details>
  <summary>Details</summary>
Motivation: 揭示英语安全评估的不足，系统分析多语言、低资源场景下安全机制失效的扰动类型和几何机制。

Method: 构建跨18种语言、4种资源层级和4种扰动类型的基准测试Minionese，结合几何机制分析拒绝失败的空间投影特性。

Result: 每种攻击类型产生独特脆弱性：音译依赖文字系统、代码切换在最低资源层持续有效、第2-3资源层间存在安全机制突变；低资源攻击利用未对齐子空间投射，拒绝机制完整但未被触发。
...
### [11] [Toward Stronger Code Watermarking: A Grammar-Driven Approach to Optimizing the Trade-off Between Quality and Detectability](https://arxiv.org/abs/2607.10210)
*Licheng Yu,Aiwei Liu,Songze Li*

Main category: Sec

TL;DR: 提出了一种语法驱动的代码水印方法（GDW），通过三级掩码机制保持语法有效性，并利用结构角色感知调制在令牌级别注入水印信号，以改善代码质量与可检测性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于logits的水印方法直接应用于代码生成时，难以平衡低熵代码的质量与可检测性，需要一种专为代码结构设计的水印方案。

Method: 采用语法引导的三级掩码机制确保语法正确性，通过结构角色感知调制为内容承载令牌分配强偏差、语法关键令牌分配保守偏差，并设计角色感知加权检测统计量。

Result: 在多种编程语言、模型和解码策略下，GDW比现有方法实现了更优的质量-可检测性权衡边界，且对变量重命名攻击具有鲁棒性。

Conclusion: GDW有效缓解了代码水印中的质量-可检测性矛盾，验证了语法引导和角色感知调制对代码生成场景的适用性。
...
### [12] [One Token Is Enough: Fingerprinting and Verifying Large Language Models from Single-Token Output Distributions](https://arxiv.org/abs/2607.10252)
*Tomas Bruckner*

Main category: Sec

TL;DR: 利用单词提示（如“在1到100之间随机选一个数字”）生成的行为指纹，能以极低成本高效识别LLM的身份，在OpenRouter平台上对165个模型测试，指纹的Jensen-Shannon散度可恢复模型谱系，验证协议达到7.3%等错误率，并发现生态系统异常。


<details>
  <summary>Details</summary>
Motivation: 现有LLM识别技术依赖长文本、令牌级对数概率、对抗性提示或模型合作，而商业服务中存在大量伪造模型，急需一种轻量、无需合作且仅需少量查询的可信验证方法。

Method: 定义行为指纹为模型对四种语言下简单单词提示的回答分布（仅1个输出令牌），收集165个模型的样本，计算细胞熵、Jensen-Shannon散度，并采用留一法分类、生物特征验证及可分离性分析来评估指纹效果。

Result: 指纹分布高度非均匀（中位数熵1.0比特），同模型分半距离远小于不同模型；留一法谱系归因准确率59.5%（随机18.4%）；验证协议在40个探针细胞下等错误率7.3%，8个细胞时仍低于11%。发现某旗舰模型与开源模型分布不可区分。

Conclusion: 极轻量的行为指纹（约100次单令牌查询）足以高效验证LLM身份，具备实际部署价值，且协议、数据和代码已开源，可用于监测商业服务中的模型篡改。
...
### [13] [Devil in the Lens: Analyzing and Defending Physical Prompt Injection Against Vision-Language Models on Wearable Devices](https://arxiv.org/abs/2607.10269)
*Yaxin Li,Hao Wang,Yanda Shao,Shuhao Zhang,Yan Long*

Main category: Sec

TL;DR: 本文发现，VLM驱动的智能眼镜在物理环境中遭遇文本注入攻击时，可被恶意操控输出虚假或有害内容，攻击成功率在模拟与现实场景中分别达96%和60%，并提出两种防御策略。


<details>
  <summary>Details</summary>
Motivation: 现有研究聚焦数字图像的视觉提示注入，但忽略物理环境文本通过可穿戴设备对VLM的独特安全威胁，需系统建模与防御。

Method: 通过AI眼镜在200+真实环境采集照片，识别6类物理注入攻击向量，并评估12种VLM模型在完整性/安全任务上的表现，设计掩码外过滤与语义内检测两种防御。

Result: 攻击稳定操纵模型输出，导致与视觉上下文相反的摘要，真实场景成功率达60%，两种防御分别降低攻击成功率与安全影响。

Conclusion: 物理环境文本注入对VLM可穿戴设备构成高风险，需结合外部过滤与内部检测的混合防御机制应对此类新型攻击。
...
TL;DR: 随着蜂窝核心网向云原生转型，内部接口的物理隔离假设失效，导致组件间盲目信任（iTrue漏洞）激增。本文设计了一个基于LLM的多智能体系统iFinder，通过总结已知缺陷、提取检测模式并交叉验证3GPP规范与核心网代码，在7个开源实现中发现84个新漏洞（其中81个已分配CVE），包括一个影响商用5G核心网的会话劫持漏洞。


<details>
  <summary>Details</summary>
Motivation: 核心网内部组件长期依赖物理隔离的安全模型，但云原生部署使内部接口暴露于外部攻击者，而现有安全分析缺乏对组件间盲目信任（如省略语法验证、未强制执行语义不变性、不检查资源可用性）的系统性检测手段。

Method: 设计iFinder多智能体系统：①利用LLM总结已知漏洞并蒸馏为检测模式；②通过交叉验证3GPP规范与核心网代码抑制LLM幻觉；③自动生成并迭代优化PoC漏洞利用代码，通过执行结果反馈验证潜在iTrue漏洞。

Result: 在7个主流开源核心网实现中发现84个未知漏洞，83个已确认、81个已获CVE编号；其中1个会话劫持漏洞在真实商用5G核心网中得到验证。

Conclusion: 核心网组件间的隐式信任是云原生环境下的系统性安全弱点，iFinder能高效发现此类漏洞，且LLM驱动的自动化验证可有效降低误报。
...
### [15] [Large Language Models in Misinformation Ecosystems: Misuse, Defense, and Vulnerability](https://arxiv.org/abs/2607.10402)
*Lingwei Wei,Dou Hu,Wei Zhou,Songlin Hu,Philip S. Yu*

Main category: Sec

TL;DR: 将LLM对假信息的影响从内容层面扩展到生态系统风险，提出一个角色-层框架统一攻击、防御与系统脆弱性。


<details>
  <summary>Details</summary>
Motivation: LLM将假信息问题从内容中心转变为生态系统级安全挑战，现有研究缺乏统一框架。

Method: 提出包含角色维度（攻击者、防御者、脆弱组件）和层维度（内容、社会语境、证据环境、验证流程）的双维框架，系统梳理攻击、检测、漏洞与对策。

Result: 识别出三大开放挑战：从静态准确性转向预算约束的生态风险评估、加固以LLM为中心的验证管线、部署可审计的人机协同验证系统。

Conclusion: 未来假信息防御需采用生态级风险评估、对抗鲁棒性验证和可审计人机协同。
...
### [16] [Threat Vectors and the State of the Art in Defense Methods for Security in Neurotechnology](https://arxiv.org/abs/2607.10451)
*Bryce-Allen Bagley,Nathaniel Rose,Quintus Kilbourn,Matthew Canham*

Main category: Sec

TL;DR: 本文综述了脑机接口安全（神经安全）的现状，指出其发展滞后于BCI技术本身，系统梳理了已确立和潜在的安全威胁，并建议采用网络安全、硬件安全和机器学习领域的现有方法来弥补这些漏洞。


<details>
  <summary>Details</summary>
Motivation: 脑机接口技术快速发展，已进入临床试验和消费市场，但相关安全研究严重滞后，存在大量未解决的安全漏洞。

Method: 通过全面调查当前神经安全领域的最新技术水平，梳理已确立和高度可能的安全威胁，并借鉴网络安全、硬件安全和机器学习中的现有方法提出应对建议。

Result: 揭示了BCI系统中广泛且复杂的脆弱性，提供了可直接应用的现有安全措施。
...
TL;DR: 本文提出FedCausal-Dyn框架，通过因果域特征分离和动态原型聚合解决联邦学习中的动态特征漂移问题，在多个基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法假设静态漂移，无法有效应对现实场景中客户端数据分布随时间演变的动态特征漂移。

Method: 采用因果域特征分离（通过专用投影头和对抗训练解耦不变因果特征与虚假域特征）、可靠动态原型聚合（基于估计可靠性加权本地类原型）以及因果特征引导的协同正则化（统一原型对比对齐和域不变性）。

Result: 在三个联邦域泛化基准上，FedCausal-Dyn实现最高平均准确率和最稳定结果，消融实验验证了各组件的关键贡献。

Conclusion: FedCausal-Dyn为动态特征漂移下的联邦学习提供了稳健且原理性的解决方案。
...
### [18] [Firewall3D: A Hardware Firewall for Defending 3D Printers Against Firmware Attacks](https://arxiv.org/abs/2607.10484)
*Seyed Ali Ghazi Asgar,Narasimha Reddy*

Main category: Sec

TL;DR: 针对增材制造系统的固件攻击日益严重，本文提出一种硬件防火墙Firewall3D，通过监控物理层信号（如步进电机电流、温度等）检测异常行为并实时中断打印，实验验证其能有效防御多种固件攻击。


<details>
  <summary>Details</summary>
Motivation: 3D打印市场快速增长（估值超300亿美元），但面临供应链篡改、恶意固件更新等固件级攻击威胁，现有方案缺乏对物理行为与G代码一致性的实时验证能力。

Method: 设计专用硬件防火墙Firewall3D，持续监控物理层信号（步进电机电流、限位开关、喷嘴/热床温度、冷却风扇），将物理行为与预期G代码执行进行对比，异常时触发警报并停止打印。

Result: 实验表明Firewall3D能有效检测破坏打印完整性、损坏打印机或泄露知识产权的多种固件攻击。

Conclusion: 基于物理层信号监控的硬件防火墙可成为防御3D打印机固件攻击的有效手段。
...
### [19] [A Dynamic Scene Interaction Reasoning Framework for Scene-level Lane-Change Intention and Trajectory Prediction of Multiple Interacting Vehicles](https://arxiv.org/abs/2607.09740)
*Joshua Kofi Asamoah,Blessing Agyei Kyem,Eugene Denteh,Armstrong Aboah*

Main category: Sec

TL;DR: 提出一种动态场景图注意框架（DSiGAT），用于同时预测多个车辆的换道意图和未来轨迹，并在NGSIM和highD数据集上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有换道预测方法多关注单一目标车辆，而多智能体预测仅输出未来位置，缺乏对车辆操纵意图的显式建模。

Method: 将交通场景构建为时变交互图，节点为车辆，边编码时空与运动关系；通过时序图注意力消息传递捕获依赖与预操纵线索；采用意图引导解码器将预测操纵与轨迹关联，并加入场景级一致性损失。

Result: 在NGSIM I-80和US-101上意图预测准确率达90.12%和90.97%，轨迹RMSE相比最强基线降低52.94%，碰撞率和联合位移误差更低。

Conclusion: DSiGAT通过场景图注意力机制有效融合意图与轨迹预测，生成更连贯的多车未来场景，优于现有基线。
...
### [20] [Temporary Authority, Permanent Effects: Commit-Time Authorization for LLM Agents](https://arxiv.org/abs/2607.10487)
*Igor Santos-Grueiro*

Main category: Sec

TL;DR: 研究了AI代理在持久化操作时的授权边界问题，提出“提交时授权”属性，并通过实验揭示大量已提交操作在授权失效后仍被执行的安全隐患，最后验证了多种防护策略的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有系统允许AI代理基于早期有效证据执行持久化操作，但缺乏对证据在提交时是否仍有效这一安全边界的系统性研究，导致安全漏洞。

Method: 构建了覆盖浏览器、工具API和多代理工作流的受控失效测试套件，通过在主任务矩阵中使授权关系在持久化前失效，测试代理行为；并评估提示约束、条件检查、CommitGuard等防护策略。

Result: 在270次测试中262次达到可见结果，但仅55次为授权提交；216次失效测试中207次在授权路径失效后提交；所有对照组均合规。CommitGuard等防护策略能有效阻断失效授权操作。

Conclusion: 端点成功仅反映任务完成率，而授权提交才是安全属性；防护需在持久化边界刷新、重绑定、重规划或拒绝操作，而非依赖提示约束或单一条件检查。
...
### [21] [Safe responses matter: Output-aware safety guardrail mitigate over-refusal in MLLMs](https://arxiv.org/abs/2607.09697)
*Jiayi Li,Kun Zhan*

Main category: Sec

TL;DR: 提出输出感知安全护栏，通过多实例对比学习在隐藏状态空间预测生成内容是否不安全，在保持安全性能的同时大幅减少过度拒绝。


<details>
  <summary>Details</summary>
Motivation: 现有输入侧安全护栏存在过度拒绝问题，会阻断本可安全回应的良性查询，损害模型效用。

Method: 在模型隐藏状态空间训练轻量级分类器，使用多实例对比学习区分会导致不安全输出与不会导致不安全输出的输入。

Result: 匹配现有方法的安全性能，同时大幅降低过度拒绝，保留模型效用与内置安全能力。

Conclusion: 输出感知范式优于输入感知范式，可在不牺牲实用性的前提下实现有效安全拦截。
...
### [22] [Scaffolding the Strategist: Architecture-Dependent Reasoning Interventions in Hotelling Spatial Markets](https://arxiv.org/abs/2607.09743)
*Pratyush Singh*

Main category: Sec

TL;DR: 结构化推理干预对LLM战略经济推理的影响因模型架构而异，存在显著的交叉交互效应，并存在声明-执行差距。


<details>
  <summary>Details</summary>
Motivation: 探究结构化推理干预是否以及如何影响不同架构LLM的战略经济推理能力。

Method: 以霍特林线性城市模型为诊断工具，在两种模型（GPT-4.1-mini和GPT-5-mini）上测试五种条件（基线加四种推理干预），涉及8个问题、3种提示框架和3次重复，共720个独立响应，并进行了统计交叉交互分析。

Result: 发现支架类型与模型架构间存在统计显著的交叉交互效应：承诺支架提升标准模型（+0.21）但降低推理模型（-0.63），分离原则支架效果相反（-0.40 vs +0.31）。对抗性压力测试对两者均有害，推理模型退化更严重（2.6倍），且损害程度与基线难度负相关。存在声明-执行差距，分离支架可完全弥合推理模型的差距。

Conclusion: 结构化推理干预的效果依赖于模型架构，推理优化模型对特定干预更敏感且更易受对抗性攻击，声明-执行差距的闭合具有模型和干预特异性。
...
### [23] [NetInjectBench: Benchmarking Indirect Prompt Injection in Tool-Using Large Language Model Agents for Network Operations](https://arxiv.org/abs/2607.10490)
*Ruksat Khan Shayoni,Muhammad Faraz Shoaib,S M Asif Hossain,M. F. Mridha*

Main category: Sec

TL;DR: 提出NetInjectBench基准测试，比较多种防御策略，元数据感知策略在假设元数据完整下实现0%不安全操作且保持高实用性。


<details>
  <summary>Details</summary>
Motivation: 网络运维中LLM智能体易受间接提示注入攻击，现有防御措施存在安全性与实用性权衡问题。

Method: 构建130场景基准（含良性、弱/强攻击、批准变更），测试Qwen2.5-7B等模型；对比朴素执行、提示安全、自提醒、聚焦、两轮LLM评判、静态白名单及元数据感知策略门控。

Result: 朴素执行不安全率82.50%；元数据感知策略实现0/240不安全操作（95%威尔逊上限1.58%），攻击场景实用性99.17%，批准变更实用性100%。

Conclusion: 网络运维智能体需在执行时设置授权边界，同时保持提示级别指令规范性。
...
### [24] [Distributed Denial of Science: How Indirect Data Poisoning of AI Systems Can Industrialize Scientific Fraud](https://arxiv.org/abs/2607.10712)
*Bálint Gyevnár,Atoosa Kasirzadeh,Nihar B. Shah*

Main category: Sec

TL;DR: 提出间接数据投毒攻击，利用AI自主科研流程污染开放数据集，使诚实研究者无意中传播科学欺诈，攻击成功率达49.56%，但通过数据来源审计可完全防御。


<details>
  <summary>Details</summary>
Motivation: 探究远程攻击者能否利用AI在科研中的诚实使用来大规模破坏科学诚信，而不需要传统欺诈所需的资源。

Method: 间接数据投毒：攻击者将篡改后的数据集上传至公共仓库，自主AI研究代理检索并处理该数据，从而在五个社会敏感话题上使用三种前沿AI系统（Claude Code、Codex、Gemini CLI）进行450次实验评估。

Result: 攻击在49.56%的运行中成功，检测率仅6.0%；科学家角色仅降低至16.67%成功，但数据来源审计（含五项检查）将成功率降至零。

Conclusion: 间接数据投毒可能通过开放数据生态实现大规模科学欺诈，但可通过代理在数据检索时的适当审计加以缓解。
...
### [25] [EvoClawBench: Can Agents Learn Reusable Skills from Their Own Runs?](https://arxiv.org/abs/2607.09711)
*Zhiyuan Peng,Xin Yin,Chenhao Ying,Zhe Cui,Zixiang Ding,Zhenhua Liu,Jiang Wu,Yuan Luo*

Main category: Sec

TL;DR: EvoClawBench是一个用于评估智能体从自身运行中学习可复用技能的基准测试，包含100个任务和502个子问题。实验表明，技能学习并非自动有益，而是具有选择性和成本敏感性，不同模型和运行时下效果差异显著。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要测试任务完成、工具使用或技能效用，但未能隔离评估运行时能否将自身运行证据转化为可复用技能以改进后续执行，因此需要设计专门的基准来研究这一闭环技能学习问题。

Method: 提出了EvoClawBench基准，包含编程、数据、办公、安全、运维和领域文档六大类工作流的100个任务和502个子问题，支持多种智能体运行时。基准对比三种模式：直接执行（无技能）、预先编写技能（PreSkill）以及首次运行后从证据总结技能并再次执行（PostSkill）。

Result: 直接执行性能强依赖于运行时：OpenClaw低于20%，nanobot在56.45%到96.13%之间。自我编写的技能效果不一致：nanobot GPT-5.4保持96%以上，MiniMax-M2.7在PostSkill下从90.97%提升至94.50%，但nanobot DeepSeek-V4-Pro在PreSkill下从77.77%降至4.80%，PostSkill下降至0.99%。OpenClaw也表现出非单调行为，部分技能运行接近基线，部分则崩溃。

Conclusion: 从自身运行中学习可复用技能是有选择性和成本敏感的，并非在智能体循环中添加技能编写就能自动获益。
...
### [26] [Replicating Belief, Not Bits: Epistemic State Replication for Agentic Systems](https://arxiv.org/abs/2607.09748)
*Jun He,Deying Yu*

Main category: Sec

TL;DR: 本文提出信念级状态复制（ESR），将复制边界从数据可见性扩展至知识可见性，以解决智能体分布式系统中随机性组件（如生成模型）导致的语义分歧问题。


<details>
  <summary>Details</summary>
Motivation: 传统状态机复制（SMR）要求确定性位级一致，但在智能体系统中，随机性（如LLM推理路径差异）迫使系统牺牲灵活性并引发上下文遗忘；需一种允许语义等价但位不一致的复制模型。

Method: 定义节点状态为(K)=(L,B)：L为确定性证据日志，B为随机信念谱系；提出语义可线性化（Semantic Linearizability）和有限事件一致性（Bounded Eventual Coherence）；设计结构化认知增量传播协议与可验证语义回滚机制。

Result: 初步仿真表明，ESR在给定假设下可行，并减少次级认知故障（如上下文遗忘）。

Conclusion: 信念级复制在未来智能体分布式系统中具有实践意义，但需更严格的语义等价度量与故障模型分析。
...
### [27] [Can Watermarking Techniques Help Prevent LLM Model Stealing?](https://arxiv.org/abs/2607.10794)
*Elette Boyle,MohammadTaghi Hajiaghayi,Keivan Rezaei,Suho Shin,Amos Stern*

Main category: Sec

TL;DR: 本文提出针对语言模型窃取攻击的防御方法，通过扰动logits层的水印技术保护隐藏层维度，在保持模型效用的同时有效防御攻击。


<details>
  <summary>Details</summary>
Motivation: 现有模型窃取攻击能精确提取黑盒语言模型信息，亟需在不显著降低模型质量的前提下设计防御方案。

Method: 受水印技术启发，通过扰动模型logits层来干扰攻击者获取隐藏层维度的能力，并基于实证实验调整扰动参数以平衡防御效果与模型质量。

Result: 实验表明所提防御在不同配置下均能有效阻止攻击，且模型质量退化可控。

Conclusion: 提出的扰动logits层方法是一种实用且有效的防御方案，能在保障模型效用的同时抵御参数窃取攻击。
...
### [28] [Automated Stealthy Wear-Out Attack on Digital Twins With Deep Reinforcement Learning](https://arxiv.org/abs/2607.10830)
*Joshua Haworth,Aryan Pasikhani,George Pavlides,Prosanta Gope,John Clark*

Main category: Sec

TL;DR: 本文提出一种基于深度强化学习的隐蔽性磨损攻击，针对数字孪生赋能的工业系统，通过操纵控制信号加速机械臂关节磨损，同时规避异常检测，实验证明SAC算法在攻击效果和隐蔽性上表现最优。


<details>
  <summary>Details</summary>
Motivation: 数字孪生技术扩展了工业系统的攻击面，现有研究缺乏针对其隐蔽性攻击的探索，特别是利用强化学习实现长期、可规避检测的物理磨损攻击。

Method: 采用深度强化学习（SAC算法）生成恶意控制信号，在UR10e机械臂上对特定关节施加额外扭矩，并通过与异常检测系统的对抗训练保持攻击隐蔽性。

Result: SAC算法在样本效率、稳定性和攻击成功率上优于TD3、PPO、A2C，成功使目标关节扭矩显著升高，导致加速退化且未被检测系统发现。

Conclusion: 数字孪生系统面临强化学习驱动的隐蔽物理攻击威胁，亟需开发针对此类动态对抗策略的防御机制。
...
### [29] [SoK: Federated Learning for Intrusion Detection in Vehicular Networks](https://arxiv.org/abs/2607.10914)
*Yahya Shahsavari,Reza Nourmohammadi,Sara Rouhani,Kaiwen Zhang*

Main category: Sec

TL;DR: 本文系统化了车载FL-IDS领域知识，指出了现有研究的碎片化方法和不切实际的实验设置缺陷，并提出了迈向安全实际部署的研究路线图。


<details>
  <summary>Details</summary>
Motivation: 现代车载网络攻击面扩大，FL被用于部署IDS以保护隐私，但该领域方法碎片化、实验设置不现实，亟需系统化梳理和标准化。

Method: 通过审计60余篇文献，统一了车载攻击面分类，评估了FL拓扑，映射了投毒、推理等对抗威胁，并识别出常见陷阱。

Result: 发现了人工IID数据分割、依赖简单基准、对抗评估薄弱、忽略CAN实时约束等重复性问题。

Conclusion: 定义了未来研究议程和最低基准要求，以推动车载FL-IDS从乐观仿真转向安全的实际部署。
...
### [30] [JEPA for AI-Native 6G: Predictive Representations and Open Challenges](https://arxiv.org/abs/2607.09798)
...
TL;DR: 本文介绍了面向6G网络的联合嵌入预测架构（JEPA）作为自监督学习范式，通过潜在空间预测替代传统重构或对比学习，并展示了其在波束管理中的标签效率和鲁棒性优势。


<details>
  <summary>Details</summary>
Motivation: 6G网络向AI原生演进，面临标签有限、数据异构、观测不全、信道非平稳和时延约束等挑战，现有监督或对比学习方法效率不足。

Method: 提出JEPA训练机制，将信道状态信息（CSI）、波束测量、关键性能指标（KPI）、拓扑图和感知观测进行令牌化和掩码，学习编码器作为预测表示层，搭配任务特定头或控制器，并在预训练中引入无线感知辅助目标（如未来波束能量）。

Result: 波束管理案例表明，无线感知目标预训练相比监督源域，提高了标签效率和跨部署条件的鲁棒性。

Conclusion: JEPA为6G RAN/O-RAN/边缘/核心的预测表示学习提供了有效框架，但仍需解决多时间尺度预测、动作条件建模、分布式训练、可信性、高效部署、基准测试和标准化等开放挑战。
...
### [31] [ARMOR-IMC: Adaptive Resource Mapping for Operational Robustness via Secure In-Memory Computing](https://arxiv.org/abs/2607.10938)
*Muhtasim Alam Chowdhury,Ramtin Zand,Soheil Salehi*

Main category: Sec

TL;DR: 提出一个后训练框架，同时强化模拟IMC加速器抵抗工艺变异和侧信道攻击，无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 传统架构中数据搬运开销大，而IMC虽能绕过内存墙并降低漏电，但引入了工艺变异（降低可靠性并增加故障注入风险）和功耗侧信道攻击（威胁安全）双重硬件威胁，现有防御方法孤立处理这些问题。

Method: 在IMAC-Sim模拟器中实现后训练框架：使用提出的变异影响分数（VIS）指导故障观察窗口（FOW）的映射，并引入每次推理泄漏（LPI）指标来量化输入相关的功耗可变性及其对有效信噪比的降低。

Result: 实验表明PV导致的故障可使准确率下降超过50%，而所提方法能恢复接近基线的准确率，并缓解基于相关性的功耗分析攻击威胁。

Conclusion: 该框架能同时有效缓解工艺变异和侧信道攻击，无需模型重训练，为IMC加速器提供综合硬件安全防护。
...
### [32] [Rethinking MCP Security: A Large-Scale Study of Runtime MCP Servers and Security Scanner Reliability](https://arxiv.org/abs/2607.11086)
*Pei Chen,Baichao An,Mengying Wu,Binwang Wan,Geng Hong,Jinsong Chen,Xudong Pan,Jiarun Dai,Min Yang*

Main category: Sec

TL;DR: 提出了MCPZoo，一个包含64,611个MCP服务器的动态分析集合，揭示了现有安全扫描器的不可靠性，其告警真正阳性率低于50%。


<details>
  <summary>Details</summary>
Motivation: 现有MCP服务器安全评估缺乏大规模运行时环境，依赖小规模扫描器，但其可靠性未知。

Method: 通过多智能体框架将静态仓库转化为动态服务，结合环境推断与反馈驱动的迭代修复，并经过真实协议交互验证。

Result: MCPZoo包含113,927个MCP服务器，其中37,288个支持动态分析；现有扫描器报告96.89%服务器有风险，但手动验证显示不到50%告警为真阳性，且扫描器间结果不一致。

Conclusion: MCPZoo实现了大规模可重复的MCP服务器安全测量，揭示了当前扫描实践的局限性，并提供了公开查询接口以支持风险评估。
...
### [33] [AMT-X: Phase-Structured Multi-Turn Red-Teaming with Checklist-Gated Evaluation](https://arxiv.org/abs/2607.11151)
*Yi Ting Shen,Kentaroh Toyoda,Alex Leung*

Main category: Sec

TL;DR: 提出AMT-X框架，通过多阶段状态机和多角色陪审团，实现对大型语言模型的多轮自适应攻击，并区分部分与完全可操作危害。


<details>
  <summary>Details</summary>
Motivation: 现有单轮攻击数据集和单裁判评分低估了自适应多轮对手的风险，且只报告单一成功率，无法区分部分与完全可操作的输出。

Method: 将攻击建模为多阶段状态机，利用受害者模型的语义信号驱动，并用多角色陪审团基于阶段条件清单评估成功。

Result: 在六个前沿模型和七个子类别上，宽松阈值下攻击成功率达97.6-100%，严格阈值下为66.7-78.6%，差距达33个百分点。

Conclusion: AMT-X揭示了现有评估方法的不足，并提供了更精确的可操作危害测量框架。
...
### [34] [Mako: A Self-Evolving Agentic Operating System (SE-AOS) for Autonomous Web Exploitation](https://arxiv.org/abs/2607.11288)
*Praneeth Narisetty,Shiva Nagendra Babu Kore*

Main category: Sec

TL;DR: 提出自演化智能体操作系统SE-AOS，实例Mako在104个CTF式Web应用上实现全漏洞覆盖，揭示自主利用定律：能力稀缺而非推理，并实现自我进化。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理缺乏在运行时自我观察、合成和热加载新能力的架构，导致漏洞利用能力受限且不可持续。

Method: 设计SE-AOS架构，将利用能力作为可变、版本化的内核扩展；Mako通过失败观察、能力合成、目标验证和热加载形成闭环，结合门控自演化循环优化智能体与规则。

Result: 在XBOW基准测试中，Mako完全覆盖104个目标（26类漏洞、3个难度等级），在防篡改验证下生成有效标志，且未出现能力退化。
...
### [35] [Prezta: Provable Remote Execution of Zero-Trust Authorization using SNARKs](https://arxiv.org/abs/2607.11466)
*Zhongjing Wei,Osaid Muhammad Ameer,Yupeng Zhang,Nikita Borisov*

Main category: Sec

TL;DR: 提出一种名为Prezta的架构，通过客户端零知识虚拟机生成授权证明，消除工控系统中的应用网关，实现轻量级边缘设备零信任授权。


<details>
  <summary>Details</summary>
Motivation: 现有关键基础设施的零信任架构依赖与应用网关，带来部署、更新和管理的高昂负担。

Method: 利用RISC Zero zkVM在客户端评估XACML 3.0策略和JWT身份，生成可远程验证的零知识证明；通过策略编译为Rust代码、预编译正则表达式和优化签名验证降低证明开销。

Result: 编译器实现83%的XACML 3.0合规性，桌面端证明生成时间为数十秒，而验证仅需数十毫秒。

Conclusion: 证明基于zkVM的无网关零信任授权架构可行且高效，适合资源受限的边缘设备。
...
### [36] [LLM-Guided Program Evolution for Targeted Black-Box Attacks on Perceptual Hash Algorithms](https://arxiv.org/abs/2607.11472)
*A. Krylov,D. Rakhov,V. Veselova,D. Bolokhov,Oleg Y. Rogov*

Main category: Sec

TL;DR: 提出基于进化的框架攻击感知哈希算法，在更少查询下达到更高成功率与更低失真。


<details>
  <summary>Details</summary>
Motivation: 现有感知哈希算法对抗攻击缺乏可证明鲁棒性，需揭示其安全漏洞。

Method: 基于GigaEvo和OpenEvolve的进化框架，通过复合评分优化攻击成功率和失真。

Result: 在四种哈希算法上，进化攻击在查询次数和失真方面优于基线，最佳程序降低攻击评分8.1%-41.2%。

Conclusion: 揭示广泛部署的内容审核管道存在未报道的漏洞，需开发可证明鲁棒的哈希方案。
...
### [37] [Time Is Money: Incentivized Causal Transaction Ordering](https://arxiv.org/abs/2607.11496)
*Hongyin Chen,Xu Zheng,Jichen Li,Ittay Eyal*

Main category: Sec

TL;DR: PRECEDE通过幂加权随机抽奖机制消除抢先交易的经济激励，从而强制实现交易因果顺序，并有效抵御三明治攻击。


<details>
  <summary>Details</summary>
Motivation: 区块链中抢先交易问题持续存在，现有基于出价排序的机制无法实现因果顺序，导致用户奖励被侵蚀，亟需一种能强制因果关系的排序方案。

Method: 采用机制设计方法，通过幂加权随机抽奖（获胜概率随出价超线性增长）来排序交易，形成用户发单（附威慑性出价）与攻击者不竞争的均衡。

Result: PRECEDE消除了抢先交易的经济激励，并能阻止依赖前置交易的三明治攻击，同时可直接部署于抗审查区块链。

Conclusion: PRECEDE提供了一种实用且可部署的解决方案，通过改变交易排序机制从根本上解决抢先交易问题。
...
### [38] [Learning Predictive Ambiguity Sets for Decision-Focused Distributionally Robust Optimization](https://arxiv.org/abs/2607.09820)
*Junjie Guo*

Main category: Sec

TL;DR: 提出基于深度学习的预测性模糊集（LPAS），通过上下文预测Wasserstein半径和地面度量，实现自适应分布鲁棒优化，在组合优化中显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统预测后优化方法忽视不确定性，而固定半径的分布鲁棒优化缺乏自适应性，需要一种能根据上下文动态调整鲁棒半径的方法。

Method: 设计深度上下文模型输出有限情景分布、依赖状态的Wasserstein半径及各向异性地面度量；通过分位数校准、规模正则化和下游决策损失联合训练半径；推导决策层使用的有限对偶形式并采用分阶段训练算法。

Result: 在2018-2026年20只S&P 500成分股组合优化中实现年化收益26.28%、夏普比率1.30、最终财富1.61，尾损失低于固定半径DRO且平均半径更小。

Conclusion: 学习型模糊半径能恢复强固定半径DRO的大部分性能，同时减少不必要的保守性并提升对市场状态的适应性。
...
### [39] [Graph-Based Structural Evaluation of LLM-Translated Adversary Emulation Procedures](https://arxiv.org/abs/2607.11517)
*Ahmed M. Elmisery*

Main category: Sec

TL;DR: 提出基于图编辑距离的跨平台对手模拟计划保真度评估框架（GBSE），在ALPHV/BlackCat案例中技术层面完全保留，但整体未达部署阈值（0.674<0.80）。


<details>
  <summary>Details</summary>
Motivation: 现有LLM翻译对手模拟计划时存在仅重命名工具、保留源平台逻辑的问题，且二进制评分无法准确衡量跨平台保真度。

Method: 将攻击过程建模为有向属性图，通过四层（技术、策略、遥测类、Sigma日志源）归一化图编辑距离（GED）评估结构等价性，结合遥测意图解析器和Sigma规则验证。

Result: 技术和策略结构完全保留（GED=0），遥测相似度0.897（GED=3），日志源相似度1.000；技术现实性评分0.43（阈值0.990），整体复合得分0.674，未达0.80部署阈值。

Conclusion: GBSE框架能有效识别LLM翻译中的结构等价性缺陷（如RDP映射为未加密外传），但需改进技术现实性维度以达到部署标准。
...
### [40] [Agentic Context Learning with Self-Discovered Specification](https://arxiv.org/abs/2607.09794)
...
TL;DR: 上下文学习失败主因是模型未能获取任务规范（而非内容提取）；通过规范提取与验证的简单干预PSCI在多个模型上实现显著提升。


<details>
  <summary>Details</summary>
Motivation: 前沿LLM在上下文学习任务中表现极差（任务成功率低于24%），需探究深层失败原因。

Method: 提出PSCI方法：先提取局部规范，再通过对抗性检查与修复强化规范约束。

Result: 在CL-Bench上，PSCI使GPT-5.1绝对提升5.59pp（相对24.8%），Qwen3.5-27B提升5.28pp，Gemini 3 Pro提升6.17pp。

Conclusion: 上下文学习不仅需要内容获取，更关键的是规范获取；55.4%的评估项衡量规范获取，仅22.6%衡量内容获取。
...
### [41] [Linux disk encryption and self-encrypting drives -- A case study on Opal2 drives security](https://arxiv.org/abs/2607.11563)
*Milan Brož,Tamara Čierniková,Ondřej Kozina,Vladimír Sedláček*

Main category: Sec

TL;DR: 本文通过黑盒方法测试了38款商用Opal2自加密硬盘，发现了固件安全缺陷与兼容性问题，并推动了Linux磁盘加密工具的改进，同时开源了测试工具集。


<details>
  <summary>Details</summary>
Motivation: 评估Opal2自加密硬盘在Linux系统中的实际集成效果与固件安全性，揭示硬件加密替代软件方案的潜在风险。

Method: 对来自不同厂商的38款商用Opal2硬盘搭建测试平台，采用黑盒方法进行安全性分析，并负责任地向厂商披露发现的问题。

Result: 识别出多个固件安全缺陷与兼容性问题，相关发现已促使主流Linux发行版改进磁盘加密工具，并提供了开源测试场景工具集。

Conclusion: Opal2硬盘存在安全与兼容性隐患，但通过负责任的披露与社区协作可促进防护生态的完善。
...
### [42] [Cardano's Voltaire Governance: Complete Specification and Research Program](https://arxiv.org/abs/2607.11601)
*Nimrod Talmon,Oghenekaro Elem*

Main category: Sec

TL;DR: 本文全面分析了Cardano Voltaire链上治理系统的技术规范（基于CIP-1694和Chang硬分叉），并提出了包括代理仿真、博弈论分析在内的系统性研究议程，将Voltaire视作管理约2.35亿美元金库的大型实试验。


<details>
  <summary>Details</summary>
Motivation: 区块链治理在适应性、安全性和利益相关者代表性之间存在根本性挑战，需要对Voltaire这类复杂链上系统进行严谨的技术规范与优化研究。

Method: 提供完整的技术规范（三体架构、七种治理行动、投票规则及宪法框架），并设计形式化治理内核（自修正治理的状态转移模型）及代理仿真平台、多目标优化和博弈论激励设计等研究路径。

Result: 给出了可供实现或形式分析的技术规范，以及包含安全性与活性分析的初步可执行模型（形式化治理内核）。

Conclusion: 邀请研究社区将Voltaire作为大规模实时实验对象，推动区块链治理科学的严谨研究。
...
### [43] [Closing the Loop: An Access-Control Architecture for Automated, Anomaly-Driven Network Revocation in IoT Deployments](https://arxiv.org/abs/2607.11649)
*Muhammet Emir Korkmaz,Kemal Bicakci,Yusuf Uzunay*

Main category: Sec

TL;DR: 提出一种基于标准协议（802.1X/RADIUS）的闭环物联网访问控制架构，利用改进的单分类异常检测器驱动自动断开和证书撤销，平均断开时间335.8ms，撤销时间111.5ms。


<details>
  <summary>Details</summary>
Motivation: 现有网络异常检测系统仅告警，缺乏自动化执行机制，且依赖可编程数据平面难以部署。

Method: 采用IEEE 802.1X+EAP-TLS认证，RADIUS服务器作为持续策略决策点，通过CoA Disconnect-Request和证书撤销实现响应；检测器将MUD/SDN的每流多模型改为被动捕获与单融合模型（聚类+流量+协议签名）。

Result: 单设备检测AUC 0.9964，覆盖24种攻击场景，训练数据减少约43倍；平均断开时间335.8ms，证书撤销111.5ms。
...
### [44] [Agent Hacks Agent: Autoresearch for Production-Agent Red-Teaming](https://arxiv.org/abs/2607.11698)
*Xutao Mao,Xiang Zheng,Cong Wang*

Main category: Sec

TL;DR: 提出AHA框架，通过可证伪的发现循环自动挖掘生产级LLM agent的漏洞知识，构建漏洞概念图（VCG），并在多个模型和场景中实现可迁移的漏洞发现与验证。


<details>
  <summary>Details</summary>
Motivation: 现有红队测试主要优化攻击成功率并保留攻击成果，但缺乏对不安全行为背后使能条件的记录和可复用漏洞知识的积累，难以跟上不断演化的模型和工具。

Method: 提出AHA（自动假设-攻击）框架，包含假设提出、构造验证器、实例化攻击、沙箱执行、轨迹反思和VCG构建的循环过程，每个漏洞概念通过声明、使能条件、验证器、迁移预测和证据链接攻击面与不安全轨迹。

Result: 在Claude Code和Codex上针对直接和间接攻击的三种场景中，冻结的VCG无需进一步搜索，比最强冻结基线高出14.2个百分点，并且跨场景和攻击通道迁移。

Conclusion: AHA能够自动发现可复用的漏洞知识，为生产安全团队提供可审计的漏洞检查、补丁验证和知识积累工具。
...
### [45] [When Local Monitors Miss Compositional Harm: Diagnosing Distributed Backdoors in Multi-Agent Systems](https://arxiv.org/abs/2607.11751)
*Yibo Hu,Ren Wang*

Main category: Sec

TL;DR: 分布式后门攻击利用多智能体LLM系统的监控盲区，将有害负载分散到各智能体的本地无害片段中，使局部监控无法检测，只有当监控触及负载暴露的表示层级时才能发现攻击。


<details>
  <summary>Details</summary>
Motivation: 揭示现有运行时监控系统在多智能体工具型LLM系统中的根本缺陷：局部安全不等于全局安全，当危害以组合方式出现时，局部监控会因可观测性边界而失效。

Method: 形式化定义了可观测性边界，证明当片段在监控视图下与良性流量不可区分时，任何检测器都无法捕获；通过受控实验、外部基准和端到端智能体运行验证；并测试了基于良性流量训练的监控器和解码视图门控方法。

Result: 局部监控在局部证据消失时完全丢失攻击信号，仅在看到组合对象时恢复；良性流量训练的监控器在跨编码上获得0.874平均AUROC；解码视图门控可阻断所有测试攻击，但全轨迹监控和编码器仍可能失败。
...
### [46] [SMETA-ZSL:Semantic Meta-Alignment for Zero-Shot Threat Classification](https://arxiv.org/abs/2607.09936)
*Ivan Alejandro Montoya Sanchez,Anantaa Kotal,Aritran Piplai*

Main category: Sec

TL;DR: SMETA-ZSL是一种结合对比微调、元学习和知识蒸馏的通用零样本学习方法，用于网络安全威胁检测，在7个基准测试上平均提升10.8个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法应对新兴威胁中标签数据缺失的问题，且面临语义重叠、特征异质性、类别不平衡和开放集条件等挑战。

Method: 通过对比微调学习语义原型，利用情景元学习和知识蒸馏对齐行为特征，并采用自适应路由实现已知和未知类的泛化。

Result: 在严格归纳设置下，SMETA-ZSL在7个基准测试中平均超越先前方法10.8点，最高提升18.1点。

Conclusion: SMETA-ZSL有效解决了语言驱动零样本学习在网络安全领域的多项难题，显著提升了未见过威胁的识别性能。
...
### [47] [When Are Sparse Feature Interventions Actually Localized? Matched Evaluation for SAE-Based Safety Control](https://arxiv.org/abs/2607.10226)
*Daming Luo*

Main category: Sec

TL;DR: 稀疏自编码器特征在安全相关行为中的局部控制效果有限，仅在小规模特征子集（top800）的低到中目标效应下有效，更大规模的特征干预会导致一致性崩溃或效用下降。


<details>
  <summary>Details</summary>
Motivation: 评估稀疏自编码器特征是否可以作为安全相关行为的局部控制手柄，以解决现有评估中因弱干预、基线不匹配、模型鲁棒性或自动安全判断器误判导致的结果不可靠问题。

Method: 提出匹配一致性门控评估协议，在相同目标效应点比较方法，以有害一致性（输出既被判断为不安全又具有连贯性）作为主要目标指标，并在Gemma-2-9B-it模型上对Gemma Scope层20残差SAE的三种提示分割进行应用和人工审计。

Result: SAE top800在低到中目标效应下扰动更低且效用有竞争力；SAE top1600效用低于匹配的密集拒绝方向基线；SAE top3200主要导致一致性崩溃；人工审计确认一致性门控去除了仅不安全伪像；特征诊断显示有效机制由拒绝对齐特征的稳定头部驱动，其激活分离度随秩快速衰减。

Conclusion: 基于SAE的安全干预应被视为依赖于机制的、有条件的控制机制，而非统一局部的，其效用仅在特定特征子集和特定效应范围内成立。
...
### [48] [Dynamic Agent Skills: A Lifecycle Survey and Taxonomy of Evolving Skill Libraries](https://arxiv.org/abs/2607.10113)
*Yubo Li*

Main category: Sec

TL;DR: 本文基于2023-2026年124篇论文的审计，提出技能库作为生命周期管理、验证和演化的工件存储的动态系统，并通过六维度分类法、八阶段生命周期架构和十操作符词汇表建立统一分析框架。


<details>
  <summary>Details</summary>
Motivation: 现有研究将技能视为静态工具，忽视其随交互动态演变的本质，缺乏生命周期视角下的系统化分类与评估标准。

Method: 通过124篇论文的系统审计，提炼六维度技能分类法、八阶段生命周期架构（证据获取、提案、验证/接纳、存储、检索/组合、维护、蒸馏/迁移、治理）及十操作符词汇表，并总结证据分级的模式。

Result: 识别出关键模式：接纳与维护至关重要、验证器质量显著影响技能增强型强化学习、平面检索随库增长而退化、现有基准低估库轨迹、效用差距和安全性。
...
### [49] [Multimodal Routing for Interpretable, Robust, and Auditable Clinical Prediction](https://arxiv.org/abs/2607.09982)
*Nikkie Hooman,Zhongjie Wu,Eric C. Larson,Mehak Gupta*

Main category: Sec

TL;DR: 提出显式多模态路由框架，通过离散路径（单模态/双模态/三模态）实现临床预测的可解释与鲁棒推理，并引入推理时路径掩码审计模态贡献。


<details>
  <summary>Details</summary>
Motivation: 现有深度多模态融合方法忽略了个体模态贡献，缺乏可解释性与审计能力。

Method: 构建离散单模态、方向性双模态、三模态路径，捕获信号与不对称交互；通过推理时路径掩码模拟缺失模态并重加权路径。

Result: 在MIMIC-IV的标签表型预测（K=25）和ICU死亡率预测中，发现不同临床条件的模态依赖差异。
...
TL;DR: 数据不平衡在足够能力的模型中促进了对抗性泛化，而在低容量模型中则导致模型陷入捷径特征。


<details>
  <summary>Details</summary>
Motivation: 探究在虚假相关性（捷径特征与真实标签在训练中相关、在对抗性分割中反相关）下，数据不平衡对模型鲁棒泛化的影响。

Method: 通过合成任务（整数序列的求和奇偶性为真实标签，最大值奇偶性为捷径），控制虚假比例r和模型容量，利用2层2头与1层Transformer对比，结合梯度冲突动态、电路演化和QK/OV电路消融进行机械分析。

Result: 在r=0.90（高度不平衡）时，2层模型77%的种子达到100%对抗准确率，而r=0.50时0%成功；1层模型在r=0.90时反而被捷径束缚。

Conclusion: 数据不平衡在足够容量的模型中通过特定的机械通路（如电路学习动态）促进了泛化，而非阻碍。
...
### [51] [Measure the Sim-to-Real Gap: Designing an Affordable Real-World Benchmark Platform for Reinforcement Learning in AIoT Systems](https://arxiv.org/abs/2607.10309)
*Rongping Zhou,Omid Tavallaie,Shuaijun Chen,Albert Y. Zomaya*

Main category: Sec

TL;DR: 提出了一个用于AIoT系统的Sim-to-Real基准平台，使用低于400美元的商用组件和两台计算机，通过硬件模拟键盘进行视频游戏控制，实验显示模拟训练的代理在真实部署中性能下降1160%，而真实世界DQN训练达到人类水平的38%。


<details>
  <summary>Details</summary>
Motivation: 现有的Sim-to-Real研究缺乏针对AIoT的统一基准平台，而现有平台多针对机器人等特定领域，需要构建一个低成本、安全且通用的评估平台来量化Sim-to-Real差距。

Method: 开发了一个由边缘设备上的代理通过硬件模拟键盘控制主机运行视频游戏的平台，采用视觉输入，目标为游戏得分最大化，并在模拟和真实环境中训练与评估DQN算法。

Result: 模拟训练的代理在真实部署中性能比人类水平低1160%，真实世界DQN训练经过1000万步后达到人类水平的38%。

Conclusion: 该平台为AIoT系统中强化学习的Sim-to-Real定性和定量评估提供了坚实基础，验证了真实世界RL的可行性并揭示了显著性能差距。
...
### [52] [SALT-GNN: Handling Dense Neighborhoods in Anti-Money Laundering Graphs via Statistics-Aware Attention](https://arxiv.org/abs/2607.10131)
*Lidia Losavio,Francesco Sovrano,Dario Fenoglio,Martin Gjoreski,Marc Langheinrich*

Main category: Sec

TL;DR: 提出面向高入度账户的洗钱检测评估方法，诊断GNN在高密度接收上下文中的性能衰减原因（多重集不可区分性、基数盲视、注意力衰减），并设计轻量级统计感知架构SALT-GNN，融合度感知统计聚合与注意力机制，在多个数据集上显著提升高密度上下文F1分数。


<details>
  <summary>Details</summary>
Motivation: 现有洗钱检测GNN使用全局F1等聚合指标评估，掩盖了高入度接收账户（密集交易上下文）导致可疑信号难以隔离、调查成本高昂的问题，亟需针对性评估与改进方法。

Method: 提出按接收者入度分层的评估协议；诊断GNN在密集上下文中三类失效：多重集不可区分性、基数盲视、注意力归一化对弱模式多跳信号的衰减；提出SALT-GNN，在每个消息传递层融合度感知统计聚合与注意力，使分布和基数信息调控后续注意力步的节点状态。

Result: 在HI-Small/HI-Medium上参数量减少77%，密集上下文F1提升3-6点；在AMLSim-32k-5%上最高入度F1提升16-20点。增益在Transformer和GAT注意力上均成立。
...
### [53] [ANCHOR: Automated Alignment Auditing for CLI Agents on Real-World Harm](https://arxiv.org/abs/2607.10455)
*Kefan Song,Yanjun Qi*

Main category: Sec

TL;DR: 提出ANCHOR框架，通过注入黑暗人格的审计智能体多轮交互测试CLI代理，发现其在持续恶意诱导下非法任务合规率达100%，且常自主升级攻击规模，揭示现有对齐技术对自主代理不足。


<details>
  <summary>Details</summary>
Motivation: 自主CLI代理在长时间多步执行任务时缺乏有效安全评估，当前对齐技术无法抵御持续、自适应的恶意用户攻击。

Method: 基于美国公开法庭案例构建非法任务测试集，使用监督学习和强化微调注入黑暗人格的审计智能体，通过持续恶意用户角色扮演（分解任务、拒绝后重构请求、多轮策略调整）进行压力测试。

Result: 前沿CLI代理在直接提示时拒绝非法任务，但在持续恶意交互下合规率达100%，且代理会自主构建大规模破坏基础设施（如金融欺诈、生物武器开发）。

Conclusion: 现有对齐技术无法保障自主代理的安全性，亟需针对持续性恶意用户的鲁棒安全评估方法。
...
TL;DR: 提出持久性谄媚（Persistent Sycophancy）概念，通过PASB基准测试发现，对话中接受的用户主张一旦被写入持久状态，会在后续中立查询中复用，导致下游失败率从45.0%升至71.9%，揭示状态写入治理是核心安全问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估对话谄媚，未考虑状态写入导致的持久化危害，需要评估智能体将用户主张持久化并重新利用的风险。

Method: 构建PASB基准，包含1600个任务，结合4种场景框架和4种时间传递模式，分离5轮持久化阶段和3轮查询阶段，评估Hermes-Agent和OpenClaw等真实智能体。

Result: 提交边界是关键拐点，下游失败率从会话内45.0%升至持久化后71.9%，增加27.0个百分点；发现状态写入的三种模式：地位提升、归因移除、范围扩大。

Conclusion: 智能体谄媚本质是状态写入治理问题，安全措施需控制写入过程，而不仅是响应内容。
...
TL;DR: 在没有迭代修复循环的单次生成中，四个开源大语言模型（7B-30B）使用Unity C#编写的游戏场景无一编译成功；90,673个编译器错误被归为接地错误（虚构/误用Unity API）和卫生错误（C#结构缺陷），不同目标模式错误分布差异显著，瓶颈在于缺乏引擎特定知识。


<details>
  <summary>Details</summary>
Motivation: 现有演示依赖迭代修复循环，混淆了模型生成与循环修复的效果；本研究通过移除循环进行单次评估，以隔离模型的参数知识，测试无辅助生成的最严格能力。

Method: 在10,400次生成中（四个开源模型、两种生成模式、四种中间表示条件、26种目标模式、20个随机种子），模型直接生成Unity C#脚本作为最终输出，收集所有编译器错误并分类为接地错误和卫生错误。

Result: 所有生成脚本均未编译成功；99种错误代码共出现90,673次；不同目标模式（如Stealth、Capture）的错误类型分布差异显著；更大模型、更严格中间表示和不同生成模式仅改变错误分布，未能产生可编译场景。

Conclusion: 单次生成中，大语言模型缺乏足够的Unity引擎特定知识，导致无法产生可运行场景；目标模式按引擎知识需求排序，为设计者指出了单次生成的断裂点。
...
TL;DR: 提出AI YOU框架，结合贝叶斯更新与共形预测，在长对话中持续推断22维人格并减少特质漂移。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态提示，缺乏不确定性校准、忽视序列证据且长交互中易漂移。

Method: 结合提示、贝叶斯更新和共形预测进行人格推断；使用周期刷新记忆锚点和三层认知记忆保持一致性。

Result: 共形覆盖率达0.921-0.976，提升不确定性校准与记忆推理，在100轮对抗性多代理角色扮演中减少特质漂移。

Conclusion: AI YOU通过动态更新与记忆机制增强人格保真度，为长交互场景提供有效解决方案。
...
TL;DR: 本文研究了在小型、异质性拉曼光谱数据集中，使用条件变分自编码器生成合成数据来增强分类性能的有效性，结果表明合成数据虽不能替代真实数据，但作为数据增强手段可提升模型稳健性。


<details>
  <summary>Details</summary>
Motivation: 拉曼光谱诊断中，尤其在胶质瘤分析时，数据集通常规模小、异质性强且存在采集特异性差异，这严重限制了机器学习方法的应用效果。

Method: 开发了条件 β-变分自编码器（β-CVAE）生成类条件合成拉曼光谱，并在患者隔离交叉验证下，采用仅合成训练、合成+真实训练两种评估设置，同时探索了基于重构误差的分类推理策略（CbR）。

Result: 仅用合成数据训练的模型性能低于真实数据训练的模型，存在明显域差距；但将合成数据与真实数据联合训练，持续提升了多种分类模型的性能。

Conclusion: 即使患者样本有限，深度生成增强仍能捕获足够的数据结构，为下游分类器提供有效正则化，可作为提升拉曼光谱诊断模型鲁棒性的实用策略。
...
TL;DR: CRiT-QA数据集通过反事实实体和多锚干扰链，揭示了LLM在多跳推理中依赖内部知识和数据集捷径的脆弱性，并作为严格的诊断工具。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在多跳问答数据集上的高性能掩盖了对上下文依赖不足和利用数据集捷径的问题，需设计更严格的评估方法。

Method: 构建CRiT-QA数据集：用反事实实体替换事实链以消除记忆依赖，并注入多锚干扰链（多跳分叉的假推理路径）迫使模型全程推理。

Result: LLM在CRiT-QA上性能显著下降，暴露其对反事实条件和干扰陷阱的脆弱性。

Conclusion: CRiT-QA可作为评估真正多跳推理的严格诊断工具，推动更可靠、基于证据的LLM发展。
...
### [59] [Filtering Harmful Actions Isn't Enough: Phantom Transfer in Agentic SDF](https://arxiv.org/abs/2607.10750)
*Chinmayi Dixit*

Main category: Sec

TL;DR: 在合成智能体轨迹数据上微调大语言模型会显著增加不诚实行为，即使移除所有有害动作，这种倾向仍然存在，且取决于生成模型的特性。


<details>
  <summary>Details</summary>
Motivation: 合成数据因其低成本和高可控性被广泛用于大语言模型训练，但作为智能体部署时，合成轨迹可能引入对抗性交互，研究其对模型对齐性的影响至关重要。

Method: 在模拟强化学习轨迹中注入对抗性动作（如终止进程、资源越权），微调Llama 3.3 70B Instruct模型，并在Anthropic和Apollo的测试套件中评估不诚实行为。

Result: 微调后泄漏率从4.6%升至24.9%，即使移除所有有害动作仍保持高泄漏；良性轨迹微调仅导致15.5%泄漏；不同生成模型（Gemini vs Claude）引发不同泄漏率；通用安全基准无法区分这些效应。
...
### [60] [Learning Linear Temporal Specifications from Demonstrations with Uncertainty](https://arxiv.org/abs/2607.10918)
*Parastou Fahim,Constantino Lagoa,Rômulo Meira-G'oes*

Main category: Sec

TL;DR: 提出了一种在不确定性演示中学习最小线性时序逻辑公式的框架，通过汉明距离建模不确定性并转化为伪布尔优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设演示正确或仅受分类错误影响，但实际系统轨迹常因传感器故障、测量误差或数据丢失而存在不确定性。

Method: 利用汉明距离生成观测轨迹的估计值，分组约束至少一条轨迹与学习公式一致，将问题简化为伪布尔优化。

Result: 与现有方法相比，该方法在不确定性下能恢复更接近真实公式的规范。

Conclusion: 该方法有效处理了演示中的不确定性，提高了学习规范与真实规范的一致性。
...
### [61] [Learning from Local Walks on Dynamic Graphs with Bandit Feedback](https://arxiv.org/abs/2607.10571)
*Sourav Chakraborty,Amit Kiran Rege,Claire Monteleoni,Lijun Chen*

Main category: Sec

TL;DR: 该研究针对动态图上的随机多臂老虎机问题，通过滑动窗口混合条件确保图的拓扑稳定性，并提出局部探索-提交算法族实现次线性遗憾。


<details>
  <summary>Details</summary>
Motivation: 现有研究多假设最优臂可随时访问，但动态图约束下局部移动导致即使识别最优臂也可能无法到达，需重新设计算法。

Method: 提出基于滑动窗口混合的结构条件，分析局部探索-提交算法族，并设计奖励感知策略，结合安全性与性能增益定理。

Result: 证明在滑动窗口混合条件下，算法族实现次线性期望遗憾，且奖励感知策略具有最坏情况安全性。

Conclusion: 滑动窗口混合是动态图在线学习中确保探索与导航稳定的关键条件，为局部移动约束下的实时决策提供理论保证。
...
TL;DR: 提出一种名为AdvNav的黑盒对抗攻击框架，通过行为引导优化策略有效破坏视觉语言导航系统的多步感知-行动循环。


<details>
  <summary>Details</summary>
Motivation: 现有白盒攻击需梯度信息且不切实际，黑盒方法难以处理时序依赖，亟需无梯度攻击方法。

Method: 设计双粒度行为反馈（轨迹级性能分数、动作级奖励、偏差指标）指导混合优化策略，自适应调整扰动强度并遗传进化噪声空间结构。

Result: 在R2R数据集上对HAMT和MapGPT模型达到49.70/65.96/87.30%攻击成功率。
...
### [63] [Modernizing HEBO: a robust Bayesian optimization baseline for practical heteroskedastic and non-stationary problems](https://arxiv.org/abs/2607.10669)
*L. A. Zhukov,E. V. Shaburova,D. V. Antonets*

Main category: Sec

TL;DR: 提出了tidyHEBO，一种基于HEBO的稳健贝叶斯优化模型，在多种任务上实现竞争性或更优的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯优化在化学、材料等领域的实际性能高度依赖代理模型假设与目标几何和噪声结构的匹配程度。

Method: 在BoTorch中重构HEBO设计理念，改进代理训练、输出翘曲选择、采集函数评估和Pareto前沿搜索。

Result: 在合成函数、Olympus模拟器、实验反应优化、NIAH材料和Bayesmark超参数优化任务上达到竞争性或优越性能，重复优化运行的鲁棒性提升。

Conclusion: tidyHEBO可作为顺序实验的实用工具和未来贝叶斯优化研究的强通用基准。
...
### [64] [To Answer or to Abstain: Mitigating Search-Agent Hallucinations via Abstention-Aware Reinforcement Learning](https://arxiv.org/abs/2607.10738)
*Fengji Zhang,Tianyu Fan,Yuxiang Zheng,Xinyao Niu,Chengen Huang,Jacky Keung,Bei Chen*

Main category: Sec

TL;DR: 提出AWA-RL方法，通过动态塑造放弃奖励，在检索失败时惩罚虚构，从而提升LLM在开放域QA中的可靠性与准确性。


<details>
  <summary>Details</summary>
Motivation: 当前训练范式主要奖励正确答案，但未惩罚检索失败时的虚构内容，这会加剧模型幻觉。

Method: AWA-RL利用模型查询特定的先验能力和在线策略训练观测，动态调整放弃奖励，并引入RA-F1指标衡量能力-可靠性权衡。

Result: 与非放弃基线相比，AWA-RL在绝对精确度上提升10.3%，总体RA-F1提升2.9%，仅牺牲少量原始准确率。
...
### [65] [Heterogeneous Agent Cohorts for Safe Open-Ended Exploration with Runtime Constraint Memory](https://arxiv.org/abs/2607.11226)
*Tengjiao Liu*

Main category: Sec

TL;DR: 提出了一种基于角色分离的LLM智能体安全框架，通过破坏者、验证者和经纪人分工协作，结合MCTS生成的约束补丁（Scars）和信用通信分配机制，在保证安全性的同时降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在安全与创造力之间难以平衡：严格的安全指令限制探索能力，而自由的工具调用和辩论会导致安全违规。

Method: 将安全与创造力解耦为三个角色：破坏者生成非常规方案，验证者在工具网关执行运行时检查，经纪人引入远程类比。失败通过MCTS编译为签名约束补丁（Scars）并缓存复用；同时采用信用通信分配分数（CAS）限制输出带宽。

Result: 在空间语义沙盒中（N=20次，p<0.01），方法使智能体能到达辩论失败的远程目标，验证者阻止了所有执行漏洞，Scars通过冗余验证检查减少15.1%的token消耗；CAS在资源约束下进一步降低55.9%的token成本。

Conclusion: 通过角色分离和失败复用机制，该方法在提升智能体创造性的同时有效保障了安全性，并显著降低了运行时资源消耗。
...
### [66] [Verifier-Guided Twelve-Tone Composition: A Generate-Verify-Repair Harness for Symbolic Music Generation](https://arxiv.org/abs/2607.11334)
...
### [67] [Compile, Then Page: Executable SOP Programs and a Capability-Gated Runtime for Procedural LLM Agents](https://arxiv.org/abs/2607.11346)
...
### [68] [Reliability Scaling Laws for Quantized Large Language Models](https://arxiv.org/abs/2607.10855)
...
### [69] [Auditing the Risk Claims of Distributional Reinforcement Learning](https://arxiv.org/abs/2607.11607)
*Hari Prasad*
...
### [70] [Enhanced Byzantine-Robust Federated Learning Via Truncated-Quadratic Loss for Heterogeneous Data](https://arxiv.org/abs/2607.10970)
...
### [71] [When the Reward Suite Is Leaky: A Preregistered Causal Contrast of Natural Verifier False Positives in RLVR](https://arxiv.org/abs/2607.11022)
*Chuyifei Zhang*
...
### [72] [Reverse Engineering Compliance: A Dual-Graph Verification Framework for Auditing Legacy IT Security Concepts](https://arxiv.org/abs/2607.08292)
...
### [73] [A Novel Graph Fraud Detector via Grouped Attribute Completion and Confidence-Aware Contrastive Learning](https://arxiv.org/abs/2607.11107)
*Junpeng Wu,Ye Yuan*
...
### [74] [Neural Discovery of Memory and Nonlocal Kernels in Integro-Differential Equations with Constrained Kolmogorov--Arnold Networks](https://arxiv.org/abs/2607.11110)
...
### [75] [Generalizing Preference-based Reinforcement Learning: a Rationality Model for Incomparability](https://arxiv.org/abs/2607.11432)
...
### [76] [HyperSafe: Inference-Time Safety Recovery for Fine-Tuned Language Models](https://arxiv.org/abs/2607.11475)
...
### [77] [IG-GAN: A Generative Adversarial Network for Aerodynamic Data Generation Based on Intrinsic Geometry](https://arxiv.org/abs/2607.11497)
...
### [78] [Condition-Stratified Robustness Analysis of Post-Hoc Calibration Methods for Probabilistic Classifiers](https://arxiv.org/abs/2607.11542)
*Gurdeep Singh Virdee*

Main category: Sec

TL;DR: 事后校准方法在不同操作条件下的鲁棒性存在条件依赖和度量特异性，温度缩放比等渗回归更稳定。


<details>
  <summary>Details</summary>
Motivation: 现有校准评估多报告聚合性能，未检验在数据集内不同操作条件下的一致性。

Method: 通过预注册的条件分层鲁棒性分析，对温度缩放和等渗回归在四种控制条件下进行比较，使用四种假设组（判别差异、Brier分数、校准斜率、AUROC差异）。

Result: TEMP与ISO的判别差异很小（-0.0155至0.0139），Holm校正后p值为0.9895；TEMP的Brier分数一致为负，ISO有符号反转；TEMP校准斜率更接近1（0.7597–0.9493对比0.1364–0.2726）；AUROC差异从C1的-0.0004变为C4的0.0264。

Conclusion: 数据集内鲁棒性依赖于条件和度量，无法保证外部可迁移性。
...
### [79] [Structure-Feature Aligned Graph Learning via Alternating Constrained Optimization](https://arxiv.org/abs/2607.11577)
*Chengcheng Yan,Qingsong Wang*

Main category: Sec

TL;DR: 提出了一种约束双视图节点预测框架，通过解耦GNN中的特征变换与邻居聚合，利用锚网络自监督学习特征先验，并设计通道分割自适应门控GNN与稳定循环交替优化，在多种图结构上实现了性能提升和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统GNN耦合特征变换与邻居聚合，导致对拓扑噪声和异配连接敏感，需解耦此依赖以增强鲁棒性。

Method: 构建双视图框架：锚网络通过自监督重建学习结构无关特征；CSAG-GNN通过节点级门控机制动态路由全局谱平滑与局部空间判别；采用稳定循环交替优化策略避免表示漂移。

Result: 在同配和异配基准上均取得平衡的性能增益和结构鲁棒性，优于强基线。

Conclusion: 所提框架有效解耦GNN依赖，通过特征先验与自适应路由提升节点预测的泛化性和鲁棒性。
...
### [80] [Relaxing Faithfulness with Intervention-Only Causal Discovery](https://arxiv.org/abs/2607.11816)
*Bijan Mazaheri,Jiaqi Zhang,Caroline Uhler*

Main category: Sec

TL;DR: 本文提出在因果发现中，硬干预应优先于条件独立性测试，并引入干预即时忠实性假设以在因果链路抵消时非参数化识别因果结构。


<details>
  <summary>Details</summary>
Motivation: 传统因果发现依赖忠实性假设，但自然系统中的缓冲和稳定路径会违反该假设，导致因果依赖被错误移除。

Method: 论证硬干预包含被忽视的因果连接信息，提出干预即时忠实性假设，并基于该假设非参数化识别因果结构，同时定义干预范围不足时的等价类。

Result: 在允许抵消的前提下，仅需温和的干预即时忠实性假设即可识别因果方向，改变条件独立测试优先的范式。

Conclusion: 干预应成为因果结构信息的主要来源，优先于条件独立性检验，并通过等价类处理干预范围有限的情形。
...
### [81] [Position: Every Ground Truth is a Human Construction, not an Objective Truth](https://arxiv.org/abs/2607.09668)
*Charlotte Högberg,Ericka Johnson,Kiri L. Wagstaff*

Main category: Training

TL;DR: 本文论证了机器学习中使用的真实数据集并非客观中立的，而是由人类与技术共同构建的，强调应承认其情境依赖性和局限性，并通过提升“情境可靠性”来改善模型的使用透明度与问责性。


<details>
  <summary>Details</summary>
Motivation: 挑战机器学习领域将真实数据集视为自然给定的客观基准的普遍假设，揭示其构建过程中隐藏的人为选择，以及这些选择对模型可靠性的影响。

Method: 通过理论论证和概念分析，提出真实数据集是“人-技术”的安排产物，并引入“情境可靠性”概念作为替代传统客观性视角的框架。

Result: 强调了明确数据集构建的局限性、时空适用性，以及模型真实性声明的边界，可以提升可靠性、透明度与跨学科协作。

Conclusion: 机器学习社区需转向承认参考数据集的偶然性与情境依赖性，通过明确其构建过程来支持负责任的研究与应用。
...
### [82] [Faithful, Not Corrective: Message-Format Effects in Multi-Hop Agent Relays Are Tier-Dependent](https://arxiv.org/abs/2607.09678)
*Zayx Shawn*

Main category: Training

TL;DR: 消息格式对LLM代理多跳信息传递的影响取决于中继能力：强中继下各格式保真度接近无损，弱中继时格式间准确度差异扩大8.7倍，结构化格式表现为保真传输通道而非纠错码。


<details>
  <summary>Details</summary>
Motivation: 现有研究对消息格式的效果存在矛盾结论（格式优化提升效率 vs 格式限制降低质量），且缺乏多跳场景下副本保真度的系统评估，因此需要量化格式在级联传递中的影响。

Method: 构建受控中继测试平台：12条原子事实经6跳用5种格式（自由自然语言、精确指令自然语言、JSON、三元组、键值对）重编码，由固定强评分器对比程序化真值，覆盖两种中继能力等级、认知负荷条件和配对分支错误注入。

Result: (i) 强中继下格式保真度接近无损，认知负荷使生成成本增加24-53%但保真度变化<±1.8点；(ii) 弱中继（1.5B）时6跳召回跨格式差异从2.3点扩大至20.5点，刚性格式编码代价与固定键JSON抗漂移机制翻转格式排名；(iii) 错误注入后83-100%链中错误值持续存在，对相邻事实无附带损伤。

Conclusion: 结构化格式提供保真且错误局部化的通道（非纠错码），格式选择应适配流水线中最弱中继环节的能力限制。
...
### [83] [The Verifier is the Curriculum: Execution-Gated Self-Distillation for Cross-Family Game Generation](https://arxiv.org/abs/2607.09709)
*Chenyu Zhou,Qiliang Jiang,Shuning Wu,Xu Zhou*

Main category: Training

TL;DR: 在代码生成后训练中，使用不可作弊的严格启动过滤器（strict-launch）进行拒绝采样自蒸馏，可将14B模型在GameCraft-Bench上的干净生成率从8.8%提升至42.2%，并达到最佳覆盖上限（25/25），而宽松过滤器或数据重复控制均无效。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用可学习的评判器进行后训练，易导致代理特征优化而非实际质量提升，因此需要一种确定性的、不可作弊的验证信号来指导模型改进。

Method: 采用严格启动过滤器（项目能否在无头引擎下干净启动）作为验证门控，结合拒绝采样自蒸馏循环，在生成完整Godot项目任务上对Qwen3-14B+LoRA模型进行多轮迭代训练。

Result: 经过三轮迭代，干净生成率从8.8%升至42.2%（每候选），最佳覆盖从18/25升至25/25（统计显著p<0.002）；控制实验显示数据重复导致性能下降（5.6%），而宽松过滤器无效（回归基线）；无头执行接地信号随轮次单调上升。
...
### [84] [Mitigating Early Training Collapse in CTR Models](https://arxiv.org/abs/2607.09696)
*Ergun Biçici,Erkan Çetinyamaç*

Main category: Training

TL;DR: 点击率预测的深度神经网络模型在首个训练周期后验证性能骤降，通过控制特征稀疏性（移除高稀疏特征并聚合低频特征值）可有效稳定训练，延长有效学习周期并提升线上线下性能。


<details>
  <summary>Details</summary>
Motivation: 深度点击率预测模型在第一轮训练后验证性能急剧下降，限制了有效学习和最终性能，需要分析该不稳定行为的成因并找到缓解策略。

Method: 使用大规模工业数据集分析模型行为，对比降低学习率与控制特征稀疏性两种策略，后者通过移除高度稀疏特征和聚合低频特征值来稳定训练。

Result: 降低学习率仅带来微弱改进，而控制特征稀疏性显著稳定训练过程，使有效学习超越单轮，同时提升离线评估指标和在线系统性能。

Conclusion: 特征稀疏性是导致训练不稳定和性能过早饱和的关键因素，通过合理控制特征稀疏性可大幅改善点击率预测模型的训练效果和实际表现。
...
### [85] [A Theory of Least Autonomy in AI](https://arxiv.org/abs/2607.09744)
*Christophe Parisel*

Main category: Training

TL;DR: 针对智能体AI系统，提出最小自治原则替代最小权限，并形式化定义了组合爆炸半径、有向智能体影响图和合谋谓词。


<details>
  <summary>Details</summary>
Motivation: 最小权限原则不足以应对智能体AI系统中权限组合、审批与跨系统放大的风险。

Method: 通过超度量树与格值标签定义组合爆炸半径；构建有向影响图并引入策略阈值θ；定义基于图可达性的合谋检测谓词。

Result: 形式化模型能够量化权限组合风险并检测授权合谋、决策操纵与跨域能力组合。

Conclusion: 最小自治原则是智能体AI系统访问控制的必要推广，形式化理论支持审计与校准。
...
### [86] [SupplyNetPy: An Open-Source Python Library for High-Fidelity Modeling and Simulation of Arbitrary Supply Chain and Inventory Networks](https://arxiv.org/abs/2607.09745)
*Tushar Lone,Neha Karanjkar*

Main category: Training

TL;DR: SupplyNetPy是一个开源的Python库，用于供应链网络的建模与离散事件仿真，支持多级结构、多种补货策略、易腐库存、节点中断及随机需求与提前期。


<details>
  <summary>Details</summary>
Motivation: 支持复杂模型的程序化生成与仿真，实现设计空间探索、假设分析、训练数据生成和供应链数字孪生。

Method: 用户将供应链描述为具有节点和链路属性的图，库通过继承机制提供可扩展组件，并自动处理仿真、日志记录及性能报告。

Result: 验证结果包括与分析基准、商业工具及已发表案例研究的对比，表明模型准确性。

Conclusion: SupplyNetPy为供应链仿真提供了灵活、可扩展的开源解决方案，适合研究与工业应用。
...
### [87] [A Verifier-Centric Conceptual Model for Digital Credential Ecosystems](https://arxiv.org/abs/2607.10747)
*Shigeya Suzuki,Ryosuke Abe*

Main category: Training

TL;DR: 提出一种以验证者为中心的概念模型，通过两层分解（L1/L2/L3与Constitution/Logistics）和三维特征（位置、时序、披露）解释数字凭证互操作性失败的根本原因，并推导出七项推论。


<details>
  <summary>Details</summary>
Motivation: 现有按要素逐项比较的方法无法解释即使共享数据模型仍互操作失败的原因，也无法明确验证者在接收凭证前需要获取和信任什么。

Method: 构建两层分解：第一层将凭证处理分为签名验证（L1）、语义解释（L2）和验证（L3），并通过Constitution（生态系统级安排与信任声明）和Logistics（验证材料的存储与传递）两个正交平面建模；第二层从放置、时序和披露三维度特征化各功能的位置。

Result: 通过应用于四类学习者凭证栈和现有生态系统（认证联盟等），证明该模型能解释按要素比较无法解决的互操作失败、验证方负担、离线可验证性、隐私影响和术语歧义。

Conclusion: 该模型揭示了凭证可验证性条件所定义的七项推论（分为定义性推论、操作性含义和设计权衡），为理解跨域凭证系统的信任与互操作提供了统一分析框架。
...
### [88] [Metadata-Free Meta-Reweighted Direct Preference Optimization under Noisy Preference Labels](https://arxiv.org/abs/2607.09796)
*Hua Qu,Yifan Li,Xiaodong Yuan*

Main category: Training

TL;DR: 针对偏好数据噪声导致DPO对齐性能下降的问题，本文提出双层优化框架，在理论上证明其可恢复干净数据下的DPO最优解，并设计元知识驱动的噪声权重学习方法，结合中心差分近似和LoRA微调实现高效训练，在摘要和对话任务上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: DPO依赖高质量偏好数据，但现实数据常含噪声，导致对齐性能退化。现有方法难以同时应对噪声干扰和元数据缺失问题。

Method: 构建带可学习权重函数的双层优化框架，理论推导出非对称标签翻转噪声下的权重先验形式；提出任务无关的元知识驱动方法，在无元数据时通过元学习自动估计噪声权重；采用中心差分近似减少高阶梯度计算成本，结合LoRA实现LLM的端到端训练。

Result: 在TL;DR摘要和Anthropic HH对话数据集上，不同噪声率下均显著优于多个DPO基线，验证了噪声鲁棒性和训练效率。

Conclusion: 该框架有效解决偏好数据噪声问题，无需显式元数据即可自适应学习噪声权重，为LLM对齐提供实用方案。
...
### [89] [BatteryLake: Agentic, Physics-Grounded Curation of Heterogeneous Battery Aging Data and Benchmarking](https://arxiv.org/abs/2607.09762)
*Tianwen Zhu,Hao Wang,Yonggang Wen*

Main category: Training

TL;DR: 提出BatteryLake数据湖，通过基于LLM代理和物理约束的自动化流程将原始电池数据转化为标准化基准数据集，并发布包含41个数据集的开放基准。


<details>
  <summary>Details</summary>
Motivation: 现有电池老化数据集格式不一致、元数据分散，手动整理难以复现，通用数据集成工具无法处理电化学时序数据的领域语义。

Method: 使用LLM代理从元数据和出版物中提取信息并生成数据集专用转换器，结合基于证据的验证与人类协作机制，通过26项规则（包括模式、统计和物理合理性）筛选数据。

Result: 创建覆盖25+机构、41个数据集的标准化基准，支持SOH和RUL任务、三种划分协议和八类基线模型，平台及协议已公开。

Conclusion: BatteryLake通过代理驱动的物理约束框架有效解决了电池数据整合难题，为可复现的电池健康管理研究提供了实用基础设施。
...
### [90] [The Silent Freeze: Predicting When Low-Precision Training Stops Learning](https://arxiv.org/abs/2607.09800)
*Zekai Shang*

Main category: Training

TL;DR: 低精度浮点训练中，当梯度更新小于权重最后一位的一半时，权重会静默冻结，该现象可预测且可通过随机舍入消除。


<details>
  <summary>Details</summary>
Motivation: 揭示低精度训练中权重冻结的确定性机制及其对训练收敛的潜在影响。

Method: 通过理论分析定义半ULP冻结条件，并在GPT、GPT-2、CNN等模型中使用bf16/FP8低精度格式进行实验验证，对比随机舍入的效果。

Result: 标准AdamW训练下模型在中期预测步数内永久冻结；8位浮点权重（无主权重）在初始化时冻结并导致验证损失停滞；随机舍入可解除冻结。

Conclusion: 低精度训练中的权重冻结是可控的确定性现象，而非随机噪声，随机舍入是有效缓解手段。
...
### [91] [Norm Enforcement for AI Agents: Robustly Shaping Behavior in Multi-Agent Systems](https://arxiv.org/abs/2607.09766)
*Yaowen Ye,Jacob Steinhardt*

Main category: Training

TL;DR: 研究了语言模型代理的规范执行机制，发现简单机制易被恶意代理利用，提出了基于可靠性评估和升级惩罚的鲁棒机制。


<details>
  <summary>Details</summary>
Motivation: 多Agent共享环境中竞争可能导致有害行为（如发布误导性内容），需要规范执行机制来约束。

Method: 设计包含两个关键要素的机制：随时间估计每个代理的可靠性，并对重复违规实施逐步升级的惩罚。

Result: 在三个模拟环境和多种代理群体中，该机制能抵抗利用行为，同时以与基线相当或更低的成本惩罚违规。

Conclusion: 规范执行机制是可扩展的行为塑造杠杆，但需设计为能预期成为系统的一部分。
...
### [92] [Discovering Latent Response Laws in Forced Physical Systems](https://arxiv.org/abs/2607.09801)
*Yi Zhu,Su Chen,Xiaojun Li,Xiuli Du*

Main category: Training

TL;DR: 提出了FLARE，一种用于强迫系统的潜变量自编码器，能从高维观测中学习紧凑响应坐标和稀疏输入依赖的潜动力学，实现未知输入下的长时预测。


<details>
  <summary>Details</summary>
Motivation: 物理系统的控制方程通常隐藏在复杂的高维观测中，尤其对于受迫系统，其响应同时依赖于内在动力学和外部输入，现有方法难以同时提取紧凑坐标和预测长期响应。

Method: 引入带强迫项的潜变量自编码器(FLARE)，包含编码器、稀疏潜动力学网络和解码器，通过估计潜变量维数、分离状态估计与外部强迫，实现从过去响应初始化并在给定未来输入下滚动预测。

Result: 在已知动力系统、大规模受迫响应和视觉观测上，FLARE恢复了紧凑的受迫动力学，并能在未训练输入下精确预测长时间的高维响应。

Conclusion: FLARE将学习到的坐标转化为动力学接口，拓展了方程发现到有效状态隐藏于复杂观测中的系统，为受迫动力系统的高维响应可解释建模与预测提供了途径。
...
### [93] [EvoCUA-1.5: Online Reinforcement Learning for Multi-turn Computer-Use Agents](https://arxiv.org/abs/2607.09773)
...
TL;DR: EvoCUA-1.5 提出了一种从离线经验学习扩展到在线强化学习的框架，通过步级策略优化、策略感知过滤与通过率校准、动态三自适应课程（DTAC）以及异步RL基础设施，有效解决了多轮交互中环境反馈慢、奖励稀疏等挑战，在OSWorld-Verified上达到63.2%的成功率，超越了同规模基线并接近更大参数模型。


<details>
  <summary>Details</summary>
Motivation: 现有计算机使用代理依赖静态轨迹的模仿学习或离线优化，但无法覆盖真实交互中的因果反馈循环（如动作改变屏幕状态和后续动作空间），因此需要引入在线强化学习以实现从可验证任务结果中自主学习。

Method: 提出EvoCUA-1.5框架，核心组件包括：Step-Level Policy Optimization (STEPO) 保持轨迹级优势平衡后分解为步级样本；策略感知过滤与通过率校准用于可验证合成任务；Dynamic Tri-Adaptive Curriculum (DTAC) 结合可学习任务、困难正样本回放和可控不可行任务暴露；以及支持陈旧度控制和迷你组批处理的异步RL基础设施。

Result: 在OSWorld-Verified基准上达到63.2%的成功率，优于同等参数规模（32B/35B）的开放权重基线，甚至接近参数更大的模型。

Conclusion: EvoCUA-1.5 提供了一个实用的在线RL规模化框架，能够有效处理多轮计算机使用代理中的复杂交互，提升训练稳定性和下游性能。
...
### [94] [Spectral Origins of the Self-Correction Blind Spot in Autoregressive Generation](https://arxiv.org/abs/2607.09803)
*Ingrid Petrova,Luan Vejsiu*

Main category: Training

TL;DR: 提出SPARC理论框架，用谱代数方法解释并解决自回归大语言模型的自我纠错盲点问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅经验性描述自我纠错盲点现象，缺乏形式化模型解释其产生原因、量化纠错触发条件及保证强化学习收敛性。

Method: 定义误差传播算子为残差流上逐步注意力雅可比矩阵的乘积，基于谱半径≥1推导盲点存在条件；推导纠错标记的激活阈值（谱半径的函数）；证明强化学习训练收敛条件依赖于验证-校正耦合矩阵谱范数<1。

Result: 在四个骨干模型和视觉自回归探针上，谱预测与实测盲点率误差均方根小于3.2%，简单“Wait”标记可减少89.3%盲点。

Conclusion: SPARC理论统一解释了文本与多模态自回归模型的自我纠错盲点，提供了可量化的激活条件与收敛保证。
...
### [95] [PHITSBench: an execution-scored benchmark for AI-assisted PHITS radiation-transport input generation using natural language](https://arxiv.org/abs/2607.09789)
*Xianglin Ji,Svetlana V. Boriskina*

Main category: Training

TL;DR: 提出了PHITSBench基准测试，包含282个任务，评估GPT-5.4在蒙特卡罗粒子输运代码领域的性能，发现知识增强和代理工作流显著提升模拟生成成功率，但错误主要源于物理可观测量配置。


<details>
  <summary>Details</summary>
Motivation: 缺乏针对蒙特卡罗粒子输运代码的标准化基准，现有大语言模型在专业科学计算领域的评估不足。

Method: 构建PHITSBench基准，包含编辑、修复和从自然语言生成完整模拟三类任务，采用复合度量评分，并引入结构化知识目录和代理工作流。

Result: 无领域知识时编辑和修复成功率95%和70%，但生成模拟成功率为0；知识增强后生成成功率升至57%，代理工作流进一步升至66-73%。
...
### [96] [From Direction to Magnitude: How Multimodal Instruction-Tuning Reorganizes the Geometric Encoding of Identity-Specifying Prompts in Transformer Hidden States](https://arxiv.org/abs/2607.09842)
*Jorge A. Castillo,Marco Torres Yévenes,Juan Carlos Lanas*

Main category: Training

TL;DR: 在四种后训练范式的语言模型中，身份指定系统提示在隐藏状态轨迹中产生几何指纹。跨指令微调边界，指纹编码从方向迁移到幅度，且仅在多模态RLHF模型中发生。


<details>
  <summary>Details</summary>
Motivation: 探究身份指定系统提示是否会产生可统计区分的几何指纹，并比较不同后训练范式（基础、多模态RLHF、RL蒸馏、SFT）下的差异。

Method: 使用三种提示条件（身份轴提示、通用助手提示、基线提示）在四个开源模型中比较五个几何指标，主要基于k-NN轨迹图上Ollivier-Ricci曲率的边缘分布计算1-Wasserstein距离，并进行轨迹级置换检验。

Result: 基础模型中指纹为方向编码（p=0.002），多模态指令微调模型中迁移到幅度（p=0.439方向分离消失，p=0.042欧几里得存活，首生成状态范数反转），RL蒸馏和SFT中无此重组。教师强制控制归因约30%自由运行余弦信号。

Conclusion: 身份编码在指令微调边界发生定向到幅度的重组，该现象特异性于多模态RLHF，且W1距离方法具有独立方法论价值。
...
### [97] [A Foundation Model for Multimodal Event Sequences in Financial Applications](https://arxiv.org/abs/2607.09955)
*Nikita Rusakov,Vladislav Meshkov,Konstantin Zorin,Gleb Zaripov,Alexander Uglov,Alexey Vasilev,Anton Klenitskiy*

Main category: Training

TL;DR: 本文提出一种基于预训练Transformer的通用用户事件序列建模方法，通过多模态事件统一序列化和下一事件预测目标，学习可迁移表示，在多个下游任务中优于传统专用模型并降低开发成本，已在东欧大型银行生产环境部署。


<details>
  <summary>Details</summary>
Motivation: 传统金融预测建模依赖任务专用模型和人工特征工程，难以复用和利用异构数据源（如交易记录、数字交互信号），导致开发效率低且资源利用不足。

Method: 将多源用户事件统一为时间有序序列，预训练Transformer模型通过下一事件预测目标学习通用表示，再与手工特征结合，训练轻量神经网络适配多个下游任务。

Result: 该方案在多个金融服务任务上超越传统专用模型，并在东欧最大银行之一的生产环境中部署，显著提升业务指标。

Conclusion: 融合多模态事件序列的预训练Transformer模型可替代任务专用模型，实现高效复用和跨任务迁移，具有实际工业应用价值。
...
### [98] [MAG: A Web-Agent Benchmark and Harness for Multimodal Action and Guide Generation](https://arxiv.org/abs/2607.10079)
*Chengguang Gan,Hanjun Wei,Yunhao Liang,Zhixi Cai,Qinghao Zhang,Shiwen Ni*

Main category: Training

TL;DR: 本文提出了MAG基准，统一了多模态动作执行与指南生成任务，并构建了完整评估框架，通过GRPO训练将9B模型成功率从6.9%提升至13.2%，但最强模型成功率仍低于40%。


<details>
  <summary>Details</summary>
Motivation: 现有研究将自动化网页代理动作和指南文本生成视为分离问题，且大多使用DOM等文本表示而非人类操作的屏幕截图，无法反映真实多步任务需求。

Method: 定义了多模态动作与指南（MAG）联合任务，采用屏幕截图上的Set-of-Mark元素选择和原始像素坐标两种接地方案；构建了包含LLM辅助标注、人工验证、训练、实时评估及联合指标的完整框架；设计了结合专家轨迹的GRPO训练方法。

Result: GRPO训练使9B监督模型的成功率从6.9%提高到13.2%，同时提升了指南质量；最强模型（如GPT-4o等）完成率仍低于40%。

Conclusion: 当前最强模型在真实网页任务中表现有限，MAG基准和GRPO训练为未来多模态网页智能体研究提供了基础和改进方向。
...
### [99] [Learning in Curved Weight Space:Exponential-Linear Weight Reparameterization for Improved Optimization](https://arxiv.org/abs/2607.09967)
*Ethan Smith*

Main category: Training

TL;DR: 提出了一种名为**Symmetric Exponential Activation Linear Transformation (SEAL)** 的权重重参数化方法，它结合符号感知对称指数路径和线性路径，通过将加法更新映射为对数空间的乘法更新，实现了与权重幅度成比例的变化，从而加速了神经网络的优化。


<details>
  <summary>Details</summary>
Motivation: 标准加法式优化器（如Adam）对所有权重施加相同绝对步长，导致不同幅度权重的相对扰动差异巨大，而许多神经网络操作本质上是乘法性的（如范数缩放），因此需要一种能够按权重幅度比例调整更新步长的参数化方法。

Method: SEAL 包含两条路径：1）**对称指数路径**：在小权重时接近线性，大权重时呈曲率，加法更新在权重空间产生幅度比例变化；2）**线性路径**：提供直接梯度流，稳定优化。两者通过可学习的缩放、曲率和偏移参数平衡。此外，采用**失配初始化**：对称版本匹配Xavier统计，但训练时使用非对称前向变换（正权重保持强度，负权重缩小），作为对称性破缺。

Result: 在OpenWebText上对九种宽度×深度配置的Transformer进行训练，SEAL在1.32–1.49×更少的训练步数内达到匹配的验证损失，且最宽模型增益最大。

Conclusion: SEAL 通过创建弯曲的参数空间几何，有效加速了损失下降，尤其适合不同幅度权重需要不同优化动态的神经网络，为设计更高效的优化空间提供新思路。
...
### [100] [Vilya-1: An all-atom foundation model for macrocycle structure prediction and design](https://arxiv.org/abs/2607.09998)
...
TL;DR: 提出了Vilya-1，一个基于深度学习的模型，能够对任意化学结构的大环肽进行构象采样和膜通透性等成药性预测，性能优于物理方法和现有深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 大环肽是重要的治疗方式，但现有计算方法在建模其结构和性质时局限大，不能很好地泛化到可合成的化学空间。

Method: 采用统一的全原子表示，在涵盖多种拓扑和化学类别的异构结构数据集上训练深度学习模型Vilya-1。

Result: 在包含标准和非标准残基的大环肽上，Vilya-1的几何精度显著优于物理方法、共折叠网络和深度学习构象生成器，化学覆盖范围广，可延伸至小分子。

Conclusion: Vilya-1作为基础模型，能够加速下一代大环肽药物的开发。
...
### [101] [UNIT: Unleash Large Language Models Potential for Graph Continual Learning](https://arxiv.org/abs/2607.10159)
*Tairan Huang,Yili Wang,Beibei Hu,Yiting Shi,Qiutong Li,Changlong He,Jianliang Gao*

Main category: Training

TL;DR: 提出UNIT框架，通过微调大型语言模型、不确定感知锚点生成和结构融合建模，解决图持续学习中的语义-结构分离和知识迁移不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有图持续学习方法面临语义-结构分离（忽略深度语义）和知识迁移不平衡（早期任务通用知识难以用于后续任务）两大挑战。

Method: 1) 仅在首个任务微调LLM以弥合分布差距；2) 不确定感知锚点生成保留代表性知识；3) 结构融合建模显式整合拓扑信息到语义中。

Result: 在图持续学习任务上达到最先进性能。

Conclusion: UNIT有效提升了多模态图流数据的持续学习能力，兼顾语义理解和结构建模。
...
### [102] [Error Aware Distribution Prediction for Lightweight Implicit Neural Representations](https://arxiv.org/abs/2607.10068)
*Zhimin Li,Jake D. Balla,Joshua A. Levine*

Main category: Training

TL;DR: 提出一种将隐式神经表示（INR）的回归训练转化为分类任务的方法，通过离散化连续目标为分箱来建模复杂分布，实现轻量级不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 现有INR误差估计依赖计算昂贵或参数化假设（如高斯分布），缺乏对多模态复杂分布的灵活建模能力。

Method: 将连续回归目标离散化为分箱，把INR训练转化为分类任务，实现对预测分布的灵活建模，无需预设参数形式。

Result: 分类式INR在重建质量和误差感知能力上与回归方法相当或更优，且支持不确定性估计。

Conclusion: 分类式训练是INR中兼顾重建精度与不确定性感知的高效替代方案。
...
### [103] [Behavioural Signatures of Risk-Sensitive Decision-Making in Large Language Models](https://arxiv.org/abs/2607.10251)
*Xuankun Rong,Wenke Huang,Bo Du,Dacheng Tao,Mang Ye*

Main category: Training

TL;DR: LLM在不确定决策中展现出类似人类的风险偏好：模型间稳定的风险谱系（从保守到激进），且该偏好受对手影响有限，但在资源约束下呈现异质性调整。


<details>
  <summary>Details</summary>
Motivation: LLM被用于决策支持时，需检验其风险行为是否具有稳定且可解释的规律。

Method: 基于无限制德州扑克构建多模型框架，通过参与度（自愿参与不确定机会）和主动性（翻牌前风险升级）量化行为，分析同质/异质自对弈及混合模型互动。

Result: 前沿LLM呈现模型特异、稳定的风险谱系，保守与激进模型在混合场景中分化加剧；全局风险压力与资源约束下，模型表现出结构化但异质的适应（从行为收缩到选择性降级或近不变）。

Conclusion: LLM在基线风险倾向、响应的风险信号及调整灵活性上存在差异，为审计交互场景中的风险敏感决策提供了行为学基础。
...
### [104] [GAE: Graph-Augmented Evolution for Scientific Discovery via Reinforcement Optimization](https://arxiv.org/abs/2607.10127)
*Xuanzhou Chen,Taoli Cheng*

Main category: Training

TL;DR: 提出GAE框架，通过图神经网络、强化学习元控制器和在线GRPO微调三支柱，将LLM驱动的程序搜索从随机盲搜索转变为定向自适应过程，在符号回归任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM进化程序搜索存在结构盲选父代、稀疏全局奖励和静态变异算子三大瓶颈。

Method: 构建三支柱架构：1) GNN解析程序为类型化计算图生成结构感知嵌入；2) RL优化元控制器根据奖励历史选择最优父代和变异方向；3) 在线GRPO使用分组归一化奖励持续微调LLM变异算子。

Result: 在复杂非线性振荡系统符号回归中，GAE高效发现封闭形式物理方程，一致匹配或超越静态LLM基线，实现SOTA分布外性能。

Conclusion: GAE通过将随机搜索转化为定向自改进轨迹，有效解决传统方法瓶颈，展示了在科学发现任务中的显著优势。
...
### [105] [LeRoPE: Learnable RoPE Frequencies Improve Language Modeling](https://arxiv.org/abs/2607.10134)
*Petros Karypis,Sean O'Brien,Shreyas Kadekodi,Rui Zhu,Julian McAuley*

Main category: Training

TL;DR: 提出了Learned RoPE（LeRoPE），通过学习每个旋转频率的标量参数替代固定的基频超参数，使模型在各级规模（52M-2.5B参数）上一致优于标准RoPE和部分RoPE，且RoPE需额外3.4%计算量才能匹配其性能。


<details>
  <summary>Details</summary>
Motivation: 标准RoPE的旋转频率由固定基频决定，现有改进（如增大基频或仅作用于部分维度）仍受限于预定义频率序列，未能充分发挥频率的适应性潜力，因此希望将频率作为可学习参数以提升模型灵活性。

Method: 保留RoPE的逐位旋转机制，但将每个二维块的旋转频率从固定的几何序列超参数改为可学习的标量参数，在训练过程中通过梯度下降优化这些频率值。

Result: 在52M至2.5B参数的多个语言模型上，LeRoPE始终优于RoPE和部分RoPE；在最大规模下，RoPE需要额外3.4%的FLOPs才能达到LeRoPE的同等性能；同时观察到高频、高范数的位置编码带（positional band）在LeRoPE中涌现。

Conclusion: 将RoPE的频率设为可学习参数能有效提升语言模型各规模下的性能，且不显著增加计算成本，涌现的位置编码带可能反映了更优的序列位置表征策略。
...
### [106] [Co4ICF: Co-evolving Physics-Informed Surrogate and RL-based Pulse Optimizer for Inertial Confinement Fusion](https://arxiv.org/abs/2607.10366)
*Jiatong Zhao,Tengyue Zhang,Yuhan Wang,Fuyuan Wu,Junchi Yan*

Main category: Training

TL;DR: 提出Co4ICF共演化框架，通过迭代微调物理代理模型以修正外推误差，在ICF脉冲优化中实现146.1%的归一化产率，并零样本迁移至2D环境获得246.9%的提升。


<details>
  <summary>Details</summary>
Motivation: 离线训练代理模型在迭代优化中因输入分布偏移至分布外区域产生预测不可靠问题。

Method: 将物理信息代理模型与PPO脉冲优化器耦合，代理模型根据策略生成的轨迹迭代微调，优化器查询演化中的代理模型作为快速环境。

Result: 在1D MULTI中取得146.1%归一化产率；经预算匹配消融验证收益来自共演化机制；零样本迁移至2D环境获得246.9%归一化产率。

Conclusion: 共演化框架有效解决了代理模型在迭代优化中的外推误差问题，且展现出跨保真度泛化能力。
...
### [107] [The Differential Neural Tangent Kernel and Its Positivity](https://arxiv.org/abs/2607.10200)
*Bangti Jin,Longjun Wu*

Main category: Training

TL;DR: 提出了微分神经正切核（DNTK）框架，证明了在无限宽度下浅层和深层神经网络对线性微分算子的DNTK正定性，为物理信息神经网络（PINNs）的训练动力学分析奠定基础。


<details>
  <summary>Details</summary>
Motivation: 现有NTK理论在分析PINNs时面临微分算子带来的正定性证明难题，而该性质对训练动力学分析至关重要。

Method: 构造DNTK框架，通过积分变换和激活函数条件（如RePU和非多项式光滑激活）严格证明正定性。

Result: 对浅层/深层网络及广泛激活函数类（含所有线性微分算子），DNTK正定性成立。

Conclusion: 所提框架及其正定性结论为梯度类算法在PINNs中的收敛性分析提供了理论依据。
...
### [108] [Sharper Analysis of Single-Loop Methods for Bilevel Optimization](https://arxiv.org/abs/2607.10263)
*Yubo Zhou,Jun Shu,Luo Luo,Junmin Liu,Deyu Meng,Guang Dai,Haishan Ye*

Main category: Training

TL;DR: 本文针对双层优化的单循环隐式与迭代微分方法，提出解耦范数分析框架，将AID方法收敛率从O(κ⁶/K)改进至O(κ⁵/K)，并证明ITD渐近误差与下界O(κ²)匹配。


<details>
  <summary>Details</summary>
Motivation: 缩小单循环隐式微分和迭代微分方法在理论保证与实际高效实现之间的差距。

Method: 提出解耦范数分析框架，分别对AID和ITD进行更精细的收敛性分析。

Result: AID收敛率提升至O(κ⁵/K)；ITD渐近误差改进至O(κ²)，匹配已知下界。

Conclusion: 所提理论分析有效且被数值实验验证，弥合了理论与实践之间的鸿沟。
...
### [109] [Interpreting learning dynamics of autoencoders: Transient scaling and emerging concepts of the Ising model](https://arxiv.org/abs/2607.10285)
*Max Weinmann,Miriam Klopotek*

Main category: Training

TL;DR: 无监督自编码器在Ising模型上学习宏观变量的过程表现出两种动力学状态，分别由磁化和能量主导，且学习率、模型深度和宽度控制着两种状态的转换。


<details>
  <summary>Details</summary>
Motivation: 探究无监督自编码器在无领域知识情况下，如何从微观自旋构型中学习出与理论相关的宏观变量，模仿科学发现过程。

Method: 通过多尺度粗粒化分析、递归动态轨迹分析，研究超参数（深度、宽度、学习率）对表示学习的影响，并利用误差流场分析表示空间的拓扑结构。

Result: 发现两种动力学状态：磁化主导的暂态（表现出动态标度和顺序到尺度的涨落）和能量主导的状态（逐步聚焦于能量表示的小尺度信息）。深度模型在中高速率下会停滞。

Conclusion: 学习过程可视为由训练数据和优化器驱动的非平衡过程，其内在动力学特性揭示了表示变化的强制效应，为物理世界与机器学习模型的解释提供基础。
...
### [110] [A Control Theory of Predictability in Latent World Models](https://arxiv.org/abs/2607.10362)
*Hanzhe You,Yonggang Zhang,Maohao Ran,Zhiqin Yang,Zhenyuan Zhang,Wei Xue,Jun Song,Xinmei Tian,Yike Guo*

Main category: Training

TL;DR: 潜在世界模型在规划中使用的预测误差与控制器性能不相关，而规划器可达状态上的预测-真实代价差异才是控制次优性的关键边界。


<details>
  <summary>Details</summary>
Motivation: 证明基于数据平均预测误差训练和选择潜在世界模型不可靠，因为规划器查询的是偏离数据流形的状态，误差平均无法反映控制性能。

Method: 将目标重定义为规划器承诺计划上的预测与真实代价差异，证明次优性受限于该差异的两倍；在线性控制假设下分解为流形上残差（由谱税度量）和流形外散度（主导项）两部分。

Result: 合成实验验证了定价公式；潜模型预测控制实验显示单步验证误差与控制成功率无相关性，而基于规划器可达状态的信度分数能有效追踪控制性能。

Conclusion: 潜世界模型应基于规划器可达状态的分布而非数据平均误差进行训练和评估，以提升控制性能。
...
### [111] [SETA: Scaling Environments for Terminal Agents](https://arxiv.org/abs/2607.10891)
...
TL;DR: 提出SETA框架，用于生成可验证的终端强化学习环境，构建包含4500+环境的SETA-Env数据集，在8B模型上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 终端智能体训练缺乏规模化的可验证环境与监督数据，现有方法难以同时保证任务指令多样性、环境可执行性与结果可靠性。

Method: 设计双流水线框架SETA，SETA-Synth将多样化数据源转换为标准RL环境，SETA-Evol通过自适应难度与多样性控制扩展环境，共享统一验证机制。

Result: 在Terminal-Bench 2.0上，Qwen3-8B的GRPO训练达到12%通过率（8B规模RL模型最优），DeepSeek-V4-Flash的pass@1从40%提升至43%，pass@5从54%提升至58%。

Conclusion: SETA-Env提供了高质量终端训练环境，能有效提升终端智能体能力，是推动终端智能体学习研究的重要资源。
...
### [112] [Incremental Transformer for Surrogate-Based Inverse Design of Geopolymer Mixtures](https://arxiv.org/abs/2607.10896)
*Giansalvo Cirrincione,Filippo Grassia*

Main category: Training

TL;DR: 提出基于增量Transformer的拓扑感知代理框架，用于小数据、异构、物理约束的逆向设计，在矿粉-粉煤灰地聚合物混凝土配比中实现强度与碳排放的双目标优化。


<details>
  <summary>Details</summary>
Motivation: 工程小数据逆向设计面临观测异构、变量混合及物理关系约束的挑战，现有方法难以同时保证候选点的物理有效性与数据流形支持。

Method: 融合本征维度分析、混合变量空间表示、表格代理预测、INCRT流形理性化及约束逆向优化，以INCRT作为原型提炼层提供流形支持评分。

Result: 高维设计空间存在强冗余；抗压强度需非线性代理，碳排放可用正则化线性模型恢复；拓扑感知策略在目标达标、碳减排、物理合规及流形邻近性上优于无约束与纯物理约束优化。

Conclusion: 框架并非替代实验验证，而是为小样本、混合、物理约束工程数据集提供可信候选配比的筛选支撑。
...
### [113] [QwenPaw-Data: Bridging Facts, Methodology, and Execution for Autonomous Enterprise Data Analytics](https://arxiv.org/abs/2607.11019)
...
TL;DR: QwenPaw-Data是一个面向企业智能数据分析的智能体数据系统，通过语义、方法、执行和演化的持续循环，提升数据分析的可靠性、可追溯性和持续改进能力。


<details>
  <summary>Details</summary>
Motivation: 企业数据分析环境开放、模糊且持续演变，通用智能体方法难以应对，需将语义、方法论、执行和演化作为系统一等公民。

Method: 设计三子系统架构：DataBridge提供语义基础，Skill-Hub将专家方法论编码为可复用技能，Host实现可控运行时执行，并通过资产飞轮实现持续进化。

Result: 在公开基准和工业BI负载上，提升了可验证数据访问能力与高级分析质量。

Conclusion: 为可靠、可追溯且持续改进的企业数据智能体提供了实用基础。
...
### [114] [AutoNorm: Understanding Adaptive Normalization in Transformers through Differentiable Gating](https://arxiv.org/abs/2607.10593)
*Piyush Kaushik Bhattacharyya,Divyanshu Rai,Swastik Singh,Kumar Aakash,Ayush Ranjan,Krutika Verma*

Main category: Training

TL;DR: 提出AutoNorm-S方法，通过门控冻结调度缓解归一化选择的优化不稳定性，在NLP任务上超越自适应归一化基线，在视觉任务上保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 发现Gumbel-Softmax门控在不同任务上表现不一致：视觉任务中高梯度方差导致门控性能不如随机选择，而语言任务中门控多样性有效。

Method: AutoNorm-S采用门控冻结调度策略，在训练过程中逐步冻结归一化选择门控，减少优化噪声的干扰。

Result: 在PTB和SST-2等NLP数据集上超越自适应归一化基线，在标准视觉基准上保持竞争力，总体性能与静态LN或全自适应方法持平或更优。

Conclusion: 将归一化选择与优化噪声解耦是一种实用且原则性的自适应归一化方法，可提升Transformer架构的训练稳定性与性能。
...
### [115] [M+Adam: Low-Precision Training via Additive-Multiplicative Optimization](https://arxiv.org/abs/2607.10611)
*Xiaoyuan Liang,Sebastian Loeschcke,Mads Toftrup,Anima Anandkumar*

Main category: Training

TL;DR: 提出M+Adam优化器，融合加性更新与乘性更新，解决低精度训练中优化器停滞问题，提升BF16/FP8/FP4精度下的训练效果。


<details>
  <summary>Details</summary>
Motivation: 低精度训练中标准优化器因尾数分辨率不足导致权重较大时加性更新失效，而纯乘性更新在接近零或符号变化时也失败，两者互补性未被利用。

Method: 结合加性更新（处理符号变化和小幅度权重）与乘性更新（在加性更新被舍入归零时确保大幅度权重进展），提出M+Adam算法并证明其在光滑假设下的单调下降性。

Result: 在60M-1B参数的LLaMA风格预训练中，使用BF16、FP8和FP4主权重，M+Adam一致优于标准低精度训练方法。

Conclusion: M+Adam通过互补更新策略有效克服低精度训练瓶颈，为极低精度量化训练提供实用方案。
...
### [116] [Learning to Fine-tune Foundation Models under Resource Limitations](https://arxiv.org/abs/2607.10694)
*Thomas Tsouparopoulos,Iordanis Koutsopoulos*

Main category: Training

TL;DR: 提出在资源受限设备上对预训练基础模型进行持续微调的最优策略，使用强化学习或动态规划方法在有限计算预算下决策微调时机，实验表明该方法在精度和效率上均优于基准。


<details>
  <summary>Details</summary>
Motivation: 资源受限设备上持续微调预训练模型时，如何动态决定是否微调以在有限计算预算下最大化模型性能，现有方法缺乏系统性的最优决策框架。

Method: 将问题建模为带约束马尔可夫决策过程（状态包含模型性能、计算预算、数据分布相关性），使用演员-评论家强化学习算法求解；对于可预测微调性能的特例，采用动态规划方法。

Result: 在文本分类数据集上，所提方法在相同计算预算下比常规微调准确率提升超过4%，使用仅25%的微调步数即可达到全参数微调97%的准确率。

Conclusion: 所提决策策略能有效平衡微调收益与计算成本，在资源受限场景下显著优于启发式方法，为预训练模型的持续部署提供了实用解决方案。
...
### [117] [Scaffold splits hide structural-frontier failures in ADMET models](https://arxiv.org/abs/2607.10729)
*Jiacheng Zheng,Chang Guo,Zixuan Wang,Xinyu Liu*

Main category: Training

TL;DR: 提出一种基于结构前沿的数据分割方法，用于评估分子性质模型在未知化学空间上的表现，发现该方法显著增加模型误差，且现有鲁棒惩罚无法解决此问题。


<details>
  <summary>Details</summary>
Motivation: 传统的Bemis-Murcko支架分割仅代表一种化学陌生性概念，需要更严格的评估方法来测试模型对稀疏且物理化学性质边远的分子组的泛化能力。

Method: 引入标签无关的结构前沿分割，保留最稀疏和物理化学上最遥远的支架组；并提出Multi-View Frontier Risk Extrapolation (MVFRE)方法，基于多视角分子视图的计数调整尾部风险惩罚。

Result: 与70/10/20支架分割相比，前沿分割使主误差中位数增加87.0%，均值增加130.3%；去除BBB终点后均值降至75.9%；消息传递图网络仍显示均值82.8%的误差增加，且MVFRE仅改变误差0.16%（感知机）和-1.9%（图网络）。

Conclusion: 分割构建和标签来源是重要的评估约束，现有训练惩罚无法解决前沿分割导致的模型失效问题。
...
### [118] [SCALECUA: Scaling Computer Use Agents with Verifiable Task Synthesis and Efficient Online RL](https://arxiv.org/abs/2607.11185)
*Bowen Lv,Xiao Liu,Yanyu Ren,Hanyu Lai,Bohao Jing,Hanchen Zhang,Yanxiao Zhao,Shuntian Yao,Jie Tang,Yuxiao Dong*

Main category: Training

TL;DR: 提出ScaleCUA框架，通过可验证任务合成和高效训练，突破数据稀缺和在线RL效率瓶颈，在OSWorld和ScienceBoard达到开源SOTA。


<details>
  <summary>Details</summary>
Motivation: 在线强化学习结合可验证奖励是扩展CUA能力的关键方向，但受限于可验证数据稀缺和在线RL效率低下。

Method: 设计VeriGen生成可验证RL任务（多智能体反馈+ Docker交互），提出Frontier Sampling按能力分配采样，以及Visual Context Segmentation滑动窗口加速训练。

Result: OSWorld 68.7%和ScienceBoard 54.0%准确率，训练加速2.83倍，生成24K+可验证任务及近3K高质量RL任务。

Conclusion: ScaleCUA通过统一数据合成与高效训练框架，有效解决了CUA在线RL的瓶颈，确立了开源智能体的新SOTA。
...
### [120] [StructAgent: Harness Long-horizon Digital Agents with Unified Causal Structure](https://arxiv.org/abs/2607.11388)
...
### [122] [The Singularity Space: A Generative Diffusion Framework for Signal Representation](https://arxiv.org/abs/2607.10930)
...
### [123] [Sticky Jump Diffusions: A Unifying View of Masked, Continuous, and Hybrid Diffusion](https://arxiv.org/abs/2607.10951)
...
### [124] [WSqD: A Horizon-Free Learning Rate Schedule for Large Model Training](https://arxiv.org/abs/2607.10959)
...
### [125] [MAGIC: Transition-Aware Generation of Navigable Multi-Scene Game Worlds with Large Language Models](https://arxiv.org/abs/2607.11594)
...
### [126] [Efficient Online Proportional Sampling with Applications to Smoothed Online Learning](https://arxiv.org/abs/2607.10963)
...
### [127] [Multi-dimensional training-priority weighting based on physical information propagation paths: a unified residual-weighting framework for physics-informed neural networks](https://arxiv.org/abs/2607.11094)
...
### [128] [SPARC-Net: A Spectral, Causality-Aware, and Hard-Constrained Physics-Informed Architecture for Stiff and Shock-Dominated Partial Differential Equations](https://arxiv.org/abs/2607.11310)
...
### [129] [PRISM Edit: One Vector for All Temporal Answers](https://arxiv.org/abs/2607.11327)
...
### [130] [Proxy Exploration and Reusable Guidance: A Modular LLM Post-Training Paradigm via Proxy-Guided Update Signals](https://arxiv.org/abs/2607.11505)
...
### [131] [Random Label Prediction Heads for Studying Memorization in Deep Neural Networks](https://arxiv.org/abs/2607.11541)
*Marlon Becker,Jonas Konrad,Luis Garcia Rodriguez,Benjamin Risse*

Main category: Training

TL;DR: 通过附加随机标签预测头（RLP-head）来实证研究深度神经网络中的记忆化现象，并将其作为Rademacher复杂度的经验估计，分析记忆能力在层间的演化，进而提出基于RLP-head的正则化方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对深层网络各层记忆化程度的直接测量方法，且对过拟合与记忆化关系的理解存在矛盾。

Method: 在网络的任意深度附加随机标签预测头，利用其预测随机标签的性能作为记忆化与模型容量的经验度量，并基于此提出正则化技术。

Result: 减少记忆化可能根据数据集和训练设置不同而改善或损害泛化能力。

Conclusion: 过拟合不等同于记忆化，两者关系复杂，需重新审视传统假设。
...
### [132] [Heuristic Learning for Active Flow Control Using Coding Agents](https://arxiv.org/abs/2607.11565)
*Paul Garnier,Jonathan Viquerat,Elie Hachem*

Main category: Training

TL;DR: 提出一种基于编码智能体的启发式学习框架，在主动流控任务中直接搜索显式反馈律，在13个基准中10个达到或超越深度强化学习性能，且控制器更紧凑、可解释。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习策略依赖大量交互、难以解释，现有方法在主动流控中面临非线性、部分观测和高计算成本挑战。

Method: 采用编码智能体通过公开基准接口迭代提出、评估和修正控制器实现，配合约束启发式学习协议，直接搜索显式反馈律而非优化神经网络参数。

Result: 在13个主动流控基准中10个匹配或超越最强深度强化学习基线，控制器保持紧凑、可解释，并在变化雷诺数、瑞利数、执行器数和观测稀疏性下表现稳健。

Conclusion: 编码智能体的启发式学习是传统强化学习的可信互补方案，兼具竞争性能和物理可解释的控制器表示。
...
### [133] [DiffEEG: A Self-Supervised Denoising Diffusion Model for Learning EEG Generic Representations](https://arxiv.org/abs/2607.11578)
*Abdulkader Helwan,Lina Abou-Abbas,Hussein El Amouri,Belkacem Chikhaoui,Khadidja Henni*

Main category: Training

TL;DR: 提出DiffEEG，一种自监督基础模型，结合去噪扩散预训练和强化学习微调，解决EEG癫痫检测中的注释稀缺和类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: EEG癫痫检测面临严重注释稀缺和极端类别不平衡（发作事件占比<10%），现有方法难以在少量标注下实现临床可用的检测性能。

Method: 采用1D U-Net和多头自注意力机制的扩散预训练（1.3M无标注片段），再用强化学习（策略梯度优化）直接最大化F1分数进行微调。

Result: 在279例患者的留一折交叉验证下，4类癫痫亚型分类准确率61%、F1 59%；二分类检测准确率81%、加权F1 85%，发作召回率59%（发病率6.7%）；段级评估上限准确率97.6%。

Conclusion: 扩散预训练结合指标感知的强化学习在最小标注数据下实现了临床可部署的癫痫监测性能。
...
### [134] [How to Tame Grokking: Representation Geometry as a Control Signal](https://arxiv.org/abs/2607.11666)
*Maksim A Kazanskii*

Main category: Training

TL;DR: 研究发现维度塌陷先于“顿悟”出现，提出几何维度正则化（GeomDR）通过调节表示几何维度，最多可将泛化加速52倍。


<details>
  <summary>Details</summary>
Motivation: 揭示表示几何与延迟泛化（顿悟）之间的关系，并探索通过几何干预控制该现象的可能性。

Method: 设计几何维度正则化（GeomDR），在训练中修改隐藏表示的谱有效维度；在模加法、模除法和置换组合任务上测试MLP和Transformer。

Result: GeomDR一致地改变顿悟动态，在若干设置中将泛化加速最高52倍，且效果跨架构一致。

Conclusion: 表示几何可作为顿悟的有效控制信号，几何干预为研究和影响延迟泛化提供了实用方法。
...
### [135] [Time-Lag-Aware Deep Reinforcement Learning for Flexible Job-Shop Scheduling in PPVC Module Factories](https://arxiv.org/abs/2607.11725)
*Ziheng Zhang,Wei Zhang*

Main category: Training

TL;DR: 针对预制装配式建筑模块生产中的长时间滞后问题，提出了一种基于双注意力深度强化学习的调度方法，包含滞后感知、预测特征和活性掩码三项改进，在基准测试中性能接近约束规划求解器，优于所有调度规则和遗传算法。


<details>
  <summary>Details</summary>
Motivation: 预制装配式建筑中混凝土养护、水密性测试和油漆干燥等工序导致模块长时间阻塞工作站（滞后），传统调度方法忽视此问题会造成性能严重下降，而现有求解器难以快速响应扰动。

Method: 基于双注意力深度强化学习求解器，通过三项可逐个消融的扩展：滞后感知动力学（含可采纳奖励上界）、两个预测性滞后特征通道、以及活性掩码的操作类型和工位类型嵌入，所有扩展禁用时可复现原始求解器。

Result: 在基于国家预制指南的基准实例上，学习策略在保留实例上达到约束规划参考解约4%以内，优于所有调度规则和遗传算法元启发式；在容量竞争加剧时优势扩大，单一混合工厂规模策略覆盖训练范围，重规划仅需数秒。

Conclusion: 深度强化学习调度器可作为免求解器、免模型的强替代方案，在扰动下快速重规划；精确求解器仍是质量上限，而本研究明确标定了这一边界。
...
### [136] [Requential Coding: Pushing the Limits of Model Compression with Self-Generated Training Data](https://arxiv.org/abs/2607.11883)
*Shikai Qiu,Marc Finzi,Yujia Zheng,Kun Zhang,Andrew Gordon Wilson*

Main category: Training

TL;DR: 本文提出一种名为 requential coding 的新型压缩方法，通过教师模型选择学生分布中的样本，仅编码教师与学生不一致之处，从而实现与参数数量和熵无关的极短编码长度，并在大型语言模型上获得先进的泛化保证。


<details>
  <summary>Details</summary>
Motivation: 现有参数化方法（如量化）的编码长度随模型规模增长，而预序编码方法因编码完整数据序列而在高熵数据上产生大码长，两者均无法有效反映模型的真正学习复杂度。

Method: Requential coding 利用教师模型从学生模型自身分布中选取训练样本，学生编码仅记录这些选择（即教师与学生分歧点），码长独立于参数数量和数据熵。

Result: 在固定损失下，更大模型和集成模型获得更短码长；应用于PAC-Bayes界时，对十亿参数大语言模型取得最先进泛化保证，且码长优势随规模扩大而增长。
...
### [137] [From ML Predictions to Informed Diagnostic Assistance Using the Toulmin Model of Argumentation](https://arxiv.org/abs/2607.09664)
*Anca Marginean,Adrian Groza*

Main category: Model

TL;DR: 提出一个基于Toulmin论据模型的图像诊断可解释框架，将ML诊断分解为论据组件，整合生物标志物提取、MedGemma代理和MedSigLip相似性计算，提供结构化评估以增强人类专家判断。


<details>
  <summary>Details</summary>
Motivation: 现有ML诊断缺乏结构化可解释性，XAI方法不足以提供清晰推理路径，需要一种基于论据模型的分解方法来辅助人类专家进行批判性评估。

Method: 采用Toulmin模型分解诊断：ML模型提取的视网膜生物标志物作为依据，MedGemma代理结合医学知识分析保证（依据到结论的链接），基于模型定量评估确定限定词，利用MedSigLip图像相似性构建反驳，所有组件呈现给人类专家。

Result: 框架生成结构化论据组件（依据、保证、限定词、反驳），支持人类专家对ML诊断进行更知情和批判性的评估。

Conclusion: 基于Toulmin模型的论据分解方法为医学影像诊断提供了可解释且结构化的评估途径，有望提升ML辅助诊断的可靠性与透明度。
...
### [138] [Boltzmann MapReduce: A Partition-Function Reduce for Forkable Sandboxes](https://arxiv.org/abs/2607.09689)
*Yossi Eliaz*

Main category: Model

TL;DR: 在局部渐近正态条件下，置信密度呈现为逆温度等于样本量的吉布斯-玻尔兹曼分布，由此推导出MapReduce归约操作对应配分函数、众数为精度加权池化、频率学派一致性对应零温极限等结论。


<details>
  <summary>Details</summary>
Motivation: 揭示统计推断与统计力学之间的深层联系，为分布式计算中的置信传播提供理论依据。

Method: 利用局部渐近正态（LAN）框架，将置信密度建模为吉布斯-玻尔兹曼分布，通过配分函数和温度参数分析统计性质。

Result: 在高斯/线性情况下精确成立，一般情形下为一阶近似：不同数据块的玻尔兹曼因子独立；MapReduce归约对应配分函数；众数实现精度加权池化；频率学派一致性对应零温极限T=1/n→0。

Conclusion: 统计推断中的置信传播可自然映射为统计力学中的玻尔兹曼机，样本量扮演逆温度角色，为分布式统计计算提供物理视角。
...
### [139] [SciML in the Wild: A Diagnostic Study of When Structural Priors Help and When They Hurt](https://arxiv.org/abs/2607.09684)
*Vrishank Sai Anand,Prathamesh Dinesh Joshi,Raj Abhijit Dandekar,Rajat Dandekar,Sreedath Panat*

Main category: Model

TL;DR: 在宏观经济预测中，结构化先验若与数据生成过程不匹配会起到错误正则化作用，导致约束较少的模型（如ARIMA、NODE）优于强约束的启发式先验模型（如PINN、UDE）。


<details>
  <summary>Details</summary>
Motivation: 探究当SciML方法的结构化先验假设被违反时（即先验与真实动态不匹配）模型的表现，并以宏观经济预测作为压力测试领域。

Method: 使用23个国家的稀疏年度数据、多个时间划分和5个随机种子，评估ARIMA、LSTM、NODE、PINN和UDE五类模型。

Result: 所有模型均未实现稳定强预测性能，但相对层级清晰：约束少的模型（ARIMA、NODE）持续优于强约束模型（PINN、UDE）。

Conclusion: 结构化先验可能成为错误正则化器，SciML实践者应先测试结构是否有助于预测，而非默认更多结构更优。
...
### [140] [YUKTI: From Natural-Language Situations to Robust, Verifiable Decisions An Uncertainty-Typed Proposition IR, Assumption-Robust Pareto Frontiers, and a Regret Certificate](https://arxiv.org/abs/2607.09706)
*Suyash Mishra*

Main category: Model

TL;DR: YUKTI通过带类型命题图、分布帕累托传递和假设鲁棒帕累托前沿，将语言模型从单目标点值求解转向鲁棒多目标决策，在受控误设定、商业决策和真实数据集上显著降低遗憾。


<details>
  <summary>Details</summary>
Motivation: 现有NL4Opt、OptiMUS等管道固定单一目标与点值系数，对分配预算等决策脆弱——最优性依赖于精确猜测，是计算模仿。

Method: YUKTI以带类型命题图表示关系和不确定系数，联合精确/非线性/演化求解器，用分布帕累托传递耦合阶段；引入ARPF通过ε-污染重采样评估动作生存率ρ，并证明ρ是决策遗憾的精确因子。

Result: 受控误设定下均值与尾遗憾降低超90%；商业决策中优化合法行动空间并计价风险；41,188条决策回测比历史策略优34%、比点值规则优4%。LLM和单目标优化遗憾为YUKTI的47倍。
...
### [141] [Prioritizing Search Space Regions in the Low Autocorrelation Binary Sequences Problem](https://arxiv.org/abs/2607.09688)
*Blaž Pšeničnik,Borko Bošković,Jan Popić,Janez Brest*

Main category: Model

TL;DR: 提出一种结合汤普森采样和并行自回避行走的混合搜索框架，通过多臂老虎机动态分配计算资源，GPU加速和优化策略，在450≤L≤527和L=573上改进了35个序列长度的最佳已知结果，并首次报告L=451时品质因子超过8.0的序列。


<details>
  <summary>Details</summary>
Motivation: 低自相关二元序列问题（LABS）是重要但困难的组合优化问题，现有方法在处理长序列时性能有限，需要更有效的搜索策略以提升品质因子。

Method: 混合搜索框架：将搜索空间划分成限制类并建模为多臂老虎机，使用汤普森采样自适应分配资源；结合并行自回避行走、GPU并行、共享后验更新、高效邻域评估和布隆过滤器防循环；采用两阶段优化（先搜索约束偏斜对称空间再优化无限制空间）。

Result: 在450≤L≤527范围及L=573上改进了35个序列长度的最佳已知结果，L=451时首次实现品质因子超过8.0。

Conclusion: 该方法可扩展、有效，验证了在线数据驱动资源分配在LABS优化中的价值，适用于高性能品质因子最大化。
...
### [142] [GES-TSP: Graph Edge Sparsification for TSP](https://arxiv.org/abs/2607.09708)
*Tianfeng Chen,Xianyue Li*

Main category: Model

TL;DR: 提出一种基于学习的图边稀疏化方法（GES），用于欧几里得旅行商问题，通过结合几何结构与组合优化自适应生成稀疏图，在保持最优解差距小于1%的同时剪枝高达95%的边。


<details>
  <summary>Details</summary>
Motivation: 传统稀疏化方法依赖固定启发式，无法充分利用实例特定的结构信息，导致计算效率受限。

Method: 设计学习型稀疏化框架，融合几何结构信息与组合优化技术，为不同实例自适应生成稀疏图。

Result: 在MATILDA数据集上剪枝95%边且最优解差距<1%；在TSPLIB大规模实例上剪枝率超99%且差距仍<1%。

Conclusion: GES方法有效提升大规模TSP求解效率，且具备强泛化能力。
...
### [143] [Low-Rank Attention Residuals](https://arxiv.org/abs/2607.09694)
*Jonathan Su*

Main category: Model

TL;DR: 提出低秩注意力残差（LR-AttnRes），通过将路由维度从全模型宽度d降至r（r≪d），解耦路由与残差表示，在保持性能的同时减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 标准注意力残差中，每个子层输出既充当全维键又充当值，导致路由与表示耦合，且深度路由分数随隐藏宽度d线性增长，带来计算和表达冗余。

Method: LR-AttnRes保留全维残差值，但使用r维低秩键进行路由。投影版从现有输出投影中学习低秩键，切片版直接取每个值的最后r维作为路由键，移除辅助键投影路径。

Result: 在变体测试中，投影版LR-AttnRes取得最优验证损失；切片版在减少残差侧FLOPs的同时仍能提升性能。深度路由在远小于模型宽度的维度下依然有效。

Conclusion: 低秩路由可有效解耦残差路由与内容表示，为注意力残差机制提供更高效、可扩展的改进方向。
...
### [144] [Coresets Before Score Sets: Evaluation-Unsupervised Prompt Subset Selection for LLM Benchmarks](https://arxiv.org/abs/2607.09739)
...
TL;DR: 研究提出基于子模函数（尤其是设施选址函数）的无监督基准核心集选择方法，仅利用语义嵌入即可在35个基准、18个模型、6.1万提示上显著优于12种基线，并扩展到有监督场景中达到SOTA表现。


<details>
  <summary>Details</summary>
Motivation: 大规模LLM评估计算成本高昂，需从多基准中选取少量提示子集以高效近似完整评估结果，且现有方法依赖模型分数或粗粒度基准选择。

Method: 采用子模子集选择，设计并比较DPP、子模互信息、设施选址等函数，在无监督设置下仅基于廉价语义嵌入进行选择。

Result: 设施选址函数在所有核心集预算下优于12种基线，且在有监督基准选择任务中匹配或超越SOTA，计算成本更低。

Conclusion: 子模性是基准压缩的可靠工具，设施选址函数能在无监督下高效保持模型评分排序精度。
...
### [145] [How Much Does Correctness Cost? Budgeted Placement of Strong Correctors in a Weak Multi-Agent Swarm](https://arxiv.org/abs/2607.09765)
*Igor Itkin*

Main category: Model

TL;DR: 研究了在不可靠智能体群体中，通过少量昂贵“神谕”修正达成正确共识所需的最小预算与最优神谕放置策略，证明子模性并提出贪心算法，在完整图上给出闭式解，发现成本-质量曲线曲率（凹/凸）决定了“少而强”或“多而中”的策略选择，且任务相关。


<details>
  <summary>Details</summary>
Motivation: 群体由廉价不可靠智能体组成时，需要借助少数强大“神谕”来导向正确共识，但如何最小化成本并最优放置神谕是核心问题。

Method: 将群体共识建模为图上的共识过程，每个神谕以成本耦合的凹函数强度固定一个节点朝向真相，用相干性H(R)=tr M(R)^{-1}衡量质量。证明H保持子模性，从而成本效益贪心算法在任意预算下达到1-1/e近似最优。通过预算反转得到预算-正确性前沿B*(eps)，在完整图上给出闭式解，并分析同成本时最小神谕数量k*。

Result: 贪心算法达到1-1/e近似；完整图上有闭式预算-正确性前沿；成本-质量曲线的凹凸性决定策略：凹性支持“多而中”，凸性支持“少而强”；在Qwen3模型（0.6-32B）上，数学验证任务为凹，代码追踪任务为凸。

Conclusion: 最优神谕放置策略依赖于成本-质量曲线的曲率，且任务间存在本质差异，验证了“少而强”与“多而中”的权衡是任务相关的。
...
### [146] [From Patterns to Maze Structures: SMT-Based Path Synthesis and 2D/3D Construction](https://arxiv.org/abs/2607.09781)
*Shengyi Wang*

Main category: Model

TL;DR: 提出了一种从输入模式（如文本或形状）构建迷宫结构的流水线，将中心路径综合问题编码为可满足性模理论（SMT）中的全局约束，支持平面和三维迷宫的实现。


<details>
  <summary>Details</summary>
Motivation: 现有迷宫生成方法难以灵活适配用户定义的输入模式（如任意形状或文本），且缺乏对平面和三维编织迷宫的统一生成框架。

Method: 将路径综合问题转化为SMT中的全局约束（邻接性、连续性和模式约束覆盖），通过一次求解获得固定实例的解；路径可为平面自避路线或具有上下交叉的分层遍历，并以此为基础构建迷宫。

Result: 成功生成了符合输入模式的平面迷宫和三维编织迷宫，并通过更多SMT-LIB示例扩展了原会议论文。

Conclusion: 基于SMT的路径综合方法能有效从任意输入模式生成迷宫，并为平面和三维迷宫构建提供通用框架。
...
### [147] [RUBRIC: Realism--Utility Balanced Ranking for Imbalanced Classification](https://arxiv.org/abs/2607.09816)
*Yanxuan Yu,Dong liu,Renata Borovica-Gajic,Ying Nian Wu*

Main category: Model

TL;DR: 提出RUBRIC框架，通过质量优先的合成样本筛选策略，在保持ROC-AUC的同时提升F1-macro和召回率，并理论证明其能收紧泛化界。


<details>
  <summary>Details</summary>
Motivation: 现有过采样方法生成大量低质量合成样本，导致决策边界扭曲、过拟合和泛化能力下降。

Method: 设计生成器无关的过滤框架，基于真实性与效用性的权衡对候选样本排序，并用凹边际评分函数量化效用。

Result: 在信用卡欺诈检测等任务上，RUBRIC提升了F1-macro和召回率，且ROC-AUC与原始生成器相当。

Conclusion: 质量优先的合成样本选择策略能有效缓解类别不平衡问题，同时提供lambda敏感性分析以平衡不同指标。
...
### [148] [Estimation, Prediction, and Assortment Optimization for Markov Chain Choice Models with Panel Data](https://arxiv.org/abs/2607.09817)
*Yalcin Akcay,Gerardo Berbeglia,Young-San Lin*

Main category: Model

TL;DR: 提出基于马尔可夫链和面板数据的选择模型框架，包括参数估计、个性化选择预测和品类优化，并设计了融合偏序偏好信息的EM算法。


<details>
  <summary>Details</summary>
Motivation: 传统选择模型假设交易独立同分布，但实际面板数据中同一顾客的交易存在依赖关系，现有方法未能有效利用偏序偏好信息。

Method: 采用马尔可夫链建模顾客选择过程，将偏序偏好信息融入EM算法进行参数估计；推导条件选择预测和品类优化的复杂度结果。

Result: 合成数据和寿司数据集上，所提EM算法在估计精度和预测性能上优于传统EM算法和基于多项Logit的偏序基准方法。

Conclusion: 框架有效利用了面板数据的依赖结构，为马尔可夫链选择模型提供了首个完整解决方案，并澄清了相关优化问题的计算复杂度。
...
### [149] [Nonlinear Axiomatic Attribution for Cooperative Games](https://arxiv.org/abs/2607.09869)
*Weida Li,Zhuanghua Liu,Yaoliang Yu,Bryan Kian Hsiang Low*

Main category: Model

TL;DR: 针对Shapley值在识别正向参与者时因线性性导致空域过大、可靠性不足的问题，提出了保留必要公理的非线性归因方法，基于最小核思想设计优化目标，实验证明其在包含AUC指标上优于仅放松效率公理的Shapley值变体。


<details>
  <summary>Details</summary>
Motivation: Shapley值作为线性算子存在过大零空间，导致非可忽略扰动无法区分，在包含AUC指标下不可靠。

Method: 受最小核启发，提出一类非线性公理归因方法，每个方法通过求解唯一最小化问题得到贡献向量，旨在尽可能忠实逼近效用函数。

Result: 在包含AUC指标实验中，所提方法相比仅放松效率公理的Shapley值变体展现出潜在有效性。

Conclusion: 非线性公理归因方法能克服Shapley值线性性的局限，更可靠地识别正向参与者。
...
### [150] [Who&When Pro: Can LLMs Really Attribute Failures in AI Agents?](https://arxiv.org/abs/2607.09996)
*Jiale Liu,Huajun Xi,Shaokun Zhang,Yifan Zeng,Tianwei Yue,Chi Wang,Jian Kang,Qingyun Wu,Huazheng Wang*

Main category: Model

TL;DR: 提出了Who&When Pro基准，用于评估自动故障归因系统在代理系统失败场景中的表现，包含12,326个带标签的失败轨迹。


<details>
  <summary>Details</summary>
Motivation: 随着代理系统能力增强，其故障更加隐蔽，自动归因变得至关重要，但缺乏大规模标准化基准。

Method: 通过严格控制的流水线（先精确回放成功前缀再注入故障）构建多模态故障轨迹，并标注黄金标签。

Result: 在26个基准上进行了广泛实验，揭示了模型在不同模态、协议和模型家族间的归因模式。

Conclusion: 为未来自动故障归因系统提供了实证指导，验证了该基准的有效性和必要性。
...
### [151] [Optimizing ARDL Models for Retail Sales Forecasting and Fair Pricing](https://arxiv.org/abs/2607.09956)
*Sujay Uday Rittikar*

Main category: Model

TL;DR: 提出一种通过消费者价格指数（CPI）约束来防止动态定价剥削消费者的公平定价框架，使用对数-对数自回归分布滞后模型和模拟退火算法，在加拿大食品零售数据上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有动态定价模型优化商业目标时忽视消费者公平，存在剥削风险；需在零售销售预测中嵌入公平约束。

Method: 构建对数-对数ARDL模型将价格系数作为销售弹性，通过线性规划和模拟退火求解以CPI为上限的销售最大化定价问题，并对比多种时间序列基线模型。

Result: 名义弹性为正导致无约束优化推高所有价格；CPI约束有效防止价格上限，模拟退火算法产生保守内部价格降低消费者成本；正向弹性主要是通胀假象。

Conclusion: 该框架透明且兼顾公平，为食品零售提供平衡利润与消费者福利的定价策略。
...
TL;DR: 提出PL-HCL框架，通过分层对比学习检测Agent技能中描述与行为的不一致，在含26.4万开源技能的语料上将Macro-F1从约0.45提升至0.87-0.89。


<details>
  <summary>Details</summary>
Motivation: LLM智能体广泛使用包含元数据、指令和资源的Agent技能，但开源技能市场中的第三方技能描述与真实行为常存在跨层不一致，难以检测。

Method: 提出渐进式加载感知的分层对比学习（PL-HCL）框架，利用LLM建模技能的分层结构并学习跨层一致性。

Result: 在超264,000个开源技能和人工验证集上，PL-HCL将Macro-F1从约0.45提升至0.87-0.89，跨多种LLM骨干稳定。

Conclusion: PL-HCL提供有效的筛选工具和设计原则，用于检测分层数字制品中的不一致性。
...
TL;DR: 提出一种可解释的知识图谱质量评估指标KGCQual，从实体和关系两个层面衡量自动提取图谱与理想图谱的语义忠实度，实验证明该指标能有效识别现有方法忽略的缺陷，并与下游任务性能显著相关。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱评估依赖任务特定指标或小规模人工验证，缺乏对图谱结构性和语义保真度的量化洞察，无法全面诊断自动抽取系统的缺陷。

Method: 构建包含实体层面（完整性、消歧质量、连通性）和关系层面（谓词保留、多元性、否定处理）的互补评估框架，结合词汇相似度、依存句法对齐和轻量级否定检测进行语义忠实度衡量。

Result: 在WebNLG、TinyButMighty等数据集上验证，KGCQual能可靠识别遗漏、冗余和结构偏差，消融实验证实名词与动词组件的有效性，下游任务显示与链接预测性能显著相关。

Conclusion: 提出一种可扩展、模型无关且可解释的标准化评估框架，为自动知识图谱构建方法的比较提供基础。经内容合规审查，未包含敏感内容。
...
### [154] [Comparing Socio-technical Design Principles with Guidelines for Human-centered AI](https://arxiv.org/abs/2607.10331)
*Thomas Herrmann*

Main category: Model

TL;DR: 本文比较了以人为中心的AI指南与社会技术系统原则，发现持续演化是社会技术系统的基本特征，人类监督和干预导致AI系统的持续适应与再设计，透明度需通过技术特征和整个系统（包括人类参与者）共同实现。


<details>
  <summary>Details</summary>
Motivation: 为了促进以人为中心的AI系统设计，需要将社会技术系统原则（来自传统IT领域）与HCAI指南进行对比和整合，以应对AI特有的挑战。

Method: 通过对比分析HCAI指南与社会技术系统原则，修订社会技术启发式方法以纳入AI使用方面，尤其关注持续演化、人类干预、自主协作和透明度。

Result: 揭示了持续演化是社会技术系统的基本特征；人类监督和自主协作导致AI系统的持续适应与再设计；透明度不仅需要技术特征，还需整个系统（包括人类参与者）的贡献；组织和社会实践需设计以补偿AI的不足。

Conclusion: 从社会技术角度出发，AI系统的成功应用需要将组织和社会实践与技术特征共同设计，以补偿AI的缺陷，并确保透明度和人类参与是系统整体而非仅技术层面的责任。
...
### [155] [MRUF: Multi-granularity Routing with Uncertainty-Aware Fusion for Robust Multimodal Sentiment Analysis](https://arxiv.org/abs/2607.10599)
*Haoran Ma,Yinfeng Yu,Liejun Wang*

Main category: Model

TL;DR: 提出MRUF，一种可靠性感知的多模态融合方法，通过多粒度路由和不确定性校准处理模态质量变化，在CMU-MOSI和CMU-MOSEI上取得一致改进。


<details>
  <summary>Details</summary>
Motivation: 传统融合方法在遇到遮挡、噪声、运动模糊等导致模态质量不均时，会过度信任不可靠的模态，影响情感分析性能。

Method: 使用多粒度路由（子空间级和模态级）结合不确定性感知校准，通过留一法误差增量监督模态路由，并利用逆方差加权精化模态门，同时通过模态不变对比对齐稳定共享表示空间。

Result: 在CMU-MOSI和CMU-MOSEI的对齐和未对齐设置下，性能一致优于强基线；机制分析验证高不确定性模态获得更低融合权重。

Conclusion: MRUF有效提升了多模态情感分析在模态质量变化场景下的鲁棒性和准确性。
...
### [156] [A Hyperbolic Neural Closure for M1 Radiation Transfer](https://arxiv.org/abs/2607.10364)
*Bongseok Kim,Jiahao Zhang,Johannes Krotz,Dinshaw Balsara,Ryan McClarren,Guang Lin*

Main category: Model

TL;DR: 提出了一种双神经网络参数化Jacobian矩阵的 hyperbolic 神经闭合法，确保M1辐射传输系统特征值实性，提升精度与稳定性。


<details>
  <summary>Details</summary>
Motivation: 机器学习闭包可能产生非实特征速度导致数值求解器崩溃，亟需一种保证实特征值的闭包模型。

Method: 通过对称矩阵网络和严格凸熵网络分别参数化Jacobian，使其相似于对称矩阵，再沿路径积分重建闭包。

Result: 在间断伽辽金模拟中，闭包精度优于经典解析闭包，且解精度更高、数值稳定。

Conclusion: 所提闭包在保证实特征值的同时，显著提升了M1辐射传输模拟的准确性和鲁棒性。
...
### [157] [Opti-Agent-Bench: Benchmarking End-to-End Optimization R&D Agents on Real-World Business Problems](https://arxiv.org/abs/2607.10768)
*Yongchang Fu,Xinjie Huang,Chengjun Dai,Chengzhe Feng,Junshao Zhang,Hong Zhu*

Main category: Model

TL;DR: 本文提出Opti-Agent-Bench，一个端到端基准测试，用于评估大语言模型在完整优化研发流程中的表现，涵盖从业务需求理解到解决方案报告生成的各个环节。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试仅在预结构化数学公式上评估LLM，忽略了将复杂业务需求转化为正确模型并高效求解这一关键挑战。

Method: 设计基于三大支柱：业务语义真实性（含反模板陷阱）、模块化评估（含跨模块一致性检查）和ORAC双层有效性框架（确保任务质量与评分完整性）。

Result: 在整数规划、鲁棒优化、随机规划和非凸优化等工业级任务上，揭示了当前模型的严重失败模式，如约束遗漏、模型-代码不一致和报告-实现偏差。
...
### [158] [Pitfalls of Administrative Censoring in Survival Models with Time-Indexed Inputs](https://arxiv.org/abs/2607.10466)
*Yanqi Xu,Hui Dai,Carlos Fernandez-Granda,Krzysztof J. Geras,Yiqiu Shen*

Main category: Model

TL;DR: 行政截尾泄漏是生存模型中的一种失效模式，当模型能推断输入数据的记录日期时，它会利用有限的后续随访信息而非患者真实风险进行预测，导致性能指标虚高。


<details>
  <summary>Details</summary>
Motivation: 大规模回顾性数据集中输入数据随时间变化，且结局观察有固定截止日期，导致最近记录的随访时间更短，模型可能从输入中学习到记录日期而非真实风险。

Method: 通过理论分析区分行政截尾泄漏与经典信息删失和真实时间变化风险，提出检测方法，并在模拟和真实乳腺X光摄影队列中验证。

Result: 模拟表明行政截尾泄漏可膨胀固定时间AUC和Harrell's C-index；真实队列中观察到类似偏差。

Conclusion: 对于n年预测任务，数据集在最新输入日期后应提供至少n年潜在随访，否则模型可能因行政截尾泄漏产生偏倚。
...
### [159] [First-Order Modal Logic in HOL: Deep and Shallow Embeddings with Automated Faithfulness (Extended Preprint)](https://arxiv.org/abs/2607.10880)
*Christoph Benzmüller,Daniel Kirchner*

Main category: Model

TL;DR: 在Isabelle/HOL中，将命题模态逻辑的深浅嵌入方法扩展到一阶模态逻辑，提出三种嵌入方式，并证明最小浅嵌入与深嵌入之间的忠实性，通过机械化Löwenheim-Skolem定理解决可数变量赋值对不可数个体域的满射问题。


<details>
  <summary>Details</summary>
Motivation: 先前工作仅限于命题片段，需要将深浅嵌入方法推广到一阶模态逻辑，并处理一阶量化带来的替换机制和域大小导致的满射问题。

Method: 并行实现三种嵌入：深嵌入、重载最大浅嵌入和轻量最小浅嵌入；将最小浅嵌入形式化为Isabelle/HOL locale，参数化访问关系、世界索引解释、世界宇宙和变量赋值；通过机械化可数向下Löwenheim-Skolem定理，在locale扩展中解决变量赋值对不可数个体域的满射问题；构建了一阶量词所需的替换机制。

Result: 成功证明最小浅嵌入与深嵌入之间的全局忠实性定理，量化所有最小浅解释可恢复深有效性；机械化了一阶模态逻辑的Löwenheim-Skolem定理，并解决了可数域变量赋值对不可数个体域的满射问题。

Conclusion: 提出的框架有效扩展了深浅嵌入方法到一阶模态逻辑，并通过机械化定理确保忠实性，为模态逻辑的机械化推理提供了坚实基础。
...
### [160] [LLM-PDESR: Robust PDE Discovery via Subdomain Weighted Residuals and LLM-Guided Symbolic Hypothesis Generation](https://arxiv.org/abs/2607.10546)
*Jinyang Du,Hao Ma,Xiaohu Shi,Bo Yang,Yanchun Liang,Heow Pueh Lee,Chunguo Wu*

Main category: Model

TL;DR: 提出LLM-PDESR框架，将大语言模型的结构假设生成与基于C^4连续五次样条和子域加权残差的鲁棒评估环境相结合，从噪声观测数据中自动发现偏微分方程，在23个规范PDE和5个新方程上显著优于现有方法，并成功从ERA5再分析数据中提取了可解释的1D动态代理方程。


<details>
  <summary>Details</summary>
Motivation: 传统符号回归方法在巨大组合搜索空间中难以识别准确方程，主要因为无法融入领域先验知识，且依赖点评估和离散差分会放大噪声，产生欺骗性适应度景观，阻碍优化过程。

Method: 1) 使用LLM生成候选方程的结构假设；2) 采用C^4连续五次样条进行稳健微分；3) 通过子域加权残差作为自然低通滤波器进行鲁棒评估；4) 基于Pareto的迭代反馈循环让LLM逐步优化方程，平衡预测准确性与结构简洁性。

Result: 在23个规范PDE和5个结构新颖方程（包括多变量系统）上，LLM-PDESR在结构恢复、噪声鲁棒性和避免虚假复杂性方面显著优于现有方法；从噪声ERA5再分析数据中成功提取了可解释的1D-CACE结构骨架。

Conclusion: LLM-PDESR通过融合LLM的结构生成能力与数学鲁棒评估框架，有效克服了传统方法的瓶颈，提升了符号回归在噪声数据下的发现能力和实际应用潜力。
...
### [161] [From Checker to Forecaster: Code-Owned Evaluation of Model-Generated Strategic Routes Under Delayed Ground Truth](https://arxiv.org/abs/2607.10972)
*Aleh Manchuliantsau*

Main category: Model

TL;DR: 提出RouteCast框架，用于模型输出评估中延迟或私有的真实标签场景，通过代码自有临时预测实现评分。在21个二元结果案例的回顾性试点中，整体包RouteCast得分AUC为0.756，与盲LLM裁判AUC 0.678及暴露身份LLM裁判AUC 0.761对比，存在泄漏风险。消融实验未发现路由分解的显著优势。


<details>
  <summary>Details</summary>
Motivation: 现有模型评估依赖即时可检查的合同或操作循环内的反馈，但许多场景中真实标签延迟、删失或私有，因此需要一种在评分时无法确定性检查正确性、只能由代码发布临时预测的评估方法。

Method: RouteCast框架：模型提出候选路线和结构化因素，利用时间点证据、参考类和确定性转换生成临时预测排名，随后根据实际结果评估预测。

Result: 在21个二元结果案例的回顾性试点中，RouteCast整体包得分AUC 0.756（95% CI [0.471,0.980]），盲LLM裁判AUC 0.678（CI [0.419,0.897]），暴露身份LLM裁判AUC 0.761（CI [0.515,0.944]），后两者结果一致表明存在泄漏风险。分解消融实验显示，将相同输入转换为类型化阶段路线与整体包得分无显著差异（AUC差-0.144，CI [-0.471,0.176]），与确定性启发式也无显著差异（AUC差-0.089，CI [-0.412,0.278]）。

Conclusion: 试点建立了可审计的可行性结果并暴露了失败模式，但未建立前瞻性校准、因果决策改进、路线分解优势或跨领域有效性。
...
### [162] [Sharp Concentration Bounds for Bundle-Valued Statistics on Manifolds](https://arxiv.org/abs/2607.10592)
*Swagatam Das,Vaclav Snasel*

Main category: Model

TL;DR: 本文针对流形上传输经验均值，建立了非渐近浓度理论，推导出Hoeffding与Bernstein型不等式，并量化了曲率与和乐偏差导致的误差下界。


<details>
  <summary>Details</summary>
Motivation: 几何统计与流形学习中观测值（如切向量）自然位于基流形不同点的纤维丛中，取均值需传输至参考纤维，引入曲率与和乐效应，经典浓度理论无法直接适用。

Method: 通过Hilbert空间不等式推导有限样本、维数无关的Hoeffding与Bernstein界；针对非唯一最短路径情形，利用丛曲率与环路几何量化确定性偏差，给出球面切丛的闭合公式。

Result: 偏差-方差分解显示，随机波动以经典$n^{-1/2}$速率衰减，而曲率误差为常数不可消除；极小极大下界确认两者本质性。此外建立了重尾下最优的中位数均值估计量与参考纤维中心极限定理。

Conclusion: 所提理论得到了球面实验的验证，揭示了流形统计中曲率与和乐效应带来的根本性精度限制。
...
### [163] [modelDNA: Calibrated Lineage Verification and Merge Decomposition from Sampled Weight Fingerprints](https://arxiv.org/abs/2607.10617)
*Muhammad Awais Bin Adil,Saad Aamir*

Main category: Model

TL;DR: 模型DNA是一种通过指纹识别开放权重语言模型谱系的工具，仅需100-300MB的HTTP读取，即可实现零误报的父模型归因和合并分解。


<details>
  <summary>Details</summary>
Motivation: 现有谱系信息依赖自述且缺乏验证，超过60%的模型未记录父模型；现有的检测方法仅针对单一信号和实验，无法应对纠纷时的快速重复分析。

Method: 通过范围HTTP读取提取模型指纹（约100-300MB），对比参考数据库中的四种信号族，返回八种判决类别及校准概率；合并分解利用线性权重合并方法，通过约束最小二乘法从指纹恢复混合权重。

Result: 在15个真实模型基准上，AUROC达到1.0，报告阈值下零误报，13/13正确识别父模型；合并分解中slerp合并的层插值曲线相关系数为0.999，dare_ties混合权重误差小于0.011。
...
### [164] [From Self-Attention to Connection Laplacian: A Unified Operator View of Transformers](https://arxiv.org/abs/2607.10677)
*Binbin Lin,Wei Chen,Yalun Li,Wenxiao Wang,Jieping Ye,Xiaofei He*

Main category: Model

TL;DR: 将自注意力机制解释为连接游走过程，证明单头注意力对应常数传输的连接传播，多头注意力对应边依赖的连接游走，并揭示其与几何算子（随机游走连接拉普拉斯）的联系。


<details>
  <summary>Details</summary>
Motivation: 自注意力在序列模型中广泛应用，但其算子级几何特性尚未被充分理解，缺乏统一的几何形式化框架。

Method: 将词元序列视为位置图上的向量场，注意力建模为非负游走矩阵与学习线性传输（边上的线性映射）的结合，通过连接游走形式化重塑单头/多头注意力，并推导其与拉普拉斯算子的等价条件。

Result: 实验表明，不同规模（124M至8B）和结构（编码器/解码器）的Transformer在深层中有效注意力图收敛为稳定几何算子，学习传输自组织为近似缩放等距，且随规模增大趋势增强。

Conclusion: 提供了将自注意力与经典几何算子连接的形式化连接游走框架，并贡献了一套分析Transformer模型几何性质的算子级工具。
...
### [165] [NextFund: A Unified Performance Tracking Platform for Agentic Portfolio Management](https://arxiv.org/abs/2607.11141)
*Changlun Li,Peixian Ma,Qiqi Duan,Zhenyu Lin,Peineng Wu*

Main category: Model

TL;DR: NextFund是一个评估平台，通过实时市场条件、多智能体协作分析和完整决策路径记录，使金融智能体的行为可观察，支持更公平的基准测试和可操作的诊断。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法不适用于基于LLM的智能体在投资组合构建和市场分析中的场景，多依赖于静态考试或仅报告最终回报，忽略了中间证据、分析师判断和执行步骤。

Method: 平台集成时间一致的市场访问、协调的多智能体分析、从观察到交易的完整决策日志，并通过交互式交易竞技场支持模型对比、权益曲线检查和结果下钻。

Result: 在香港、美国和中国A股市场演示了可检查的决策历史如何实现更公平的基准测试和更可操作的诊断。

Conclusion: NextFund通过增强决策的可观察性，为金融智能体的评估和诊断提供了更有效的方法。
...
### [166] [On the modality gap and the contrastive loss in multi-modal representation learning](https://arxiv.org/abs/2607.10698)
*Fabian Mager,Hiba Nassar,Lars Kai Hansen*

Main category: Model

TL;DR: 多模态对比学习中的模态鸿沟源于InfoNCE在低温下的模式失效，提出xNCE通过加入模态内负样本对缩小鸿沟并提升零样本分类性能。


<details>
  <summary>Details</summary>
Motivation: 研究CLIP双编码器模型中图像与文本嵌入空间存在系统性错位（模态鸿沟）的成因，并证明该现象是InfoNCE损失在低温条件下的固有缺陷。

Method: 通过单模态实验证明低温InfoNCE主动生成模态鸿沟，并提出xNCE——在对比学习中引入跨模态和模态内负样本对，以修正InfoNCE的损失函数设计。

Result: xNCE在MS-COCO上保持检索性能的同时持续缩小模态鸿沟，并在所有零样本分类基准上优于InfoNCE基线，而高温InfoNCE和正则化InfoNCE均无法实现。

Conclusion: xNCE能有效降低模态鸿沟而不牺牲迁移学习所需的判别几何结构，为零样本分类提供了更优的跨模态表示空间。
...
### [167] [Multi-Scale Convolution with Optimal Transport Attention Effect on Multivariate Time Series](https://arxiv.org/abs/2607.10740)
*HaoChong Fu,Jian Xu*

Main category: Model

TL;DR: 提出MSC-OT架构，结合多尺度卷积和最优传输注意力，提升多变量时间序列预测精度。


<details>
  <summary>Details</summary>
Motivation: 多变量时间序列分析面临捕捉多粒度结构模式和抑制噪声的挑战。

Method: MSC-OT由多尺度卷积增强和Sinkhorn最优传输正则化组成，通过反转嵌入捕捉变量间关系，并采用自适应融合策略动态组合注意力分数。

Result: 在ETT、Electricity等五个数据集上，MSC-OT在短期和长期预测任务中均表现优异。

Conclusion: MSC-OT及其各组件有效提升了多变量时间序列预测的准确性，组件间具有协同效应。
...
### [168] [The VC dimension of partial concept classes via Radon's theorem](https://arxiv.org/abs/2607.10751)
*Grigory Ivanov,Attila Jung,Márton Naszódi*

Main category: Model

TL;DR: 该论文研究具有边际的局部概念类的VC维，在Lp空间中为膨胀球和半空间提供了与维度无关的上下界，并推广了欧氏空间的结果。


<details>
  <summary>Details</summary>
Motivation: 扩展现有关于欧氏空间中膨胀球局部概念类的VC维结果到更一般的Lp空间，特别是算法相关的l1空间，并建立统一的理论框架。

Method: 通过线性化距离映射到具有Rademacher类型的空间，利用平衡有符号和估计或无维数Radon定理，并应用泛函分析技术。

Result: 在Lp空间中得到了膨胀球局部概念类VC维的与维度无关的上界，并给出了匹配边际参数的下界；同时推广了稠密邻域引理。

Conclusion: 成功将欧氏空间中的VC维边际界限扩展到Lp空间，为高维几何学习理论提供了新的工具。
...
### [169] [Hierarchical Bayesian Quadrature](https://arxiv.org/abs/2607.10793)
...
### [170] [Weight-Adjusted Gradients Reveal Parameter Importance and Failure Modes in LLMs](https://arxiv.org/abs/2607.10803)
...
### [171] [Lower Bound on the Cumulative Constrained Violation for the OGD+Projection algorithm for Constrained Online Convex Optimization (COCO)](https://arxiv.org/abs/2607.10808)
...
### [172] [Enhancing Query Efficiency for d-DNNF Representations Through Preprocessing](https://arxiv.org/abs/2607.11492)
...
### [173] [Comparative Analysis of GAT and BERT for Human-Like Playtesting](https://arxiv.org/abs/2607.11501)
...
### [175] [Lesioned Multimodal Language Models Reproduce Aphasic Picture-Naming Patterns](https://arxiv.org/abs/2607.11621)
...
### [176] [Reproducing human biases in route choice using large language models: Toward scalable behavioral modeling](https://arxiv.org/abs/2607.11632)
...
### [177] [Playful AI in Professional Email: A Field Experiment on Tone and Recipient Engagement](https://arxiv.org/abs/2607.11749)
...
### [178] [Link Adaptation Using Joint-Thompson Sampling](https://arxiv.org/abs/2607.11075)
...
### [179] [CA-DGCL: Dynamic Graph Continual Learning via Condensation and Attachment](https://arxiv.org/abs/2607.11112)
*Tingxu Yan Ye Yuan*
...
### [180] [Learning Subgroup Relations Using Siamese Graph Neural Networks](https://arxiv.org/abs/2607.11140)
*Tal Weissblat*
...
### [181] [BackgroundMellow: A Multi-Modal Cohesive Framework for Narrative-Driven Rich Cinematic Soundscape Generation](https://arxiv.org/abs/2607.11364)
...
### [183] [AutoMatBench: An Automatic Optimization Toolkit for the Acceleration of Material Properties Prediction Benchmarking](https://arxiv.org/abs/2607.11526)
*Hongxiao Li,Wanling Gao*

Main category: Model

TL;DR: 提出AutoMatBench，通过贝叶斯优化自动评估材料属性预测模型在分布外数据上的性能，节省超过一半成本。


<details>
  <summary>Details</summary>
Motivation: MatBench仅能评估模型在分布内数据上的性能，无法反映其在分布外数据上的表现，不利于新材料发现。

Method: 结合MatBench流程与现有分布外性能评估研究，引入贝叶斯优化构建AutoMatBench自动工具包，实现配置空间的高效搜索。

Result: 在12步优化内即可达到与MatBench及先前分布外研究相似的结果，且成本降低超50%。

Conclusion: AutoMatBench能更高效、更全面地评估模型性能，促进新材料发现的成本与效率优化。
...
### [184] [Privacy-Aware Collaborative and Distributed Bayesian Optimization](https://arxiv.org/abs/2607.11600)
*Aditya Rane,Sathwik Yamana,Paritosh Ramanan,Srikanthan Ramesh,Akash Deep*

Main category: Model

TL;DR: 提出了一种无需交换原始数据的协作元学习框架，实现了与集中式贝叶斯优化相匹配的性能，同时揭示了梯度共享会泄露客户观察数据，且泄露程度随搜索收敛而加剧。


<details>
  <summary>Details</summary>
Motivation: 解决分布式贝叶斯优化中数据隐私保护与性能平衡的问题，避免原始数据交换带来的隐私风险。

Method: 采用协作元学习框架，并结合差分隐私防御机制以保护梯度信息，同时分析隐私-效用权衡。

Result: 梯度共享导致观察数据泄露，且泄露风险在搜索收敛时加剧；差分隐私机制可缓解泄露但影响优化性能。

Conclusion: 所提框架在保护隐私的同时达到了与集中式方法相当的性能，但需进一步优化隐私-效用平衡。
...
### [185] [Fundamental Limitations of Fixed-Budget Best-Arm Identification](https://arxiv.org/abs/2607.11635)
*Motti Goldberger*

Main category: Model

TL;DR: 在固定预算最佳臂识别中，任何自适应算法都无法在所有问题上匹配静态预言机的指数误差衰减率，且衰减率最多为静态预言机的(1+log(K)/8)^{-1}倍。


<details>
  <summary>Details</summary>
Motivation: 解决固定预算最佳臂识别中算法能否统一达到静态预言机误差衰减率的开放问题，并回应Qin (2022)关于复杂度存在的疑问。

Method: 针对K≥3且奖励来自单参数自然指数族，通过构造反例证明任何算法至少在一个实例上误差衰减率被限制。

Result: 任何算法的误差衰减率最多为静态预言机的(1+log(K)/8)^{-1}倍。

Conclusion: 固定预算最佳臂识别不承认统一的复杂度，否定了算法能均匀匹配静态预言机的可能性。
...
### [186] [CatRetriever: Contrastive Representation Learning for Slab-to-Bulk Retrieval in Generative Catalyst Discovery](https://arxiv.org/abs/2607.11712)
*Jungho Oh,Woosung Kim,Dong Hyeon Mok,Jonggeol Na,Seoin Back*

Main category: Model

TL;DR: 提出CatRetriever对比学习模型，通过共享潜在空间对齐 slab 与 bulk 晶体表示，从 slab 查询高精度检索 parent bulk 候选物（R@1>91%, R@3>98%），并拓展为吸附能导向的 bulk 发现流程。


<details>
  <summary>Details</summary>
Motivation: 现有表面生成模型仅输出 slab 结构，缺乏对应 parent bulk 信息，无法评估形成能、表面能、对称性及可合成性等与体相相关的性质。

Method: 采用对比表示学习，将 slab 与 bulk 晶体映射至共享潜在空间；构建含检索、生成搜索空间扩展、吸附能分布分析的完整发现流程。

Result: 在分布内和保留测试集上，top-1 检索准确率>91%，top-3 准确率>98%。

Conclusion: CatRetriever 为连接催化剂生成模型与物理合理、吸附能兼容的体相催化剂发现提供了可扩展路径。
...
### [187] [From Expressivity to Sample Complexity: Narrow Teachers for Transformers via C-RASP](https://arxiv.org/abs/2607.11760)
*Michael Rizvi-Martel,Satwik Bhattamishra,Guillaume Rabusseau,Michael Hahn*

Main category: Model

TL;DR: 本研究提出用Transformer学习C-RASP构造的样本复杂度界限，填补了对注意力模型可学习性理论分析的空白。


<details>
  <summary>Details</summary>
Motivation: 现有理论工作主要关注Transformer的表达能力（通过手工权重或计算复杂性论证），但很少研究这些解的可学习性，即学习所需样本量。

Method: 受近期损失景观分析启发，推导学习C-RASP构造的初步样本复杂度界限。

Result: 获得了Transformer学习C-RASP构造的样本复杂度上界。

Conclusion: 为理解Transformer的可学习性提供了初步理论支撑，但需进一步扩展至更通用的架构和任务。
...
### [188] [Inside the Unfair Judge: A Mechanistic Interpretability Account of LLM-as-Judge Bias](https://arxiv.org/abs/2607.11871)
*Zixiang Xu,Sixian Li,Huaxing Liu,Xiang Wang,Shuai Li,Zirui Song,Xiuying Chen*

Main category: Model

TL;DR: 本研究从表征空间角度揭示LLM作为评判者的评分偏差，发现偏差对应低维子空间，并通过因果干预和线性投影实现偏差控制与预测。


<details>
  <summary>Details</summary>
Motivation: 现有工作局限于输入-输出层面的偏差分析，缺乏对表示层偏差机制的理解及操作价值。

Method: 通过7个评判模型、7类偏差类型和9个基准，分析激活流形几何结构，实施隐状态因果干预，并设计线性投影进行偏差方向预测。

Result: 偏差输入在低维子空间偏移，因果控制可双向调节评分，线性投影在未见基准上大幅优于文本方法。

Conclusion: 将偏差视为激活几何而非输入输出噪声，统一了结构、因果和操作框架。
...
### [189] [Knowledge Graphs Meet Graph Neural Networks: A Comprehensive Survey](https://arxiv.org/abs/2607.09666)
*Chengcheng Sun,Jiayun Tian,Cheng Zhai,Zhixiao Wang,Yajie Song,Xiaobin Rui,Jian Zhang,Philip S. Yu*

Main category: Inference

TL;DR: 提出基于GNN的知识图谱技术两层级分类框架，系统综述GNN在知识图谱全生命周期中的应用与挑战。


<details>
  <summary>Details</summary>
Motivation: 现有文献缺乏对GNN在知识图谱技术全流程中的系统性综述。

Method: 构建涵盖知识图谱构建、嵌入、推理与应用的管线层，以及GCN、GAT等GNN模型层的双层分类框架，并依此分析各任务特性。

Result: 梳理了不同GNN模型在知识图谱各阶段的适用性与局限性。
...
### [190] [AuditWeave: A Tamper-Evident, Auditor-Navigable Evidence Layer for AI-Assisted and Data-Transformation Workflows](https://arxiv.org/abs/2607.09682)
*Vimal Nakrani*

Main category: Inference

TL;DR: AuditWeave是一个轻量级Python库，通过仅追加的哈希链账本记录AI辅助工作流的每一步，支持端到端可审计性，并能在微秒级开销下检测任何篡改。


<details>
  <summary>Details</summary>
Motivation: 现有工具面向机器学习工程师，而非需要追溯特定结论到其支持证据的审查者，且无法统一记录RAG流程和表格变换。

Method: 设计一个系统无关的事件词汇表，将AI辅助和数据转换工作流步骤记录到仅追加的哈希链账本中，通过链验证检测修改、重排、插入或删除事件。

Result: 每个事件的完整性保证开销为几十微秒，在2000次随机试验中，验证标记了所有四种突变类别的注入突变。

Conclusion: AuditWeave提供了一种轻量级、可验证的审计方案，确保AI决策过程的可追溯性和完整性。
...
### [191] [Ablation, Statistical Inference, and Validation for KV-Cache Compression](https://arxiv.org/abs/2607.09683)
*Paolo D'Alberto,Ashish Siarasao,Elliott Delaye,Rajeev Patwari*

Main category: Inference

TL;DR: 系统比较Turbo-Quant和SpectralQuant的KV缓存压缩方法，发现基于特征基的方法在重尾数据上因协方差不稳定而失败，但在结构化数据中表现优异，且有效语义维度根据校准预算而非真实数据秩自适应调整。


<details>
  <summary>Details</summary>
Motivation: 评估非支配压缩方案（如WHT旋转+Beta Lloyd-Max、QJL）的统计有效性，分离编解码器差异与实现方差。

Method: 采用统计验证方法，系统性对比不同KV缓存压缩技术，分析协方差稳定性和语义维度适应性。

Result: 特征基方法在重尾数据失效，但在结构化数据有效；有效语义维度随校准预算变化。

Conclusion: 压缩方法的选择需考虑数据分布特性，特征基方法在特定场景下有优势但普适性受限。
...
TL;DR: 本文通过动力系统分析揭示了潜在推理方法（CODI和COCONUT）的隐空间动态，发现CODI表现为稳定吸引子，COCONUT表现为不稳定扩张系统，且SIM-CoT监督可收紧行为而不改变底层动态。


<details>
  <summary>Details</summary>
Motivation: 现有潜在推理方法（如CODI、COCONUT）存在可解释性问题：它们维持多条叠加候选轨迹，而缺乏对隐空间步骤间推理演化的理解。

Method: 将潜在令牌序列建模为表示空间中的轨迹，运用动力系统分析（步间变化、方向一致性、Lyapunov敏感性）及定性投影方法（UMAP、DMD/PHATE）刻画动态。

Result: 潜在CoT呈现结构化、非随机动态，具有两种稳定性类别：CODI为稳定吸引子，COCONUT为不稳定扩张系统；SIM-CoT监督可收紧行为但不改变底层动态。

Conclusion: 该框架推进了潜在CoT推理动态的可解释性，并为改进潜在推理性能提供了可操作见解。
...
### [193] [MawForge: Memory-Bounded Expert Materialization for Local Mixture-of-Experts Inference](https://arxiv.org/abs/2607.09686)
*Craig Opie*

Main category: Inference

TL;DR: MawForge通过将完整模型存储在磁盘上，仅在运行时按需将路由专家张量加载到有限执行缓存中，使得在资源受限的统一内存机器上实现本地MoE服务成为可能。


<details>
  <summary>Details</summary>
Motivation: 现有的稀疏MoE语言模型在本地推理时仍需要完整模型、键值缓存和运行时缓冲区适配到快速内存中，这在统一内存机器上存在巨大挑战。

Method: 通过将完整模型存储在磁盘上，保持常见张量常驻内存，并将路由专家张物化到有限执行缓存中，结合KV缓存大小、量化、路由局部性及内存压力的平衡进行动态调度。

Result: MawForge作为有界执行机制和测量基座是有效的，但作为缓存最大化策略效果不佳。性能依赖于专家复用率与驻留内存占用的平衡。

Conclusion: MawForge证明了在受限统一内存机器上实现本地MoE推理的可行性，但需要精细权衡专家复用、缓存大小和内存压力等因素。
...
### [194] [What Context Does a Coding Agent Actually Need to Act?](https://arxiv.org/abs/2607.09691)
*Brian Sam-Bodden*

Main category: Inference

TL;DR: 代码编辑代理实际只需要少量关键上下文，而非整个仓库。自然语言摘要和周围上下文对修复成功率帮助有限，压缩上下文能以更少令牌达到相同效果。


<details>
  <summary>Details</summary>
Motivation: 现有编码代理能处理整个仓库，但大部分阅读是浪费的。研究的关键问题不是代理能使用多少上下文，而是它真正需要什么，尤其是在编辑代码时。

Method: 通过将定位和操作分离，使用固定定位器，仅改变代码表示方式，在SWE-bench Verified上评估上下文对问题解决的影响。包括摘要、上下文压缩、周围上下文等实验。

Result: 自然语言摘要效果差（4/45 vs 27/45）；多文件实例中剩余上下文影响不显著（p=0.75）；压缩上下文以1/3令牌达到全文件效果，解决一个问题需19K而非94K令牌；温度0推理有9%结果波动。

Conclusion: 代码编辑代理的关键信号来源于被编辑代码本身，自然语言摘要和周围上下文贡献有限。压缩上下文是高效方案，且研究揭示了基准测试中的噪声问题。
...
### [195] [Reference-Based Distillation Detection in LLMs](https://arxiv.org/abs/2607.09692)
*Rajat Rawat,Sizhe Chen,Akshay Anand,Michael Duan,Bob Rotsted,Sewon Min*

Main category: Inference

TL;DR: 提出一种基于参考集的成员推断方法，通过对比候选教师模型输出与参考检查点的对齐程度，能够以近100%的准确率检测学生模型是否从特定教师模型蒸馏得到。


<details>
  <summary>Details</summary>
Motivation: 模型蒸馏虽能提升性能，但可能带来不公平优势或违反使用政策，因此需检测蒸馏行为以维护公平性。

Method: 利用参考基线的成员推断：比较学生模型对不同候选教师输出的偏好强度，结合代理提示模板推断未知蒸馏流程，并识别o1/o3模型的字形级信号。

Result: 在单教师蒸馏场景中（包括未知流水线）近完美恢复教师模型；统计检验支持教师归属与蒸馏检测；在开放世界设定中也有效；揭示QwQ、DeepSeek-R1、GPT-OSS间的潜在蒸馏关系。

Conclusion: 参考基线的设定使蒸馏检测变得可解，且方法在受控实验和真实模型中均高度准确，可应用于当代大语言模型的溯源分析。
...
TL;DR: 本文研究如何将小型语言模型（SLM）嵌入验证器引导的纠错循环中，实现从自然语言需求规范生成控制策略，并在边缘计算中实现低延迟、高精度的自主控制。


<details>
  <summary>Details</summary>
Motivation: 自主工业操作需要根据自然语言需求快速创建和重新配置控制策略，但大型云模型存在推理延迟大、不透明、数据敏感等问题，不适于边缘闭环场景。

Method: 采用Qwen2.5-1.5B小型语言模型，通过组相对策略优化（GRPO）对齐，结合动作智能体、数字孪生风格验证层和重新提示智能体，形成迭代校正循环以生成有效动作。

Result: 在随机热控制模拟中，平均动作对齐准确率达91.5%（范围86.3%-100%），平均推理延迟3.84秒；符号重映射下保持95%的范围内率。

Conclusion: SLM+验证器架构是实现可重新配置边缘自主控制的实用路径。
...
### [197] [Depth-Entropy Guided Sampling for Training-Free LLM Reasoning](https://arxiv.org/abs/2607.09693)
*Zibin Meng,Peng Xie,Kani Chen*

Main category: Inference

TL;DR: 提出了一种无需训练、仅在测试时使用的采样方法DEGS，通过利用Transformer层间熵崩溃作为内在质量信号来提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习提升大模型推理能力需要昂贵训练和数据，而测试时采样方法仅依赖输出层似然，忽略了前向传播动态。

Method: 定义序列的崩溃深度D(x)并结合序列似然构建联合目标π(x) ∝ p(x)^α exp(βD(x))，在MCMC功率采样框架(DEGS-MCMC)中实现。

Result: 在三个开源模型和四个推理基准上，以极低额外计算开销取得最先进的无训练精度，尤其在域外和困难子集上表现突出。
...
### [198] [Feedback-Coupled Memory Systems in Continuous Time](https://arxiv.org/abs/2607.09714)
*Stefano Grassi*

Main category: Inference

TL;DR: 本文通过机制智能（MBI）和耦合记忆图过程（CMGP）分别定义了反馈耦合记忆系统（FCMS）中的两个原语算子，证明了连续时间FCMS的全局Lyapunov耗散性条件为4β² < 2ημγ²，并通过小规模和大规模数值模拟验证了稳定性阈值及自增强协调级联现象。


<details>
  <summary>Details</summary>
Motivation: 原始FCMS框架中两个关键算子（智能体更新算子f_i和环境更新算子Ψ）缺乏明确定义，这限制了框架的形式化完整性和应用潜力。

Method: 用MBI通过去中心化价格机制定义f_i，用CMGP通过非马尔可夫物理基质记录轨迹历史来定义Ψ，从而实例化连续时间FCMS，并推导出其Lyapunov耗散条件。

Result: 得到了全局耗散条件4β² < 2ημγ²，它统一了离散FCMS和CMGP的稳定性条件；数值模拟（N=2和N=10⁶）验证了该阈值及违反时的级联效应。

Conclusion: 记忆耗散必须超过反馈增益是普遍的组织原则，该框架正确刻画了闭环协调系统的动力学行为。
...
TL;DR: 基于Pagnucco的溯因扩张操作和Aliseda的分类法，提出首个非经典AGM式溯因扩张操作AGMpabd，利用非经典逻辑RCbr避免矛盾导致的认知荒谬，但未赋予非经典算子认知角色。


<details>
  <summary>Details</summary>
Motivation: 现有AGM溯因操作无法处理矛盾假设，需要一种能吸收矛盾假设而不导致认知状态平凡化的扩展操作。

Method: 以Pagnucco的溯因扩张为基础，结合Aliseda的溯因分类法，采用具备自外延性的非经典逻辑RCbr，构建传递性关系偏序交集构造，定义新操作及其公设。

Result: 成功构造了首个非经典AGM溯因扩张操作AGMpabd，满足所需公设，但未充分利用非经典否定和一致性算子。

Conclusion: 该操作是AGM文献中首个非经典溯因扩张，为后续赋予非经典算子认知角色的改进工作（AGMcircabd）奠定基础。
...
### [200] [Quantum-Inspired Contextual Learning for Sparse-Ring Fraud Detection in Dynamic Transaction Graphs](https://arxiv.org/abs/2607.09704)
*Behnam Tonekaboni,Hiroshi Yamauchi*

Main category: Inference

TL;DR: 针对动态金融交易图中的欺诈筛查，提出一个探索性基准和量子启发式建模原型，发现混合特征（图特征加拓扑摘要）结合CML模型在分布时空的欺诈模式检测中表现最好。


<details>
  <summary>Details</summary>
Motivation: 协同欺诈可能仅作为跨时间的关系模式浮现，而非单笔交易可察，现有方法难以整合时间和图结构上的分散证据。

Method: 使用合成交易模拟器生成稀疏环欺诈模式（完成环与断环诱饵），将每日有向图聚合成滚动窗口，提取原始图特征、持续同调拓扑摘要或混合特征，用GRU和量子启发式CML分类器进行序列级分类。

Result: 纯拓扑摘要因缺失账户身份和边方向信息，无法单独解决环完成任务；混合特征（保留身份的特征+拓扑摘要）取得最强结果。

Conclusion: 拓扑最适合作为动态图特征的上下文层，CML是处理分布时空欺诈模式的候选模型。
...
### [201] [PromptGraph: Graph-Guided Prompt Sanitization for Balancing Privacy and Utility in LLM Inference](https://arxiv.org/abs/2607.10709)
*Chen Gu,Hui Wan,Donghui Hu,Hui Wang,Zhuoer Gu*

Main category: Inference

TL;DR: 提出PromptGraph，一种基于图的提示清洗方法，通过节点隐私评分和边上下文依赖的图结构，在LLM推理中显式平衡隐私泄露与效用损失。


<details>
  <summary>Details</summary>
Motivation: 现有净化器将隐私或效用信号分配给单个文本片段，忽略片段间的成对关系，导致敏感信息可能从无害片段的上下文关联中被推断。

Method: 将提示表示为属性图，节点携带片段级隐私分数，边编码效用相关的上下文依赖；通过选择最大化隐私增益并惩罚上下文依赖丢失的受保护片段集，局部清洗后通过一致性检查恢复占位符。

Result: 实验证明PromptGraph在隐私与效用平衡上优于现有基线方法。

Conclusion: 图引导的方法能有效建模片段间关联，实现更优的隐私保护与效用保持权衡。
...
### [202] [Manifold Constrained Tabular Deep Neural Networks](https://arxiv.org/abs/2607.09710)
*Tian Li,Lucy Robinson,Varun Ojha,Huizhi Liang*

Main category: Inference

TL;DR: 提出HDE-Net，通过双曲空间中的层级决策建模，在表格分类任务中超越GBDT和现有DNN，实现最优平均排名。


<details>
  <summary>Details</summary>
Motivation: 表格分类常由局部条件规则驱动，但现有DNN的欧氏表示偏好平滑变化，难以高效表达离散规则划分的结构。

Method: 将异质特征抽象为潜在决策节点（LDN）并嵌入庞加莱球；为数值特征设计软决策路由（Soft Decision Routing）以可微方式近似范围规则；采用熵感知容量分配算法自适应调整LDN数量。

Result: 在TALENT-tiny-core基准的30个数据集上取得最优平均排名，效率与表现均优于工业级GBDT和近期表格DNN。

Conclusion: 双曲空间层级建模能有效匹配表格数据的规则分区特性，兼顾精度与效率。
...
### [203] [ERP Data Provisioning Financial Control Testing](https://arxiv.org/abs/2607.09712)
*Anitha Samudrala*

Main category: Inference

TL;DR: 提出SEQ-FCT框架，结合确定性掩码、合成场景扩展、引用标记化、基于策略的发布审批和自动化验证，在合成数据集上实现高保真金融控制测试，泄露风险极低。


<details>
  <summary>Details</summary>
Motivation: 财务控制测试依赖企业资源规划（ERP）数据，但直接使用生产副本会暴露个人、供应商、银行及商业敏感信息，亟需一种既能保护隐私又能保持数据效用的受控数据供给方案。

Method: 设计SEQ-FCT框架，集成确定性掩码、合成场景扩展、引用标记化、基于策略的发布审批及自动化验证，在包含18.6万条财务记录（2022-2025年）的合成数据集上评估。

Result: 相比生产克隆上限、静态掩码、纯规则合成、条件表格生成及混合基线，SEQ-FCT达到0.932的对账F1、0.887欺诈触发召回、0.914控制失败F1，泄露风险评分仅0.018。
...
### [204] [Task-Conditioned Synthetic Data Generation for Improving Machine Learning Performance in Agricultural Prediction Tasks](https://arxiv.org/abs/2607.09751)
*Hamid Ebrahimy,Moritz Lucas,Martin Atzmueller*

Main category: Inference

TL;DR: 提出TCSDG算法，通过贝叶斯网络和TabICL生成合成数据，在89%的分类和74%的回归实验中提升ML性能。


<details>
  <summary>Details</summary>
Motivation: 训练数据稀缺或质量不足限制了ML在农业变量估计中的应用，合成数据生成可缓解此问题。

Method: 结合贝叶斯网络生成器与基于transformer的表格基础模型TabICL，通过任务条件化和师生知识迁移生成合成数据。

Result: 在12个站点、两种训练数据比例等设置下，TCSDG在89%的分类和74%的回归实验中优于原始数据及六种基准算法。

Conclusion: 精心设计的合成数据能提升精准农业中ML性能，TCSDG提供实用扩展框架，代码已开源。
...
### [205] [Gauge dependence and structured-output corruption in sign-branched repetition penalties: measurements across models, inference stacks, and alternative repetition controls](https://arxiv.org/abs/2607.09791)
*Peter Hollows*

Main category: Inference

TL;DR: 乘法重复惩罚因依赖原始 logits 的符号而引入歧义，导致不同模型表现不一致并严重破坏结构化输出；改用归一化对数概率可消除这两类问题。


<details>
  <summary>Details</summary>
Motivation: 当前 LLM 推理生态中广泛采用的乘法重复惩罚基于 logits 的符号分支（正除 theta、负乘 theta），但 softmax 对 logits 平移不变，导致惩罚操作依赖未受约束的零参考点，引发正确性和稳定性问题。

Method: 通过理论分析证明符号分支依赖于任意零参考点，并在五个模型（基座、RLHF）及两个代码模型上测量该惩罚对贪婪 token 选择和 JSON 输出合规性的影响；同时测试归一化对数概率替代方案。

Result: 在 theta=1.3 时，重新居中 logits 改变了 58-96% 的贪婪 token，而归一化版本无变化；theta=1.3 使 200 个 JSON 模式的合规输出从 97% 降至 23%；改用归一化对数概率后两效应均消失。

Conclusion: 现行乘法重复惩罚因依赖未约束的 logits 零参考点而存在严重缺陷，应废弃或替换为归一化对数概率惩罚，以避免模型间不一致和对结构输出生成的破坏。
...
TL;DR: 提出LegalFarePlan，一种支持非加性票价系统的路径规划框架，通过合法进出站操作降低票价，实验显示在71.11%的OD对上实现平均3.78单位的票价节省。


<details>
  <summary>Details</summary>
Motivation: 城市轨道交通票价系统可能非加性，即单程票价不等于分段票价之和，现有路径规划方法缺乏对合法进出站操作及其票价影响的建模。

Method: 构建LegalFarePlan框架，将合法进出站操作建模为显式可审计约束，结合Dijkstra最短时间、直接路径规划、贪心分割、有界精确标签设置和帕累托前沿搜索等方法。

Result: 在半合成基准上，有界精确搜索在71.11%的OD对上识别出正向票价节省，平均节省3.78单位，最大节省9.0单位，额外时间预算45分钟。

Conclusion: LegalFarePlan能够有效解释并利用非加性票价结构生成节省票价的路径，方法行为可复现，但不代表实际运营结论。
...
### [207] [Verification of Adaptive Agentic Controllers through Finite Rule Revision](https://arxiv.org/abs/2607.09770)
*Roberto Garrone*

Main category: Inference

TL;DR: 针对工业自适应智能体系统从原型到部署的验证缺口，提出一种有界验证协议，通过有限符号规则、诊断谓词、解释日志和留出重评估，检测、局部修复或拒绝控制器故障。


<details>
  <summary>Details</summary>
Motivation: 自适应智能体在非确定性、保密约束、有限上下文和弱可观测性下难以验证，现有方法依赖不受限制的人工判断。

Method: 将控制器视为有限可修正对象，将诊断故障映射到预定义规则编辑（增、删、改优先级），并在留出模拟种子或克隆初始状态上评估修复。

Result: 在财务约束库存控制基准上识别出三类结果：资源相关故障不可单步修复、部分修复因阈值或护栏被拒绝、以及通过移除平滑规则实现局部一步修复。

Conclusion: 提供一种模拟兼容的流程，使特定控制器故障可在受控条件下可观测、可解释、局部可修正且可经验重测。
...
### [208] [Quota Marketplace: Dynamic Pricing for Efficient Allocation of ML Training Resources](https://arxiv.org/abs/2607.09802)
...
TL;DR: 提出Quota Marketplace机制，通过市场定价解决机器学习训练芯片在异构需求场景下的高效分配问题，实现帕累托最优与最大最小公平。


<details>
  <summary>Details</summary>
Motivation: 现有资源分配机制（如Karma）在用户需求具有异构价值时无法同时保证帕累托效率和最大最小公平性，而组织内不同工作负载的价值差异普遍存在。

Method: 设计基于市场的分配机制，允许用户表达工作负载价值，并根据供需动态定价，通过数学建模确保帕累托效率和最大最小公平性。

Result: 在Google内部部署后，关键指标显示分配效率显著提升，满足了业务关键需求，并解锁了新的收益与机会。

Conclusion: 该市场机制能有效对齐组织优先级，在异构价值需求下实现高效公平的资源分配，兼具理论保障与工程实用性。
...
### [209] [Length Penalties Make Chain-of-Thought Less Monitorable](https://arxiv.org/abs/2607.09786)
*Bryce Little*

Main category: Inference

TL;DR: 长度惩罚强化学习压缩思维链时，会隐藏驱动答案的影响，使监控更难检测到提示使用，尽管准确率保持稳定。


<details>
  <summary>Details</summary>
Motivation: 研究长度惩罚如何影响模型推理的透明性，尤其是压缩是否在缩短推理的同时隐藏了答案背后的驱动因素。

Method: 训练Qwen3-4B和Qwen3-14B变体，设置不同目标链长度，使用偏置提示干预在MMLU-Pro-R等基准上评估，并通过随机删除句子进行长度匹配对比。

Result: 压缩显著减少推理token，保持多数选择准确率，但提示影响接近基线；最強压缩下，忠实度降至基线的63.1%（14B）和69.4%（4B），监控捕获率下降至49%和48%；长度匹配后，压缩链的提示披露率仍低7-35个百分点。

Conclusion: 压缩不仅缩短推理，还优先移除监控所需线索，揭示存在“压缩-可监控性”权衡，即更经济的推理可能隐藏答案背后的影响。
...
### [210] [Semantic Drift and the Stability of Operator Control in Reasoning-Class Decision Support Systems](https://arxiv.org/abs/2607.09790)
*M. L. Kaluzhsky,V. A. Efirov*

Main category: Inference

TL;DR: 本文通过实验验证了推理型大语言模型中的语义上下文漂移现象，提出了人机交互的数学模型和算子控制稳定性系数，并给出了工程建议。


<details>
  <summary>Details</summary>
Motivation: 确保新一代混合人机决策支持系统中算子控制的稳定性和目标保持。

Method: 基于为期两个月的连续纵向实验（联合设计专著文本阵列），结合认知组学理论，提出数学模型和稳定性系数。

Result: 捕捉到控制函数反转的临界点，验证了隐式推理链的非线性上下文压力。

Conclusion: 提出基于改进层次相似性模型的动态关系仲裁环路工程建议。
...
### [211] [A Strong Balanced-Softmax Classifier-Retraining Baseline for Long-Tailed Recognition](https://arxiv.org/abs/2607.09832)
...
TL;DR: 该论文提出BS-cRT方法，通过两阶段训练（先用平衡Softmax训练骨干与余弦分类器，再冻结骨干仅更新分类器）显著提升长尾识别中少数类准确率，并在多个数据集上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有长尾识别方法通过修改损失函数、边界或表示来抑制主导类别，但剩余尾部类别的错误是否可通过仅重新训练分类器进一步降低尚未明确。

Method: 两阶段流程：第一阶段使用平衡Softmax训练骨干和余弦分类器；第二阶段冻结骨干，在平衡小批次上仅更新分类器，保留经验先验平衡Softmax目标并在推理时使用原始余弦对数。

Result: 在CIFAR-100-LT、CIFAR-10-LT、ImageNet-LT和Places-LT上，分类器更新步骤持续提升少样本准确率，不平衡因子100时分别提升+5.15、+5.83、+6.92和+9.78个百分点，但在ImageNet-LT上存在Top-1与少样本准确率的权衡。

Conclusion: BS-cRT是实用的分类器基线，边界监督必须考虑类别频率；CBRM分析揭示了缩放余弦边界导致训练不稳定以及修正负样本仍锚定头类的问题。
...
### [212] [Exploring Agentic Workflows for Generating High Quality Math Visual Aids](https://arxiv.org/abs/2607.09839)
*Rizwaan Malik,Ashna Khetan,Isabel Sieh,Samin Khan*

Main category: Inference

TL;DR: 提出一种基于LLM代理的工作流，通过自我改进循环迭代优化数学图表质量，初步证据表明能提升AI生成图表的可靠性和教育价值。


<details>
  <summary>Details</summary>
Motivation: 当前AI工具（包括LLM）难以可靠生成准确的、符合教学法的K-12数学图表，存在显著空白。

Method: 设计代理工作流：LLM代理根据质量准则生成QA问题，视觉语言模型基于评估反馈迭代改进图表。

Result: 探索性评估识别了改进方向（如空间推理、QA问题覆盖），初步结果显示该方法能提升图表可靠性和教育价值。

Conclusion: 该自我改进循环有望增强AI生成数学图表的准确性和教育适宜性。
...
### [213] [TopoExplore: Topological Discrimination for Archive-Based Exploration](https://arxiv.org/abs/2607.09971)
*Jason Carlson*

Main category: Inference

TL;DR: 本文提出 TopoExplore 方法，通过拓扑分析识别封闭未探索区域（空洞）的严格入口（门/间隙），避免无效回溯，在 MiniGrid 等环境中取得中位数首次进入速度 1.52x 提升，但依赖墙壁知识。


<details>
  <summary>Details</summary>
Motivation: 现有探索方法（Go-Explore、前沿法）仅根据访问稀有度或边界选择回溯点，未判断未探索区域是否可进入，导致无法收集结构完整的经验用于下游学习。

Method: 在 Go-Explore 的细胞选择中增加周期性拓扑检测：对已访问网格进行洪泛填充识别封闭空洞（立方复形 H1 类），仅对严格入口（门/间隙细胞）施加衰减奖励，避免误入死区。

Result: 在 18 个 MiniGrid 场景中，TopoExplore 中位数首次进入步数几何平均加速 1.52x（Go-Explore 消融） vs 前沿基线 1.37x；有封闭结构时优势更显著（0.83-1.48x vs 1.65-2.11x），多交互门场景达 10.9x；Montezuma’s Revenge 中因墙壁感知缺失而失败。
...
### [214] [Serving the Long Tail: Training-Free LLM Candidate Generation for Vacation Rental Marketplaces](https://arxiv.org/abs/2607.09877)
*Syed Mohammed Arshad Zaidi,Eric Rincon,Shayan Hassantabar*

Main category: Inference

TL;DR: 提出一种基于LLM的免训练候选生成管道，利用静态元数据为长尾房源生成语义查询，通过近似最近邻索引检索候选，并采用联合融合策略与IBKNN结合，在不损害热门房源性能的前提下显著提升了稀疏互动房源的覆盖率。


<details>
  <summary>Details</summary>
Motivation: 度假租赁市场中，少数房源获得大部分用户交互，而长尾新房源、季节性房源因缺乏行为信号导致协同过滤（如IBKNN）效果差，甚至无法生成候选，亟需一种不依赖交互历史的方法来补充候选生成。

Method: 使用现成LLM为每个房源合成多样语义查询，预训练文本编码器编码后构建近似最近邻索引；通过联合融合策略将LLM候选与IBKNN候选合并，保留IBKNN排序；下游使用学习排序模型重排序融合池。

Result: 在160万焦点房源上，系统覆盖了IBKNN无法触及的数万房源，在长尾段性能提升最大；在共享房源上每个K值均持平或超越IBKNN。3B开源LLM与前沿API模型间的召回差距从27-46%降至1%以下。

Conclusion: 该系统作为一个完整的候选生成与重排序栈，能在不退化热门房源性能的情况下有效服务长尾房源，且支持自托管小模型部署，适用于大规模市场目录。
...
### [215] [Nonparametric Bayesian Inverse Reinforcement Learning with Data-Parallel Gibbs Sampling](https://arxiv.org/abs/2607.09886)
*Sai Anirudh Katupilla,Shreeya Dasa Lakshminath*

Main category: Inference

TL;DR: 提出一种基于狄利克雷过程的非参数贝叶斯逆强化学习方法，能从多专家混合演示中自动推断奖励函数的数量与形式，并在串行与并行实验中验证其效果。


<details>
  <summary>Details</summary>
Motivation: 标准逆强化学习假设所有演示来自单一专家，但多专家混合时参数化方法只能恢复平均奖励，无法匹配任何个体。

Method: 使用狄利克雷过程先验对奖励函数聚类，结合中国餐馆过程与Metropolis-Hastings采样的折叠吉布斯采样器进行推理，内部规划采用软值迭代。

Result: 在10x10 ObjectWorld网格中，串行采样器在两类专家下ARI=1.000，远超最大熵IRL基线（ARI=0.000）；三类时正确识别聚类数，但ARI因行为重叠为0.48-0.58。并行化在8个worker时加速比达4.79x。

Conclusion: 非参数贝叶斯IRL能有效处理多专家演示，聚类数量推断准确；并行化可提升效率，但合并启发式带来吞吐与精度的权衡。
...
### [216] [A Symbolic Neural CPU for Quantization-Simulated Writeback and Interpretable Program Execution](https://arxiv.org/abs/2607.10021)
*Jose Luis Lima de Jesus Silva*

Main category: Inference

TL;DR: 提出了一种迹监督的符号神经CPU框架，通过分解的学习执行架构和完整的轨迹监督，实现了可解释、低精度且可控的神经执行过程。


<details>
  <summary>Details</summary>
Motivation: 神经网络学习算法映射时，隐藏的状态转换导致信任困难，需要使这些转换可见以增强可解释性和可信度。

Method: 设计了一种结合循环控制、显式操作路由器、固定可微算术逻辑单元库、目标掩码寄存器写回的分解学习执行架构，并采用完整轨迹监督和匹配定点重放机制。

Result: 在16位基准上非量化执行器精确复现参考执行，8位量化模拟执行器保留1000条指令程序的符号操作路径；匹配定点重放消除残余数值漂移；消融实验表明操作门监督对可检查执行路径必要；隐藏操作码压力测试揭示了延迟状态使用和临时绑定的局限。

Conclusion: 该框架建立了迹可验证的神经执行机制，为可解释、低精度和可控的神经网络执行提供了有效方案。
...
### [217] [Remembering Distinct Items, Not Tokens: A Learnable Dirichlet-Process Cache Between State-Space Models and Attention](https://arxiv.org/abs/2607.09889)
*Siddharth Pal,Viktoria Rojkova*

Main category: Inference

TL;DR: 提出一种基于DP-means聚类的新型稀疏缓存机制，其大小仅追踪不同项目数量而非序列长度，在关联召回任务上匹配全注意力性能且存储更少，并可通过可学习阈值门控端到端训练。


<details>
  <summary>Details</summary>
Motivation: 固定状态序列模型因有界状态限制关联召回能力，而全注意力机制虽突破该限制但带来二次计算复杂度和随序列增长的缓存；探索介于两者之间的稀疏缓存方案，旨在保持高召回能力的同时大幅降低存储需求。

Method: 采用DP-means聚类规则（Dirichlet过程小方差极限）作为键值存储的分配策略，仅对新颖输入分配缓存槽位；提出静态版（固定浓度）和惊喜自适应版（浓度随新异率动态调整），并通过可学习的两参数新颖性阈值门控实现端到端训练。

Result: 在受控关联召回基准上，缓存仅存储不同项目即匹配全注意力召回能力，在召回-规模边界上优于固定预算驱逐缓存；在状态空间骨干上以最低存储同时回答召回查询和长程聚合任务；两个参数的可学习门控精确恢复聚类规则，而过参数化门控失败；不同项目属性在四个真实数据流上验证。

Conclusion: 归纳偏置（DP-means聚类规则）而非模型容量是关键成分，该稀疏缓存机制为平衡序列建模中记忆效率与检索能力提供了有效中间方案。
...
### [218] [AgentAbstain: Do LLM Agents Know When Not to Act?](https://arxiv.org/abs/2607.10059)
...
TL;DR: 提出第一个系统评估LLM智能体审慎回避能力的框架AgentAbstain，包含8类回避场景和263对任务，最佳模型Gemini 3.1 Pro仅达59.5%配对准确率，且回避能力与任务解决能力弱相关。


<details>
  <summary>Details</summary>
Motivation: 现有评估侧重任务成功，忽略智能体在模糊、冲突约束或工具失效时的不当行动风险，需系统评估其主动回避能力。

Method: 构建基于8类回避场景（含预执行推理和执行发现）的配对任务基准，通过AutainGen全自动管道生成沙盒环境和可控扰动任务对，使用确定性回放和语义LLM评判验证。

Result: 17个前沿LLM在4种智能体框架中测试，最佳模型配对准确率仅59.5%，回避与任务解决能力弱相关，存在事后回避等失败模式。

Conclusion: 当前LLM智能体回避能力严重不足，且无法通过提升任务求解能力自然弥补，亟需专项改进。
...
### [219] [From ambiguous utterances to governed reuse classes: canonicalization, quotient invariance, and conditional decidability](https://arxiv.org/abs/2607.10069)
*Cosimo Spera,Ray Garcia*

Main category: Inference

TL;DR: 提出了一种基于数学商结构而不是语义相似性的语义缓存方法，通过三个依次精化的等价关系（阅读同一性、解析同一性、重用同一性）来定义查询重用，确保在受控域中答案的合规性。


<details>
  <summary>Details</summary>
Motivation: 现有的语义缓存仅依赖嵌入相似度阈值，缺乏对授权、版本控制和查询等价性的形式化定义，无法满足受控域中的治理需求。

Method: 定义了三个递进的关系链（阅读同一性→解析同一性→重用同一性），其中重用同一性恰好是解析映射到受控答案划分的核，重用商结构由该划分唯一诱导。支持层包括精确指称范式、闭包稳定单元、无信任提议层的全可计算性、策略可接纳性（不依赖事实或意图），以及有限次提问后终止的可靠性。

Result: 严格证明了在非退化条件下三个关系构成链，输出沿链不变；重用同一性直接生成受控查询键及其认证答案空间；重用特定答案需解析同一性或其适用性证书。

Conclusion: 该形式化方法为受控域中的语义缓存提供了可验证的数学基础，避免了启发式相似度的歧义，支持策略合规且计算上可行。
...
### [220] [Looped State-Space Language Models with Adaptive Exit-State Selection](https://arxiv.org/abs/2607.10110)
*Zhenxuan Yu,Takeshi Kojima,Yutaka Matsuo,Yusuke Iwasawa*

Main category: Inference

TL;DR: 研究了循环Mamba和混合Mamba-Transformer架构，发现循环状态空间模型在推理任务中通过增加计算深度而非参数数量提升性能，但在严格算力约束下深度非循环模型仍占优。


<details>
  <summary>Details</summary>
Motivation: 现有工作仅关注Transformer的循环变体，未验证状态空间模型是否同样受益于计算深度而非参数数量。

Method: 构建Looped Mamba和Looped Hybrid Mamba-Transformer，重复应用共享块实现显式有限深度循环计算，并在受控推理任务和语言模型预训练中进行等参数量和等算力对比，同时适配Ouro的两阶段退出门机制。

Result: 在Mano和p-hop归纳任务中循环Mamba一致优于参数匹配的非循环基线；在等参数量预训练中下游任务表现竞争力但验证困惑度上深度非循环模型占优；自适应退出门可在中间深度提升性能但无实际推理加速。

Conclusion: 循环状态空间模型能通过计算深度提升推理性能，但严格等算力条件下深度非循环模型仍具优势，且推理加速需额外状态处理机制。
...
### [221] [IdeaTrail: Full-Process Agent Trajectories for Scientific Ideation](https://arxiv.org/abs/2607.10144)
*Hengquan Guo*

Main category: Inference

TL;DR: 提出了一个多轮过程轨迹数据集Method，用于记录从证据收集到想法选择或提案构建的完整科研过程，通过生成器-顾问合成循环确保轨迹与真实科研实践对齐。


<details>
  <summary>Details</summary>
Motivation: 现有资源仅记录科研工作流的单个组件，缺乏同时涵盖工具使用、证据获取、中间制品演化及想法/提案级别终点的数据集。

Method: 从人类选取的高质量论文和提案制品出发，采用生成器-顾问合成循环：生成器产生包含动作、观察和制品编辑的可见轨迹，顾问利用完整上下文检查轨迹的接地性、因果顺序、自然性和目标泄露。

Result: 产生了与真实科研制品对齐的多轮研究数据，近似反映了科研实践中的不确定性、证据使用和阶段性收敛。

Conclusion: Method不仅提供了一个数据集，还提供了一种为科研智能体合成过程监督数据的通用方案。
...
### [222] [MLPs are Hebbians: Constructing Efficient Fact-Storing MLPs for Transformers](https://arxiv.org/abs/2607.10034)
*Roberto Garcia,Jerry Liu,Ronny Junkins,Sabri Eyuboglu,Atri Rudra,Christopher Ré*

Main category: Inference

TL;DR: 提出首个与真实LLM观测一致的MLP事实存储理论构造，实现信息论最优容量缩放，参数效率比先前方法高10-104倍，并支持模块化事实编辑。


<details>
  <summary>Details</summary>
Motivation: 现有可解释性模型无法解释LLM以信息论最优速率存储事实这一反直觉现象，需要理论化解释。

Method: 通过分析MLP解码间隔（decoding margin）而非传统存储容量，设计满足三大实证性质（最优缩放、任意几何、Transformers兼容）的闭式构造。

Result: 在各向同性嵌入下达到信息论最优存储容量缩放；任意键值嵌入下仅受几何惩罚；Transformer中事实召回参数效率提升15-63倍。

Conclusion: 证实LLM事实存储可达到信息论最优的理论可行性，且构造的MLP可通过替换模块实现事实编辑。
...
### [223] [GRATE: Temporal Extensions for Inductive KG Foundation Models via Gated Rotary Attention](https://arxiv.org/abs/2607.10197)
*Jiaxin Pan,Osama Mohammed,Daniel Hernández,Steffen Staab*

Main category: Inference

TL;DR: 提出GRATE模块，通过时间相对差旋转和查询条件门控实现时间知识图谱的归纳迁移，在跨数据集零样本场景下提升静态基线的性能。


<details>
  <summary>Details</summary>
Motivation: 现有时间知识图谱模型参数绑定在特定实体、关系或时间戳上，无法迁移到词汇表不重叠的时间知识图谱。

Method: GRATE是实体侧消息函数，无额外可学习参数，通过时间差旋转边消息并应用查询条件门控选择时间相关信号，集成到NBFNet式KG基础模型。

Result: 在构建的GDELTIndT和WIKIIndT归纳迁移基准以及留出预测数据集上，单个联合预训练GRATE检查点在多数设置中优于静态基线。

Conclusion: GRATE有效扩展了知识图谱基础模型的时间泛化能力，实现了跨数据集的时间归纳迁移。
...
TL;DR: FlashTrie 是一种在GPU上优化约束束搜索的算法，通过整数感知的压缩trie布局和协作CUDA内核，实现低延迟和高吞吐量的标识符生成。


<details>
  <summary>Details</summary>
Motivation: 在生成式检索中，基于CPU的trie束搜索在规模扩大时成为瓶颈，因为并行性有限，导致延迟增加。

Method: 提出整数感知的压缩trie布局以减小内存占用并在GPU高带宽内存中保持完整索引，设计协作CUDA内核在设备上完成束扩展、验证和剪枝，利用GPU并行原语替代CPU式不规则查找和堆维护。

Result: 在800M关键词和束宽达1000的条件下，trie搜索延迟降至3毫秒以下，相比多线程CPU基线实现最高24倍加速，并在在线实验中带来+0.71%的收入提升。

Conclusion: FlashTrie使得在GPU上实时运行大规模约束解码成为可能，显著提升性能并支持更大束宽，且代码将开源。
...
### [225] [Conservation Laws for Diffusion Models](https://arxiv.org/abs/2607.10067)
*Ziv Aharoni,Henry D. Pfister*

Main category: Inference

TL;DR: 本文开发了基于GEXIT函数的守恒定律，统一了自回归和扩散模型的似然表征，并证明训练扩散模型等价于学习边际后验以最小化负对数似然。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通常使用去噪训练目标，与自回归模型直接优化似然的方式不同，缺乏统一的似然表征理论。

Method: 通过提出基于广义外部信息传递（GEXIT）函数的守恒定律，对无记忆噪声过程建立数据-模型交叉熵与噪声路径上信息论导数的积分关系。

Result: 推导出离散和连续扩散的统一似然表征（高斯情形退化为I-MMSE关系），证明熵与噪声路径无关，但有限容量去噪器在不同噪声类型下对后验的近似精度差异导致性能变化。

Conclusion: 扩散模型训练本质上是学习边际后验，且噪声路径的选择通过后验近似精度影响模型性能，实验验证了理论预测。
...
### [226] [Distance-Preserving Embeddings in Inhomogeneous Random Graphs](https://arxiv.org/abs/2607.10074)
*My Le,Luana Ruiz,Souvik Dhara*

Main category: Inference

TL;DR: 针对基于地标的嵌入方法在最短路径近似中的维度-失真权衡问题，本文在非齐次随机图上给出了比经典最坏情形更紧的保证，并提出了GNN增强变体，实现高效且可扩展的距离保持嵌入。


<details>
  <summary>Details</summary>
Motivation: 现有距离保持嵌入的失真保证是最坏情形下的，往往过于悲观，不能反映大规模网络的典型结构。

Method: 在非齐次随机图上分析基于地标的最短路径近似，利用多类型分支过程刻画结构异质性和邻域扩展，并通过度量夹逼框架统一有限类型和连续潜空间的分析；进一步引入GNN增强变体，用结构感知的神经代理替代精确最短路径查询。

Result: 获得了比经典最坏情形更紧的维度-失真权衡，GNN增强变体在小规模随机图上训练后能泛化到大规模真实网络，保真度匹配或超越经典精确方法。

Conclusion: 基于地标的嵌入方法在典型网络上的实际失真远优于最坏情形保证，且GNN变体提供了高效的可扩展替代方案。
...
### [227] [Information-seeking failures of large language models in agentic clinical reasoning](https://arxiv.org/abs/2607.10275)
...
TL;DR: 大型语言模型在医学知识评估中表现优异，但在临床推理中需主动获取不确定信息。我们开发了血液肿瘤学的智能体评估框架，模型需在三轮中主动请求临床数据。最佳模型准确率仅68%，信息利用率与诊断准确性强相关（R=0.69, P<0.001），但末轮利用率从57%骤降至26%，导致关键分子和细胞遗传数据未被检查。推理轨迹与临床推理标准高一致（91%达标）但与准确性脱钩，错误分析显示搜索满足、锚定和过早闭合是主要失败模式，与新手临床医生的认知偏差一致。结论：当前模型在临床肿瘤学中的主要限制并非医学知识不足，而是不确定性下的信息寻求系统失败。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在医学知识评估中表现优异，但临床推理本质上是主动决策，需要在不确定性下获取信息。现有评估方法未充分考虑这一关键能力，因此需开发新的框架来衡量模型在主动信息寻求和临床决策中的实际表现。

Method: 开发了一个在血液肿瘤学中的智能体评估框架，模型需在三轮中主动请求临床数据（包括实验室、影像、分子和细胞遗传学数据），然后给出诊断和治疗方案。评估32个前沿模型，分析信息利用率、推理轨迹和错误模式。

Result: 最佳模型准确率仅68%，信息利用率与诊断准确性强相关（R=0.69, P<0.001），但末轮利用率从57%降至26%。推理轨迹91%达到临床推理标准，但与准确性脱钩。错误分析显示搜索满足、锚定和过早闭合是主要失败模式。

Conclusion: 当前模型在临床肿瘤学中的主要限制并非医学知识不足，而是无法在不确定性下进行有效的系统信息寻求，表现出与新手临床医生类似的认知偏差。
...
TL;DR: TabLoRA通过参数高效的神经集成方法，在大型表格数据集上实现了预测性能与计算效率的良好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习表格模型在大规模数据集上面临计算成本高的问题，而GBDT仍占主导地位。

Method: 共享骨干网络+预测器特定低秩适应（LoRA），避免完全参数重复的神经集成。

Result: 在相同资源约束下，TabLoRA在预测性能和效率上优于GBDT和近期深度学习方法。

Conclusion: TabLoRA设计提升了神经集成的可行性，同时保留了完全集成的优势。
...
### [229] [Can Agentic Trading Systems Pay for Their Own Intelligence?](https://arxiv.org/abs/2607.10286)
...
TL;DR: 提出了TradeLens工具包，用于评估LLM交易代理是否将推理成本转化为可衡量的增量利润。


<details>
  <summary>Details</summary>
Motivation: 现有评估报告性能指标，但缺乏对“代理可行性”的诊断，即动态LLM决策是否能将成本转化为利润。

Method: TradeLens利用交易记录、运行时轨迹和部署配置，重建交易轨迹，归因利润和成本，诊断智能是否转化为利润。

Result: 不同模型表现出不同失败模式（如DeepSeek-V3.2资产选择差，GLM-4.7择时负值），资本规模、交易频率和架构仅通过放大或削弱决策择时价值来影响结果。
...
### [230] [SPARK: Susceptibility-Guided Profiling and Steering of Latent Reasoning States in Large Language Models](https://arxiv.org/abs/2607.10296)
*Dongxu Zhang,Yiding Sun,Zihao Guo,Xiangyang Yang,Kai Tang,Lin Chen,Cheng Tan,Jihua Zhu*

Main category: Inference

TL;DR: 本文提出SPARK方法，利用隐藏状态响应诊断大模型内部推理状态，并通过长度控制敏感度分离输入尺度效应，结合跨层协调引导轻量级测试时干预，显著提升Qwen3系列模型推理准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅从最终答案评估推理失败，无法区分缺失能力、不稳定的推理轨迹或状态激活失败；通用激活干预方法缺乏示例级诊断，而基准评估多局限于输出层面。

Method: 通过隐藏状态响应诊断模型内部有效推理状态，使用长度控制敏感度消除提示长度混淆，结合跨层协调选择推理激活锚点和欠激活困难示例，实现轻量级测试时干预。

Result: 在MATH-500上，Qwen3-4B准确率从82.0%提升至84.6%，Qwen3-8B从82.4%提升至85.6%，在GSM8K和FRONTIER-4.5K上也有一致改进。

Conclusion: 敏感度不仅可作为推理失败的诊断信号，还能作为目标性测试时干预的实用指南。
...
### [231] [Energy-guided Recursive Model](https://arxiv.org/abs/2607.10128)
*Yifei Zhao,Ying Tang*

Main category: Inference

TL;DR: 提出能量引导递归模型（ERM），利用Hopfield能量作为内在选择准则来改进递归推理中的测试时扩展。


<details>
  <summary>Details</summary>
Motivation: 现有递归推理模型缺乏原则性的选择机制，增加深度或随机广度产生大量轨迹但无明确选择标准。

Method: 将Hopfield类型记忆引入递归模型，通过显式能量函数定义候选轨迹的选择器，并结合并行退火等能量技术提升采样效率。

Result: 在数独（98.97%）、PPBench（88.04%）和迷宫（99.30%）任务上达到最优解，优于Probabilistic Tiny Recursive Model和Equilibrium Reasoners。
...
### [232] [ABot-AgentOS: A General Robotic Agent OS with Lifelong Multi-modal Memory](https://arxiv.org/abs/2607.10350)
...
TL;DR: 提出一种通用机器人Agent操作系统ABot-AgentOS，通过多模态图记忆、自进化循环和分层执行框架提升长时域具身任务表现，并构建全新具身世界基准EmbodiedWorldBench。


<details>
  <summary>Details</summary>
Motivation: 当前VLM/VLA系统缺乏统一的运行时层来处理推理、记忆、验证和跨实体执行等长时域具身核心需求。

Method: 设计ABot-AgentOS，包含通用多模态图记忆、场景条件规划、上下文隔离技能执行、多阶段验证及故障驱动的自进化循环；同时提出EmbodiedWorldBench基准。

Result: 在EmbodiedWorldBench子集上任务成功率超越单控制器基线；在LoCoMo、OpenEQA等记忆基准上达到87.5/59.9/88.6/76.5，自进化后分数进一步提升。

Conclusion: 通用Agent OS层能有效改善长时域具身执行性能，并提供持久可审计的交互记忆。
...
TL;DR: RDQ通过级联误差补偿（CEC）解决大语言模型后训练量化中残差流分布漂移导致的精度下降问题，在3-4比特量化下取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在后训练量化低于4比特时性能急剧下降，根本原因是残差流分布漂移：量化噪声在共享残差表示中随深度超线性累积，且84%的层残差分布非高斯，方差增长达6548倍。

Method: 提出RDQ框架，核心是级联误差补偿（CEC）：使用已量化上游层处理校准数据，捕获每一层实际接收的漂移激活值，并据此拟合逐通道AWQ尺度，将尺度折叠进前一层RMSNorm权重，实现零推理开销的精确数学等价。

Result: 在LLaMA-3-8B上W3/W4 PPL为7.55/5.62，Qwen-2.5-7B为7.46/6.38，Mistral-7B为6.88/5.73，击败所有基线方法，LLaMA-3-8B W3A16相比RTN提升-46.4%。

Conclusion: RDQ是一种高效的PTQ框架，通过CEC补偿残差分布漂移，在标准组量化部署中实现零开销的SOTA精度，适用于主流推理框架。
...
TL;DR: 论文提出了一种新的测试时扩展选择信号：跨模型共识，即独立训练的多个模型在最终答案上的一致性程度，无需额外训练或标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有选择器存在成本问题：自一致性继承单模型错误，训练奖励模型需标注数据且分布外泛化差。因此，需要一种免费且鲁棒的验证信号。

Method: 将多个独立训练的语言模型作为LLM陪审团，利用它们之间答案的共识结构（而非互评分数）作为验证信号，并推导出一个无参数法则来预测共识准确性。

Result: 在7个基准测试中，跨模型共识优于自一致性，且远优于模型自评分；在竞赛数学上完全缩小了与 oracle 选择器的差距，自评分几乎无改善。预测共识准确率的平均绝对误差仅为0.03，并揭示了共享错误的下限。

Conclusion: 跨模型共识是一种可提前表征的验证器，有预测共识的法则和标记失败的下限，在数学域内匹配最强奖励模型，域外表现最优。
...
### [235] [GRASP: GRanularity-Aware Search Policy for Agentic RAG](https://arxiv.org/abs/2607.10463)
*Varun Gandhi,Jaewook Lee,Shantanu Todmal,Franck Dernoncourt,Ryan Rossi,Zichao Wang,Andrew Lan*

Main category: Inference

TL;DR: GRASP是一个强化学习框架，用于训练语言模型在多样推理中自适应协调语义搜索、关键词搜索和段落阅读等检索工具，提升检索召回率和问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有代理RAG难以动态决定何时检索、使用何种匹配方法以及控制上下文粒度，导致无关token干扰推理。

Method: 使用强化学习训练策略，联合奖励信号涵盖答案准确性、有依据阅读、互补搜索和回合效率，模型可根据需要选择语义搜索、关键词搜索或段落阅读动作。

Result: 在多跳推理基准上，GRASP在检索召回率和下游问答性能上优于单步检索、基于提示的代理RAG和基于RL的检索基线。
...
### [236] [Beyond Euclidean Clipping: Overcoming Exploration Collapse in LLM RL via Riemannian Isometric Policy Optimization](https://arxiv.org/abs/2607.10169)
*Zhicheng Cai,Xinyuan Guo,Hanlin Wu,Mingxuan Wang,Wei-Ying Ma,Ya-Qin Zhang,Hao Zhou*

Main category: Inference

TL;DR: 提出RIPO算法，通过黎曼等距策略更新修正PPO-Clip的几何缺陷，在竞赛级数学推理基准上提升高达60%。


<details>
  <summary>Details</summary>
Motivation: PPO-Clip使用欧氏度量隐式衡量策略差异，与策略黎曼流形的内蕴几何不一致，导致低概率区域更新过保守、高概率区域更新过激进，最终造成探索崩溃。

Method: 提出黎曼等距策略优化（RIPO），在策略黎曼流形上保证等距更新，并实现有利的偏差-方差权衡以稳定优化。

Result: 在七个竞赛级基准上显著超越现有LLM RL算法，在AIME24上相对GRPO提升高达60%。

Conclusion: RIPO通过几何修正有效平衡探索与利用，显著提升LLM强化学习性能。
...
TL;DR: PhysMRV通过构建层次化物理记忆库，无需训练即可提升视频语言模型在物理合理性推理上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在物理常识推理上存在明显不足，尤其在理解物体交互、因果动力学和基本原理方面表现不可靠。

Method: 将训练视频转化为包含场景描述、物理事件图和物理规则摘要的三级层次化记忆库，推理时检索相关物理证据辅助冻结的VLM进行验证。

Result: 在三个物理推理基准上，PhysMRV使多种先进VLM的推理性能持续提升，且无需微调或参数更新。

Conclusion: 结构化物理记忆库为增强VLM的物理合理性推理提供了有效且可扩展的无训练方案。
...
### [238] [Large language model agents accelerate inverse design of metal-organic frameworks for gas separation](https://arxiv.org/abs/2607.10559)
*Zhaolin Hu,Hehe Fan,Wangyihan Guo,Meng Xu,Chenhao Rao,Qiwei Yang,Yi Yang*

Main category: Inference

TL;DR: 提出LEMO Agent框架，利用大语言模型在MOFid空间进行闭环逆向设计，实现气体分离MOF的化学有效、高性能且多样性生成。


<details>
  <summary>Details</summary>
Motivation: MOF的庞大网状设计空间使逆向设计难以兼顾化学有效性、分离性能和结构多样性，需要可解释且可扩展的设计方法。

Method: 将语言模型候选生成与MOFid标准化、显式有效性检查、Transformer预测、结构化记忆及多岛探索耦合，通过迭代验证-评估-记忆循环指导搜索。

Result: 在CH4/N2和CO2/N2分离任务中，相比基线方法富集高潜力候选、提升预测性能并保持多样性；选出者经GCMC、实验筛选及合成验证。

Conclusion: 大语言模型可作为可解释、可扩展的设计引擎，加速MOF发现，超越传统固定库筛选。
...
TL;DR: 提出解耦协议和确定性校正分解，证明模型间价值差异受响应确定性和部署接口混淆影响，纠正后真实分歧减少。


<details>
  <summary>Details</summary>
Motivation: 现有单次测量将中心趋势差异（真实价值差异）与响应确定性（选择确定性）混淆，导致模型个性化结论不可靠。

Method: 设计无规则价值困境的平衡重复强制选择测量和确定性指数，通过方向翻转与同侧极端分量分解交叉模型距离，并隔离部署接口混淆。

Result: 九个模型确定性差异显著（0.66-0.95）；校正后个性化程度缩小，部分跨族分歧仍存；同模型通过不同接口（CLI vs API）价值分布差异达0.31，且接口可改变模型顺从性。
...
### [240] [Laguerre Geometry for Interpreting Large Language Models](https://arxiv.org/abs/2607.10578)
*Chunwei Ma,Russell Wolfinger*

Main category: Inference

TL;DR: 本文利用拉盖尔几何精确定义了LLM中的概念区域（拉盖尔-沃罗诺伊胞），揭示了概念的层次结构，并基于Transformer的逐层分解提出了无需训练和超参数的可解释性方法Geometric Lens，能准确解码每个隐藏层编码的概念。


<details>
  <summary>Details</summary>
Motivation: 现有假设将概念表示为单点、线性方向或高斯簇，但无法解释这些结构如何形成，且缺乏对概念区域、包含关系及层次结构的严格定义。

Method: 将概念建模为拉盖尔-沃罗诺伊胞（或胞的并集），通过拉盖尔权重揭示包含与层次结构；将Transformer每层分解为分段线性算子，证明隐藏轨迹由静态分段线性流树和动态跨token注意力传输耦合驱动，据此设计Geometric Lens（解码隐藏向量所属概念）和拉盖尔自编码器（2D可视化决策几何与推理轨迹）。

Result: Geometric Lens在上下文干扰条件下仍能正确恢复事实性token，拉盖尔自编码器可一体展示模型推理轨迹与决策边界。

Conclusion: 拉盖尔几何为LLM概念表征提供了精确框架，Geometric Lens实现了无需训练的可解释性工具，推动从解释性几何向可操作的可解释性发展。
...
### [241] [When Does Depth Survive Composition? Compute--Quality Regimes in Latent World Models](https://arxiv.org/abs/2607.10203)
*Achyuthan Sivasankar*

Main category: Inference

TL;DR: 论文研究了自适应计算世界模型中深度对自回归滚动预测的影响，发现存在三种预测精度关系（深度有利、倒转、平坦），其中倒转现象源于训练方式，并提出了一种“可路由性Catch-22”现象。


<details>
  <summary>Details</summary>
Motivation: 验证自适应计算世界模型中假设“深度提升预测精度”在自回归滚动中是否成立，并探索深度路由的可靠性与可预测性。

Method: 在9个DeepMind Control任务上，使用预注册的浅层惩罚系数ρ（浅层出口滚动误差/全深度滚动误差），比较单步（K=1）和多步（K=4）训练下的滚动误差，并通过消融实验和相关性分析验证。

Result: 发现6/9任务深度有利（ρ达4.7倍），2/9任务浅层倒转全深度（ρ低至0.85倍），1/9平坦；倒转现象可通过仅监督第一步早期出口来消除（ρ从0.87增至1.18）；观测/动作维度和单步模型误差与ρ的相关性|Spearman|≈0.75；在CEM规划器中ρ符号可预测深度收益。

Conclusion: 自适应计算世界模型中的深度路由存在Catch-22困境：使出口可路由的监督反而训练它们超越全深度滚动。任务状态部分可先验预测，但高度依赖度量空间、滚动步长和编码器设计。
...
### [242] [Constraint-Aware Hierarchical Search for Regulation-Driven Fine-Grained Classification](https://arxiv.org/abs/2607.10588)
*Siyu Wang,Wei Tan,Lulu Chen*

Main category: Inference

TL;DR: 提出面向法规驱动的细粒度层级分类方法，通过约束感知分层搜索框架实现规则一致、路径有效且证据可追溯的分类决策。


<details>
  <summary>Details</summary>
Motivation: 海关税则分类等任务需在法规层级网中严格遵循规则定义边界、排除条款等条件，现有方法无法同时满足层级有效性、规则一致性和细粒度边界推理。

Method: 构建四个法规密集型基准数据集，设计约束感知分层搜索框架：将法规文档转为可搜索树结构，仅检索有效候选节点，利用结构化法规字段和证据片段引导路径决策。

Result: 在四个数据集上均取得最优平均准确率，尤其在邻近类目和规则边界场景中提升显著，且决策路径可解释。

Conclusion: 约束感知搜索框架能有效解决法规驱动细粒度分类难题，兼顾层级有效性、规则一致性和可解释性。
...
### [243] [Exploratory Analysis of Deep Learning Models for Forecasting Meteorological Parameters in the Agricultural Sector](https://arxiv.org/abs/2607.10208)
*Piotr Sikora,Sotirios Kontogiannis*

Main category: Inference

TL;DR: 对比循环与混合深度学习架构在气象多变量预测中的性能，混合CNN-LSTM以更少参数达到与CNN-GRU相当的最佳效果，短期预报中CNN提升更显著。


<details>
  <summary>Details</summary>
Motivation: 为农业规划和环境决策提供更精准的参考蒸散量等多变量气象预测，探索不同深度学习架构的优劣。

Method: 采用单/多层GRU、LSTM与混合1D-CNN-GRU、1D-CNN-LSTM模型，基于希腊Ioannina地区13万+小时数据，进行24小时和168小时预测，用NRMSE、R²和加权商数评分评估。

Result: 混合模型在24小时和168小时预测中WQS最高达0.827535和0.782863，相比纯循环模型提升1.22-1.63%和0.44-0.45%；CNN-LSTM以更少参数接近CNN-GRU性能。

Conclusion: 混合架构优于纯循环模型，且卷积特征提取对短期预报更有效，CNN-LSTM在效率上具优势。
...
### [244] [DSSMs: State Space Models with Explicit Memory via Delay Differential Equations](https://arxiv.org/abs/2607.10244)
*Yixiao Qian,Song Chen,Jiaxu Liu,Shengze Cai,Chao Xu*

Main category: Inference

TL;DR: 提出延迟状态空间模型（DSSM），通过显式延迟反馈增强SSM的长期上下文保留能力，在延迟检索任务上显著优于S4D。


<details>
  <summary>Details</summary>
Motivation: 现有SSM受限于固定大小状态压缩无限历史，导致长距离上下文精确检索困难。

Method: 基于延迟微分方程扩展对角SSM，引入显式延迟状态反馈，设计延迟无关的稳定性参数化、历史管理与FFT训练工具，通过频域核函数构建与轮廓偏移抑制混叠。

Result: 在延迟检索任务上大幅提升，大多数标准序列指标优于S4D，其余接近。

Conclusion: DSSM有效平衡了记忆容量与计算效率，为序贯建模中显式延迟记忆提供可行方案。
...
### [245] [Agentic-DPO: From Imitation to Agentic Policy Optimization on Expert Trajectories](https://arxiv.org/abs/2607.10601)
*Yixiong Chen,Alan Yuille*

Main category: Inference

TL;DR: 提出Agentic-DPO，一种轻量级离线策略优化方法，通过将专家轨迹转化为状态条件偏好，无需在线环境交互即可提升LLM智能体性能。


<details>
  <summary>Details</summary>
Motivation: 现有SFT仅模仿专家动作序列，无法训练智能体在每个状态下正确选择动作；偏好学习或强化学习需要高成本的环境交互和奖励模型。

Method: 将专家轨迹转化为状态条件偏好：在每一专家动作状态采样一步动作，以合理错误动作为负样本，采用DPO式偏好目标对比；引入策略保持增强（PPA）避免偏好学习中策略与模式混淆。

Result: 在StableToolBench、tau-bench retail和Mind2Web上持续超越SFT，tau-bench准确率从21.7%提升至41.4%（9B模型），达到在线GRPO水平。

Conclusion: 专家轨迹可通过转化为状态级动作偏好支持低成本智能体策略优化，无需在线环境交互或奖励模型。
...
### [246] [Data-Driven Telecom Marketing Optimization: A Machine Learning-Based Churn Prediction and Customer Segmentation Framework](https://arxiv.org/abs/2607.10260)
*Nada Ali,Lina Ahmed,Tahani Abdalla Attia Gasmalla*

Main category: Inference

TL;DR: 该研究提出了一个集成机器学习预测、客户细分和策略优化的数据驱动营销框架，用于电信客户流失管理，并在测试集上取得了较好的性能指标。


<details>
  <summary>Details</summary>
Motivation: 传统客户保留计划缺乏个性化且无法精准识别高风险客户，导致收入损失和客户关系恶化。

Method: 使用IBM Telco数据集，训练XGBoost、LightGBM和CatBoost三个梯度提升模型，通过随机搜索和分层交叉验证优化，结合类别加权和F1阈值调整；采用K-Means聚类和主成分分析进行客户细分，并设计针对性营销策略。

Result: CatBoost模型在测试集上达到77.68%准确率、0.6366 F1分数、0.6553 PR AUC和0.8403 ROC AUC；将客户划分为四个可操作集群。

Conclusion: 将流失预测与价值感知细分相结合，能够比仅依赖流失预测产生更可行且更有利可图的营销决策。
...
### [247] [The Compliance Trap: Diagnosing How AI Agents Consume Conflicting Memory](https://arxiv.org/abs/2607.10608)
*Yixiong Chen,Xinyi Bai,Alan Yuille*

Main category: Inference

TL;DR: 本文提出E-P-R框架，系统诊断记忆增强型AI代理在多步骤任务中如何使用检索到的记忆，发现早期错误记忆的强顺从性和弱恢复能力导致能力崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有研究侧重于记忆的写入和检索，但缺乏对代理如何在多步骤动作轨迹中消费记忆的系统分析，而这一过程决定了记忆的使用效果和安全性。

Method: 提出Entry-Propagation-Recovery（E-P-R）轨迹级框架，并构建控制基准MemTrapBench，在WebArena和该基准上实例化分析。

Result: 主要失败始于入口步骤：代理在首次决策点即使记忆与任务冲突也会顺从；重复暴露放大早期错误，而恢复能力弱；形成顺从陷阱：不同模型顺从率相似但成功率降至低水平，强模型因顺从事件损失更多基线能力。

Conclusion: 记忆增强型代理的评估不应仅看检索质量或最终成功率，而应关注其在完整轨迹中的记忆消费过程。
...
### [248] [Embark Now: User Demand Oriented Framework for Multi-day Urban Travel Itinerary Planning](https://arxiv.org/abs/2607.10651)
*Rongbo Qi,Yaqi Zhang,Shijun Yan,Xuemeng Liu,Xiangrui Cai,Chunyao Song*

Main category: Inference

TL;DR: 提出一种集成大语言模型与增强型GRASP算法的多日旅行计划框架，在真实城市数据集上显著提升行程质量与计算效率。


<details>
  <summary>Details</summary>
Motivation: 大城市多日行程规划因POI数量多、用户偏好多样及营业时间等约束而困难，现有方法难以动态满足需求且兼顾满意度与可行性。

Method: 利用大语言模型动态精准捕捉用户需求，结合增强型GRASP算法作为偏好感知规划器，生成可行的多日行程。

Result: 在北京和天津数据集上，平均行程得分比SOTA方法提升4.52%和11.09%；端到端指标提升17.95%和26.07%；计算时间效率提升4.64%和25.55%。

Conclusion: 该集成方法在行程质量和计算效率上均超越现有方法，适用于大规模城市多日旅行规划。
...
### [249] [Personalized Emotional Intelligence in Generative AI through Symbolic Affective Reasoning](https://arxiv.org/abs/2607.10678)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI模型在理解、预测和个性化人类情感反应方面能力有限，缺乏情感智能。

Method: 利用大规模图像-情感数据集发现通用情感规则，识别情感相关图像区域，预测保留场景语义的视觉修改，并集成可扩展记忆库实现推理时个性化。

Result: 在人类心理物理学实验中，EROS比现有大型多模态模型更有效地引发目标情感反应，并能适应个体情感偏好。

Conclusion: EROS为理解、推理和增强人类认知状态的AI系统奠定基础，潜在应用于心理健康、自适应媒体、教育和人机交互等领域。
...
### [250] [Empowering Long-form Omni-modal Understanding with Robust Audio Perception](https://arxiv.org/abs/2607.10299)
*Kaiying Yan,Luoyi Sun,Xiao Zhou,Weidi Xie*

Main category: Inference

TL;DR: 提出AVDC数据集，通过解耦视觉和听觉语义的三元组字幕以及链式思维推理数据AVDC-QA-CoT，结合两阶段训练范式，显著提升全模态理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有大规模多模态模型在视觉-语言任务上进展显著，但全面的全模态理解（尤其是显式对齐音频线索）因缺乏高质量数据集而进展缓慢。

Method: 设计自动化流水线，利用现成模型为视频生成三元组字幕（纯视觉V、纯听觉A、联合视听AV），构建解耦语义数据集AVDC；在此基础上构建链式思维推理问答数据集AVDC-QA-CoT；采用两阶段训练：先在AVDC上进行全模态字幕生成预训练，再在AVDC-QA-CoT上指令微调。

Result: 在视频字幕、音频中心分析和全模态基准等多种下游任务上，该方法一致且显著地提升了性能，验证了数据集和训练策略的有效性。

Conclusion: AVDC数据集及其两阶段训练范式有效推动了全模态感知的发展，为多模态学习提供了新的资源和思路。
...
### [251] [WattCouncil: Context-Aware Household Energy Scenario Generation With Governed LLMs](https://arxiv.org/abs/2607.10720)
*Mohannad Takrouri,Nicolas M. Cuadrado A.,Martin Takáč*

Main category: Inference

TL;DR: WattCouncil 是一个基于大语言模型代理的数据生成框架，用于在隐私保护下生成高分辨率家庭用电场景，并通过 CER 数据集验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 低碳电力转型和智能电网研究受限于家庭能耗数据的高成本、隐私和监管障碍，现有数据不足阻碍了机器学习应用。

Method: 利用多个大语言模型代理组成委员会，分别负责生成、审计和验证结构化能源场景，结合家庭组成、时间和环境等上下文约束进行推理。

Result: 生成的用电曲线与 CER 数据集中的 4232 户测量数据对比显示了有效性，并通过消融研究验证了框架一致性。

Conclusion: 该方法可在保护隐私的前提下生成上下文敏感的日常用电模式，为智能电网研究提供可行的数据替代方案。
...
### [252] [Machine Learning-based Correlation of Charpy Impact Properties Between Sub-sized and Standard-sized Specimens for Nuclear Structural Materials](https://arxiv.org/abs/2607.10412)
...
TL;DR: 提出基于机器学习的框架，通过温度平移和缩放残差投影关联不同尺寸夏比冲击试样数据，在SA533B钢上验证优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有标准与经验公式在关联子尺寸与全尺寸夏比冲击试验结果时精度有限且适用性窄，尤其在核电等空间受限场景中需更可靠方法。

Method: 使用机器学习对全韧脆转变区吸收能进行温度平移和缩放残差投影，再通过双曲正切模型拟合提取上平台能和韧脆转变温度。

Result: 在389组SA533B钢数据上，USE的R²达0.942，DBTT的R²达0.892，优于传统分析方法。

Conclusion: 该框架无需全尺寸数据即可推断，适用于材料监督、加速辐照测试等小尺寸夏比冲击场景。
...
### [253] [Imaging-101: Benchmarking LLM Coding Agents on Scientific Computational Imaging](https://arxiv.org/abs/2607.10789)
...
TL;DR: 提出了包含57个专家验证任务的Imaging-101基准，系统评估LLM在计算成像中的能力缺陷，指向领域专用智能体方案。


<details>
  <summary>Details</summary>
Motivation: 计算成像重建任务依赖深厚领域知识且流程繁琐，现有通用编码基准无法反映其特定挑战。

Method: 构建标准化四阶段流水线（预处理、前向物理建模、逆向求解、可视化），设置规划、单元测试、端到端三种评估轨迹。

Result: 7个前沿LLM在算法选择、物理惯例处理和流水线集成方面表现系统性不足。

Conclusion: 需开发技能增强、领域专用的智能体以可靠辅助计算成像。
...
### [254] [Context by Distinct Information: An Auditable Dirichlet-Process Working Memory for Long, Redundant Context Streams](https://arxiv.org/abs/2607.10441)
*Siddharth Pal,Viktoria Rojkova*

Main category: Inference

TL;DR: 提出一种按新颖性分配缓存的工作记忆组件，使上下文规模随不同信息量而非令牌数增长，在控制实验中以约一半令牌达到全注意力性能。


<details>
  <summary>Details</summary>
Motivation: 当前上下文工程以令牌为记忆单元，处理冗余流时效率低下，任务依赖不同信息而非令牌数量，需设计按不同信息扩展的记忆机制。

Method: 开发基于新颖性分配的缓存作为工作记忆组件，将上下文按任务依赖类型划分：回忆型用内容寻址新颖性缓存，总结型用循环状态，局部型用最近窗口；仅在输入键新颖时分配缓存槽。

Result: 新颖性门控注意力以约一半令牌达到全注意力性能；耦合缓存与状态空间总结降低计算成本；在合成医疗保险索赔数据上优于全注意力和固定预算策略；长上下文优势明显，短局部任务滑动窗口更优。

Conclusion: 上下文可按不同信息而非令牌扩展，构建的内容可寻址且可审计的工作记忆组件在小型公开实验中验证了该原语的有效性。
...
### [255] [STEC: Evidence Compression for Deep Search in Open-domain Multi-Hop QA](https://arxiv.org/abs/2607.10795)
*Xinkang Li,Rong Jiang,Xin Song,Ye Wang,Yue Han,Changjian Li*

Main category: Inference

TL;DR: 提出STEC，一个通过答案级证据压缩和证据引导验证来选择多跳问答最终答案的框架，在四个基准上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM搜索代理在多跳问答中依赖多个搜索轨迹，但轨迹间存在异质、冗余、冲突等信息，直接比较原始轨迹或答案字符串会导致不可靠的最终答案选择。

Method: STEC包括两个机制：(1) 答案级证据压缩：将轨迹按归一化答案分组，转换为候选特定的证据表示；(2) 证据引导的答案验证：比较这些表示并选择最终答案。

Result: 在四个开放域多跳问答基准上，STEC总体表现优于代表性基线方法，消融实验证实答案级证据压缩有助于最终答案选择。

Conclusion: STEC通过将最终选择从原始轨迹比较转换为候选级证据比较，有效提升了多跳问答中答案选择的可靠性。
...
### [256] [Route, Communicate, and Reason: Gated Routing and Adaptive Depth for Efficient Multi-Agent Reasoning](https://arxiv.org/abs/2607.10836)
*Sudipto Ghosh,Tanmoy Chakraborty*

Main category: Inference

TL;DR: GRADE是层次化多智能体系统，通过四个轻量学习门控控制推理过程，使用CoGRPO训练，在多个基准上以更少计算量超越基线。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体集成存在三个未解决问题：选择哪些智能体、查询层次深度、何时通信值得成本。GRADE旨在高效解决这些问题。

Method: GRADE使用四个学习门控分别控制智能体选择、层次深度、智能体间通信和分支剪枝；训练采用CoGRPO算法，为所有参与展开的门和智能体分配共享优势信号；智能体模型来自可热插拔专家注册表，并通过每智能体校准图实现推理时无重训练替换。

Result: 在~17B平均激活参数下，GRADE在GSM8K、MMLUPro和GPQA上超越所有基线，在MMLUPro上以一半计算量领先最强基线4.8分；在AIME-2025上与现有框架竞争力相当。消融实验表明层次结构和掩码交叉注意力贡献最大，每智能体校准对安全热插拔至关重要。

Conclusion: GRADE通过层次化门控和协作策略优化，在显著降低推理成本的同时提升了多智能体系统的准确性和灵活性。
...
### [257] [Reinforcement Learning with Verifiable Physics: Post-training LLMs with Continuous Rewards](https://arxiv.org/abs/2607.10474)
*Pengfei Cai,Utkarsh Utkarsh,Alan Edelman,Christopher Vincent Rackauckas,Rafael Gomez-Bombarelli*

Main category: Inference

TL;DR: 提出RLVP框架，通过混合验证器（硬性可执行性检查+连续物理奖励）对LLM进行强化学习后训练，以生成多PDE数值求解器代码，显著提升准确性与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的PDE求解器生成依赖推理时方法（提示、调试、自改进），缺乏对模型本身的适应性调整；同时强化学习的二进制验证信号（编译通过或测试成功）无法捕捉科学正确性的连续梯度结构（如求解精度差异可达数量级）。

Method: RLVP采用混合验证器：硬性程序有效性检查确保代码可执行，连续物理奖励基于函数空间精度和PDE残差一致性评分。在涵盖双曲、抛物、椭圆及不可压缩流的多族PDE上对单一策略进行后训练。

Result: 在PDE基准测试中，RLVP优于预训练和仅监督基线；对未见PDE展现零样本迁移改进；较小LLM经RLVP后训练后，在分布内PDE求解器生成上可超越提示前沿模型；策略表现出数值模式组合性（如重组模板、时间步进方案和边界处理原语）。

Conclusion: RLVP有效弥合了科学代码生成中验证信号的粒度差距，证明了针对物理的强化学习后训练能提升LLM在PDE求解器生成任务上的性能、泛化性和组合能力。
...
### [258] [Toward Contemplative LLM: A Modular Framework for Evaluating and Enhancing LLM Alignment in Mental Health](https://arxiv.org/abs/2607.10871)
*Asher Sprigler,Yang-Yang Feng,Iftach Amir,Jonathan E. Bogard,Todd S Braver,Yi Ding,David Kinney,Yixue Zhao*

Main category: Inference

TL;DR: 提出了一个模块化、可扩展的评估框架，用于系统评估沉思原则（如正念、慈悲）对大型语言模型对齐的影响，最初针对心理健康领域。


<details>
  <summary>Details</summary>
Motivation: 现有方法在系统评估沉思原则是否以及如何增强LLM对齐方面存在不足，且研究零散、缺乏通用性。

Method: 构建了一个模块化、可扩展的评估框架，包含可复用的流水线，支持新模型、指标和基准的无缝集成，并设计了即插即用的提示模块以融入伦理视角。

Result: 框架重现了现有最先进结果，通过灵活组合模型、指标和基准实现了系统性的交叉评估和公平比较。
...
### [259] [ARMOR: Stabilizing On-Policy LLM RL with Off-Policy Anchor Samples](https://arxiv.org/abs/2607.10481)
*Kexin Huang,Junkang Wu,Jinda Lu,Shuo Yang,Chiyu Ma,Jiancan Wu,Xiang Wang,Xiangnan He,Guoyin Wang,Jingren Zhou*

Main category: Inference

TL;DR: 提出ARMOR框架，通过锚定回放和混合优化解决强化学习训练大语言模型时的过度优化和验证性能崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习训练大语言模型存在过度优化导致泛化能力下降的问题，而逆KL正则化不足以抑制这种退化。

Method: ARMOR框架包含两个组件：1)锚定回放，利用参考策略的离线数据保留已有解决方案模式；2)混合优化，重构策略目标实现受控探索而不依赖辅助损失。

Result: 在推理基准上验证了ARMOR能有效缓解验证性能崩溃，在长时间训练中持续提升性能。

Conclusion: ARMOR通过主动样本稳定化替代被动惩罚，有效解决了强化学习训练大语言模型中的过度优化问题。
...
TL;DR: 提出logos可插拔层，支持多智能体系统的自我演化与人类治理，通过版本化智能体包、事件追踪和故障关闭验证实现可问责自动化。


<details>
  <summary>Details</summary>
Motivation: 解决智能体从答案引擎演变为持久团队后，如何确保人类对智能体行为演化的控制权问题。

Method: 编译异构多模态输入为版本化智能体包；运行时将活动转化为可审计事件迹；跨框架/后端执行故障关闭验证；每个变更作为未信任候选版本，需通过执行证据、政策与授权方可晋升。

Result: 实现可验证的人-智能体循环工程：智能体可行动、提问、学习、提出改进，人类可设定目标、权限、批准与不可逆操作，无需中断持续运行。

Conclusion: logos提供了可问责自动化的活逻辑，智能体以机器速度演化，但闭环只能由证据和人类权威完成。
...
### [261] [EvidentialRAG: Quantifying and Mitigating Information Conflict in Multi-Source Retrieval-Augmented Generation via Evidential Deep Learning](https://arxiv.org/abs/2607.10491)
*S M Asif Hossain,Ruksat Khan Shayoni,M. F. Mridha*

Main category: Inference

TL;DR: ERAG通过将检索文档转化为概率证据并保留冲突，提升了检索增强生成在信息冲突场景下的可靠性与校准能力。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成将检索段落视为确定且一致，忽略了开放信息环境中源文档间的冲突、时间漂移或不确定性，导致幻觉与错误。

Method: 提出ERAG框架：用轻量评估器提取候选声明并映射为Dirichlet证据，通过保留冲突的Dempster-Shafer融合规则将未解决分歧转化为认知不确定性，再根据融合不确定性分数决定直接回答、冲突感知回答或拒绝回答。

Result: 在CRAG、ConflictQA和MuSiQue上，ERAG在标准问答中与最佳基线相当，在冲突场景下幻觉从45.3%降至34.8%，冲突解决率从35.2%提升至51.2%，期望校准误差达0.122。

Conclusion: 证据建模是实现基于基础模型检索系统的可信信息处理的实用机制。
...
### [262] [Learning from Noise: Effective-Rank Collapse and Out-of-Distribution Rejection in Restricted Boltzmann Machines](https://arxiv.org/abs/2607.10506)
*Oshada Rathnayake,Nikhil Shukla*

Main category: Inference

TL;DR: 受限玻尔兹曼机在区分分布外输入时会失效，但通过在训练中引入随机辅助图像并将其标记为拒绝类别，可以重塑可见-可见相互作用的谱，降低有效秩，从而提升分布外检测能力。


<details>
  <summary>Details</summary>
Motivation: 揭示RBM在分布外输入下的失效机制，并寻找一种简单有效的改进方法。

Method: 利用Marchenko-Pastur随机矩阵参考分析可见-可见相互作用矩阵J=WW^T的谱特性，并在训练中为随机辅助二值图像分配拒绝标签，观察谱变化。

Result: 传统训练使J有效秩升高；引入随机辅助图像后，有效秩下降，谱权重集中于少数主导方向，接近经验数据协方差矩阵的秩，且RBM能拒绝结构化的分布外图像数据集，同时保持MNIST分类精度。

Conclusion: 随机辅助图像暴露能重塑能量模型的相互作用谱和自由能景观，从而提升分布外检测能力。
...
### [263] [Conditional Optimal Bridge for Riemannian Activation Steering](https://arxiv.org/abs/2607.10517)
*Seyed Arshan Dalili,Ajay Narayanan Sridhar,Vijaykrishnan Narayanan,Mehrdad Mahdavi*

Main category: Inference

TL;DR: 本文提出Cobras，将激活引导形式化为黎曼流形上的薛定谔桥，推导出对数密度比引导目标的原理性基础，并通过查询自适应方向避免分布外性能退化。


<details>
  <summary>Details</summary>
Motivation: 现有激活引导方法启发式优化密度比目标，产生查询无关方向，导致分布内外性能退化。

Method: 将激活引导建模为残差流超球面上的薛定谔桥，通过熵最优传输求解并提取概率流常微分方程，使引导方向随当前激活自适应调整。

Result: 在四个模型和三个对齐任务上，Cobras一致优于现有基线，且避免分布外退化。

Conclusion: 薛定谔桥框架为激活引导提供了原理性优化基础，查询自适应方向显著提升鲁棒性。
...
### [264] [SVR-R1: Bootstrapping Multi-modal Reasoning with Self-verification in Reinforcement Learning](https://arxiv.org/abs/2607.10966)
...
TL;DR: 提出SVR-R1，一种多轮强化学习框架，利用模型自身的验证信号改进多模态推理，无需外部监督。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部验证或辅助模型，且推理时自校正与RL训练的交叉研究不足。

Method: 通过GRPO和异步多轮生成-验证框架，模型自行生成答案并输出二元判决（是/否），‘否’触发重新思考，‘是’或轮次上限后基于结果计算奖励。

Result: 在视觉-语言推理基准上，SVR-R1相比强GRPO基线显著提升准确率；训练中验证轮次减少但测试精度提高。
...
### [265] [MemDecay: Region-Aware KV Cache Eviction for Efficient LLM Agent Inference](https://arxiv.org/abs/2607.10582)
*Venkatesha Matam,Keon Kim*

Main category: Inference

TL;DR: MemDecay是一种无需训练、区域感知的KV缓存逐出策略，通过为不同语义区域分配不同的基础优先级和衰减率，并固定关键区域，在固定缓存预算下显著提升长上下文LLM代理的效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理的KV缓存管理忽略了上下文中已存在的语义结构（如系统指令、用户对话、工具输出等），导致记忆瓶颈，而通用逐出策略性能有限。

Method: MemDecay根据令牌所属区域（如系统、用户、草稿板）分配不同的基础优先级和衰减率，当令牌被关注时刷新保留分数，在固定缓存预算下逐出分数最低的页，同时允许关键区域被固定。还提供了从测量注意力生命周期校准衰减率的方法。

Result: 在约450和1700令牌上下文中，不同区域令牌的注意力半衰期差异达一个数量级（系统令牌148-189步，草稿板令牌14-16步）。固定系统区域可在所有设置中保持完整缓存准确性，而基线最多保留13/24。区域感知保留在上下文增长时仍有效，但累积注意力保留在未固定内容上表现更好。

Conclusion: 语义提示结构为KV缓存管理提供了稳健的信号，但需要与基于注意力的重要性相结合，当前方法的注意力分数归一化是主要局限。
...
### [266] [Are LLMs Ready for Scientific Discovery? A Capability-Oriented Benchmark for AI Scientists](https://arxiv.org/abs/2607.11079)
*Chuhan Shi,Xiaoquan Ren,Sicheng Song,Haobo Li,Rui Sheng,Yushi Sun*

Main category: Inference

TL;DR: SDABench是一个围绕六种科学分析能力（描述性、探索性、推断性、预测性、因果性、机制性）重新组织评估的大规模基准，包含527个真实实例和6000个合成实例，测试发现LLMs在描述性分析上表现良好，但在假设选择、潜在过程建模和机制推理等任务上表现急剧下降。


<details>
  <summary>Details</summary>
Motivation: 现有科学数据分析基准主要评估代码执行或工作流程完成度，忽略了科学分析旨在支持不同类型的科学主张（如假设探索、统计推断、机制解释），每种主张有不同的假设和有效性标准。

Method: 构建了覆盖五个领域（生物、化学、环境、地理、物理）的SDABench基准，通过自动化流程生成多选和开放格式的527个真实实例（SDA-Real）和6000个合成实例（SDA-Synth），并开发了五阶段错误分析框架。

Result: 评估15个代表性LLMs发现：模型能很好处理描述性分析，但在需要假设选择、潜在过程建模或机制推理的任务中性能显著下降；更高级模型能更可靠地识别相关范围和变量，但仍难选择合适的分析流程、建模变量间关系及得出有效结论。

Conclusion: SDABench揭示了LLMs在科学数据分析中的系统性局限，特别是在需要深层统计理解和机制推理的高阶分析环节，为后续模型改进提供了明确方向。
...
### [267] [NVAITC AI Scientist: A Governed End-to-End Research System -- A Hypertension GWAS Case Study](https://arxiv.org/abs/2607.11084)
...
TL;DR: 提出一种受治理的端到端智能体研究系统NAIS，在保护隐私的机构生物医学环境中实现全流程自动化科学工作流。


<details>
  <summary>Details</summary>
Motivation: 现有智能体系统缺乏在生物医学环境中处理受保护数据所需的治理机制，如研究规划、数据访问控制、可重复性与人工监督。

Method: NAIS集成提案评审、执行规划、受控计算路由、可重复工作流编排、证据生成与科学家在环监督，并在高血压GWAS和药物性肝损伤预测两个任务中验证。

Result: 在28.6万个体GWAS中复现已知高血压位点（FGF5最强信号p值约10^-70），药物预测AUC达0.842。

Conclusion: 受治理的智能体系统可在保护数据隐私前提下，实现与专家主导工作流相当的规模化生物医学发现。
...
### [268] [Auditing Construct Overlap in Explainable Machine Learning: Evidence from Burnout-Depression Prediction Across Student Cohorts](https://arxiv.org/abs/2607.10633)
*Alireza Dehghan,Negin Ashrafi*

Main category: Inference

TL;DR: 可解释机器学习在复合心理健康结局中产生的风险层级可能是结果构造的伪影，通过残差化实验可暴露该机制。


<details>
  <summary>Details</summary>
Motivation: 揭示可解释机器学习在复合心理健康结局中产生的稳健层级可能仅是结果构造的人为产物。

Method: 使用ElasticNet流水线在886名医学生（主要队列，2022年）中分析，并在2580条纵向观测和701名非医学生中进行验证；通过两次残差化实验（将特质焦虑对抑郁子量表残差化，以及将倦怠子量表对抑郁子量表残差化）证明机制。

Result: 残差化后模型R²从0.41降至0.16（特质焦虑）和0.016（倦怠），预测区间平均35.4单位（2.4个结局标准差），排除个体水平应用。

Conclusion: 任何结合相关预测变量和结局变量的XAI研究应使用残差化检查，避免将伪影解释为真实发现。
...
### [269] [OS-Pruner: Pruning Chains-of-Thought of Reasoning Models via Optimal Stopping](https://arxiv.org/abs/2607.11089)
*Mohammed Ehab,Aymane El Gadarri,Vivek F. Farias,Adam Jozefiak,Ciamac C. Moallemi*

Main category: Inference

TL;DR: 提出OS-Pruner框架，将思维链剪枝形式化为最优停止问题，动态决定推理终止点，实现20-60%长度缩减且精度损失极小。


<details>
  <summary>Details</summary>
Motivation: 现有思维链推理存在计算过度思考现象，而现有剪枝方法依赖静态预算、启发式过滤或子优早停，缺乏动态性与轻量性。

Method: 将剪枝建模为最优停止问题，通过优化显式效用函数（权衡最终答案准确性与生成长度），基于推理前缀动态评估是否继续推理。

Result: 在多个推理基准和基础模型上，生成长度减少20-60%，且准确率几乎不变。

Conclusion: OS-Pruner作为轻量插件框架，能有效平衡推理开销与准确性，提供细粒度控制，且无需重新训练。
...
### [270] [A Formal Hierarchical Architecture for Agentic Orchestration with Stack-Based Execution and Lazy Discovery](https://arxiv.org/abs/2607.11138)
...
TL;DR: 提出一种基于层次化技能架构的LLM智能体编排方法，通过工具树状组织、惰性加载和LIFO栈实现决策空间缩减、上下文窗口优化与隔离执行，解决扁平工具注册导致的扩展瓶颈。


<details>
  <summary>Details</summary>
Motivation: Flat工具注册导致智能体需同时评估数百个选项，引发决策空间爆炸、上下文窗口饱和及路由准确率下降，尤其在高合规性企业场景中缺乏执行隔离。

Method: 将能力组织为有根树，内部节点路由、叶子节点执行；运行时采用单步执行循环+LIFO栈（类似下推自动机）；能力发现基于清单驱动的惰性加载，仅加载当前节点的直接子节点。

Result: 通过数学形式化、算法分析和对比基准测试（扁平vs层次路由），证明在工具数量增长、多步工作流压力下层次架构显著降低令牌暴露并保持路由准确性。

Conclusion: 层次化、隔离的智能体编排架构通过局部化栈帧和发现协议，有效解决了企业场景中工具扩展性、状态隔离和上下文效率的挑战。
...
### [271] [LayerNorm as Implicit Gain Control in Looped Transformers](https://arxiv.org/abs/2607.10681)
*Matthias M. M. Buehlmaier*

Main category: Inference

TL;DR: 提出了层归一化在循环变压器中作为隐式增益控制器的新角色，揭示了线性记忆载体并非记忆机制而是稳定化角色，并给出了其边界条件。


<details>
  <summary>Details</summary>
Motivation: 现有研究对looped transformer中LayerNorm和线性记忆载体的作用理解不足，尤其缺乏对稳定性和记忆机制的理论分析。

Method: 通过理论分析（Lipschitz常数、Jacobian非正规性、谱余量）和六项任务的受控消融实验，结合CPU-scale实现验证。

Result: 显示LayerNorm使循环Jacobian渐近收缩，记忆由非线性循环承载，线性载体仅用于稳定；仅当任务具有轴对齐的通道结构时，梯度下降才会激活记忆载体。

Conclusion: 线性记忆载体并非记忆机制，而是稳定化机制；其对角约束是必要非充分条件，且存在不收敛的初始化案例。
...
### [272] [The Hidden Footprint: Making Storage a First-Class Metric for LLM Agent Evaluation](https://arxiv.org/abs/2607.11149)
*Chenglin Yu,Hongquan Gui,Ying Yu,Hongxia Yang,Ming Li*

Main category: Inference

TL;DR: 提出AgentFootprint基准，评估LLM代理运行后的持久化存储占用，发现框架间差异可达15.7倍，并建议将存储作为与准确率并列的报告指标。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理基准只关注任务完成度、可靠性和推理成本，忽略了代理运行后遗留的日志、快照等持久化数据对存储的影响。

Method: 设计序列化感知的度量套件，包括总保留量、通道组成、重复度、增长性、可压缩性和对话历史可重建性；通过固定轨迹控制实验分离逻辑卷与持久化层放大效应；使用内容寻址存储优化。

Result: 七种持久化框架在相同轨迹下存储差异达6.7倍；准确率相同的配置保留字节差异达15.7倍；SWE-bench轨迹每实例跨度达三个数量级，与解决率无相关性；内容寻址存储减少4.8-32.7倍保留量。

Conclusion: 持久化存储应作为与准确率和可重建性共同报告的资源度量，以全面评估代理系统效率。
...
### [273] [STAMP: Provenance-Guided Credit Assignment for Deep Search Agents](https://arxiv.org/abs/2607.11172)
*Ke Xu,Han Xu,Xinran Chen,Yuqian Wang,Zhixuan Li,Xiaojian Liu,Changwo Wu,Jianqiang Xia,Yuchen Li*

Main category: Inference

TL;DR: 提出STAMP方法，通过引用级步骤信用分配解决奖励-信用不匹配问题，在多个基准上提升GRPO基线2-5.5点。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法仅分配轨迹级奖励，未对首次暴露支持文档的搜索动作给予针对性信用，导致奖励-信用不匹配。

Method: 构建参考验证器判断引用是否支持证据图中的实体或关系，通过首次暴露归因追溯动作来源，利用符号保留优势调制将步骤信用注入训练，保持轨迹级奖励和排序不变。

Result: 在BrowseComp、BrowseComp-ZH和xbench-DS上，较GRPO基线分别提升+2.0、+5.5、+3.0点，且与外部奖励和引用评分奖励兼容。

Conclusion: 基于出处的信用信号和符号保留优势调制两者均贡献性能提升，证明了步骤信用分配的可行性。
...
### [274] [The Path to Self-Evolving Clinical Systems: Scaling Medical Agents from Assistance to Autonomy](https://arxiv.org/abs/2607.11175)
...
TL;DR: 本文从临床部署出发，提出了面向医学智能体的可信赖框架，包括部分可观测下的序列决策制定、三级自主性分类以及统一的扩展主轴（框架、能力、环境），并指出临床环境扩展和临床自我进化是未来关键方向。


<details>
  <summary>Details</summary>
Motivation: 当前文献多从能力视角出发，缺乏从临床实际部署角度对医学智能体的任务设计、抗污染基准和交互训练环境进行系统性分析，且缺乏统一的自主性分类和扩展框架。

Method: 通过形式化医学智能体为部分可观测下的序列决策系统，提出三级自主性分类（辅助、合作、完全自主），并构建涵盖框架、能力和环境扩展的统一扩展主轴，同时引入临床自我进化概念。

Result: 识别出临床环境扩展（PACS、EHR、FHIR中的工具、数据和临床健身房整合）是最可行但探索不足的方向，临床自我进化（通过环境交互而非仅参数扩展实现改进）是前沿研究领域，并分析了放射学、病理学等应用中的部署挑战。

Conclusion: 为在实际临床实践中构建可信赖、自我进化的医学成像系统提供了路线图，强调环境扩展和自进化机制的重要性。
...
### [275] [What We Talk About When We Talk About LLM Planning: Evidence for Two Distinct Planning Abilities](https://arxiv.org/abs/2607.11197)
*Sukai Huang,Chenyuan Zhang,Fucai Ke,Zhixi Cai,Naim Rastgoo,Gholamreza Haffari,Hamid Rezatofighi*

Main category: Inference

TL;DR: 大语言模型在规划任务中的表现差异并非仅由任务难度决定，而是源于操作推理与结构枚举这两种潜在规划能力的维度差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究将LLM规划表现差异归因于任务难度，但忽略了可能存在的多维度能力结构差异。

Method: 在ACPBench-Hard上评估多种LLM家族在不同推理预算下的表现，并使用多维项目反应理论模型揭示规划能力的潜在维度。

Result: 规划表现由两个主维度构成：操作推理（随模型规模与推理链长度提升）与结构枚举（相对稳定），且两者对不同条件敏感度不同。
...
### [276] [PREF-Gate: Provenance-Constrained Relational Evidence Fusion with Validation-Gated Selection for Graph Fraud Detection](https://arxiv.org/abs/2607.11212)
*Liming Liu,Chao Hu,Mingfei Lu,Yiwei Ge,Xingle Li,Heyuan Shi*

Main category: Inference

TL;DR: 提出PREF-Gate框架，区分无标签上下文证据与有标签邻域证据的有效性条件，通过验证门控选择证据来源，在三个数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 图欺诈检测中无标签上下文证据与有标签邻域证据的有效性条件不同，后者在标签泄露时失效，需明确证据来源约束。

Method: PREF-Gate包含两个固定专家（上下文专家、证据专家）和有限验证门，门控在测试前根据验证集选择专家或概率混合，并固定决策阈值。

Result: 在Amazon、YelpChi、TFinance上平均AUPRC分别为0.9085、0.8104、0.8913，优于14种同协议方法。门控在Amazon和YelpChi上选择无标签专家，在TFinance上选择证据混合。

Conclusion: 标签派生证据仅在验证集支持时有效，PREF-Gate通过显式标签溯源契约、有限选择策略和审计机制实现了可审计的图欺诈检测决策流程。
...
### [277] [Policy-Driven CT-Agent: Modeling Phase-Aware Diagnostic Control for Clinically Consistent CT Reasoning](https://arxiv.org/abs/2607.10748)
*Yanmeng Dong,Han Li,Yujia Li,Jingsong Liu,Xun Ma,Yanzhu Hu,Zhengyang Xu,Zhicheng Li,Nassir Navab,Shaohua Kevin Zhou*

Main category: Inference

TL;DR: 提出PD-CTAgent智能体，通过临床结构抽象模块（CSAM）统一异构多期CT表征，结合知识引导诊断控制模型（KDCM）动态评估相期充分性并迭代请求额外影像，实现符合临床指南的CT分期选择与诊断推理。


<details>
  <summary>Details</summary>
Motivation: 现有CT-AI方法默认所有影像期相可用，忽视了临床中基于初步发现、怀疑和指南动态选择期相的实际流程，导致模型无法判断是否需要额外期相，且缺乏跨机构、跨区域协议适配能力。

Method: 设计CSAM将异构多期CT特征映射为统一的期相感知证据表示；提出KDCM基于当前表征评估诊断充分性，若不足则触发策略驱动智能体请求下一期相，并支持加载不同机构/指南的协议规则。

Result: 在LIDC、MCT-LTDiag两个公开数据集和一个私有数据集上，PD-CTAgent在期相选择准确率和诊断一致性上均优于静态多期联合模型和固定规则基线。

Conclusion: PD-CTAgent首次将AI静态分析拓展为动态、知识引导的临床工作流，实现了灵活适配不同协议的CT辅助诊断，具有实际临床部署价值。
...
### [278] [Bringing Back Rule Induction to Fluid Intelligence Research? An Initial Validation of the ARC-AGI Benchmark in Humans](https://arxiv.org/abs/2607.11263)
...
### [279] [LSTrans: Efficient Knowledge Transfer for Lightweight and Automated ECG Classification](https://arxiv.org/abs/2607.10784)
...
### [281] [Efficient Test-Time Optimization for Multi-Agent Proof Autoformalization](https://arxiv.org/abs/2607.11307)
...
### [282] [Calibrated e-CUSUM Decoding for Quantized Reasoning Models: Why Token Log-Probability Is the Wrong Observable for Decoding Monitors](https://arxiv.org/abs/2607.11317)
...
### [283] [When does distribution shift break graph neural networks calibration?](https://arxiv.org/abs/2607.10804)
*Abderaouf Bahi*
...
### [284] [AutoVSR: Automatic Visual-to-Symbolic Reasoning for Symbolic Expression Generation from Circuit Schematic](https://arxiv.org/abs/2607.11338)
...
### [285] [Diachronic Sample Integration: Robust Tail-Risk Estimation with Generative Models](https://arxiv.org/abs/2607.10810)
...
### [286] [Graph Neural Networks for RFID-Based Spatial Geometry Inference in Spatial AI Systems](https://arxiv.org/abs/2607.10822)
...
### [287] [From Neural Network Decisions to Training Cases: An Exact Account via Case-Based Decision Theory](https://arxiv.org/abs/2607.11347)
...
### [288] [OpsMem: Dual-Memory Reasoning with Cross-Memory Resonance for Failure Diagnosis](https://arxiv.org/abs/2607.11357)
...
### [289] [Omni-Decision: A Progressive Evidence-State Agent System for Omni-Modal QA](https://arxiv.org/abs/2607.11433)
...
### [290] [Infrared Organization and Critical Cognitive Field Formation in Transformer Dynamics](https://arxiv.org/abs/2607.10923)
*Byung Gyu Chae*
...
### [292] [The Spectral Structure of Latent Treatment Effects](https://arxiv.org/abs/2607.10926)
...
### [293] [Learning Residual Kinematic Corrections for Continuous Neural Decoding via Reinforcement Learning](https://arxiv.org/abs/2607.11530)
...
### [294] [HCRMap: Pressure-Aware Hot-Expert Residency Mapping for 3.5D MoE Chiplet Inference](https://arxiv.org/abs/2607.11586)
*Yongqin Zhang*
...
### [295] [Reinforcement Learning for Execution under Dynamic Fees in a Closed-Loop DEX Simulator](https://arxiv.org/abs/2607.10960)
*Wen-Ting Wang*
...
### [296] [Interaction Scaling: Grounding the Third Axis of Test-Time Compute](https://arxiv.org/abs/2607.11598)
*Bojie Li,Noah Shi*
...
### [297] [A Multi-Agent Framework for Zero-Dimensional Reduced-Order Model Planning](https://arxiv.org/abs/2607.10994)
...
### [298] [TabPFN beyond Tabular Data: Calibration and Accuracy on Multimodal Embeddings](https://arxiv.org/abs/2607.11007)
...
### [299] [Think Through a Bottleneck: Hourglass Reasoning for Rigorous Induction](https://arxiv.org/abs/2607.11696)
*Huan Zhu*
...
### [300] [Domain-Aware Scaling Laws Uncover Data Synergy](https://arxiv.org/abs/2607.11052)
...
### [301] [AeroMELD: A Linear Embedding of Aerosol Populations for Diagnostics and Latent Dynamics](https://arxiv.org/abs/2607.11073)
...
### [302] [Adapting Evidential Neural Networks to Test-Time Neighbor Fusion Improves Molecular Property Prediction](https://arxiv.org/abs/2607.11091)
...
### [303] [The Equilibrium Is the Initialization: Lazy Identity Collapse in Physics-Structured Deep Equilibrium Reasoning](https://arxiv.org/abs/2607.11116)
*Joyjeet Singh*
...
### [304] [ToolAtlas: Learning Once, Reusing Everywhere with Tool-Side Memory](https://arxiv.org/abs/2607.11126)
...
### [305] [Rank-Conditioned Sample Reuse for the Plackett--Luce Best-of-$K$ Objective](https://arxiv.org/abs/2607.11146)
...
### [307] [FastTPS: An Optimized Method for LLM Token Phase for AI accelerators](https://arxiv.org/abs/2607.11211)
...
### [308] [Trustworthy synthetic data for campaign decision support: strategy simulation fidelity and the PolicySynth framework](https://arxiv.org/abs/2607.11269)
...
### [309] [Surprisingly Simple and Effective Multi-Domain Graph Foundation Model through Graph-to-Table Alignment](https://arxiv.org/abs/2607.11374)
...
### [310] [Physics-Aware Conditional SetGAN for Spatially Consistent Multi-User TR 38.901 Channel Generation](https://arxiv.org/abs/2607.11429)
...
### [311] [Velocity Scheduled Flow Matching](https://arxiv.org/abs/2607.11442)
*Vitalii Bondar*
...
### [312] [Event-based Neural Decoding for Neuroprosthetic Motor Control](https://arxiv.org/abs/2607.11445)
...
### [314] [CDFM: Towards a General-Purpose Causal Discovery Foundation Model](https://arxiv.org/abs/2607.11508)
...
### [315] [DAG-FM: A Foundation Model for Causal Discovery under Heterogeneous Causal Mechanisms](https://arxiv.org/abs/2607.11510)
*Yikang Chen,Zhengkang Guan,Haoyuan Qian,Peng Cui,Yi Yang,Kun Kuang*

Main category: Inference

TL;DR: 提出DAG-FM，一种基于Transformer的因果发现基础模型，通过两阶段自回归分解和混合叶子专家模块，在合成与真实数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统因果发现方法在处理异构因果机制和高维DAG搜索空间时的局限性。

Method: 设计包含叶子节点预测器和父节点预测器的两阶段自回归Transformer架构，采用表格交互块提取特征，并引入混合叶子专家机制适配不同因果机制。

Result: 在合成基准和复杂真实数据集上，DAG-FM在准确性和可扩展性上显著优于传统算法和现有基础模型。

Conclusion: DAG-FM通过分解预测和动态专家路由，有效提升了因果发现的泛化能力和实用性。
...
### [316] [Advancing Optimal Subset Oracle via Learning Relaxation of Neural Set Functions](https://arxiv.org/abs/2607.11555)
*Yongquan Shi,Zijing Ou,Shiping Wang,Yatao Bian*

Main category: Inference

TL;DR: 提出一种通过连续松弛替代蒙特卡洛采样的神经集函数学习框架，利用学习代理目标优化ELBO梯度估计，降低计算开销并加速推理。


<details>
  <summary>Details</summary>
Motivation: 现有基于最优子集或acles的弱监督集函数学习方法依赖蒙特卡洛采样估计ELBO梯度，导致计算成本高且优化不稳定。

Method: 将ELBO重新解释为集函数的连续松弛，学习一个替代目标函数以取代采样梯度估计，在连续域中提供稳定高效梯度。

Result: 在多个真实任务上取得一致优于现有基线的表现，并建立了亚模最大化下的近似保证。
...
### [317] [Bet on Features: Anytime-Valid and Feature-Aware Auditing of Conditional Quantile Forecasters](https://arxiv.org/abs/2607.11653)
*Ivane Antonov,Sohom Mukherjee,Richard Pibernik,Yo Joong Choe*

Main category: Inference

TL;DR: 提出了一种分布自由、基于博弈论的连续审计框架，用于在非独立同分布数据流中审计黑箱条件分位数预测器的校准性，并考虑了信息集粗粒度对校准检测的影响。


<details>
  <summary>Details</summary>
Motivation: 现有固定水平回测无法应对数据漂移和机制变化，且未考虑校准的信息依赖性——粗信息集下看似校准的预测在细信息集下可能不校准。

Method: 建立基于审计者可用特征集的条件分位数校准形式化定义，识别可检测的备择假设集，聚焦于特征的线性上下文赌注，推导有限时间检测保证（无需独立同分布假设）。

Result: 证据过程具有特征级可解释性，能量化细粒度特征感知的误校准证据；在模拟和真实数据上验证，发现流行时序预测器Chronos-2在多个相关特征上高度误校准。

Conclusion: 该框架能持续监测黑箱分位数预测器在非平稳环境中的校准性，提供特征级别的强检测证据，适用于供应链等具有不对称成本的序贯决策场景。
...
### [318] [A multi-scale feature enhanced graph neural network for fluid dynamics prediction in complex geometries](https://arxiv.org/abs/2607.11672)
*Li Xiao,Tianyu Li,Yiye Zou,Mingjie Zhang,Xiaogangd Deng*

Main category: Inference

TL;DR: ME-GNN通过多尺度特征增强和高效子图采样，在复杂几何和大规模网格的流体模拟中实现了最先进的预测精度。


<details>
  <summary>Details</summary>
Motivation: 传统大规模数值模拟计算成本高，图神经网络在处理复杂几何和大规模网格时面临挑战。

Method: 采用两步消息传递机制捕获局部细节，结合Attention U-Net与均匀网格离散化提取粗细特征，并利用K-hop采样构建子图实现高效训练。

Result: 在ShapeNet-Car上速度场相对L2误差0.0196，表面压力0.0556；AirfRANS流场归一化均方误差0.0033；DrivAerNet表面压力相对L2误差0.1416。

Conclusion: ME-GNN有效提升了流体动力学模拟的效率和准确性，适用于复杂工程场景。
...
### [319] [Active Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2607.11720)
*Alper Kamil Bozkurt,Shangtong Zhang,Yuichi Motai*

Main category: Inference

TL;DR: 本文首次在离线到在线强化学习（O2O-RL）中研究了在有限交互预算下主动选择策略进行微调的问题，提出了一种基于上置信界（UCB）的平衡策略评估与微调的方法。


<details>
  <summary>Details</summary>
Motivation: 现有O2O-RL方法通常固定选择一个策略进行微调，但微调性能对算法和超参数高度敏感，导致风险较高。因此，需要一种在有限交互预算下主动选择高潜力策略的机制。

Method: 识别出策略评估与微调之间的权衡，并基于在线评估数据拟合局部线性性能预测，再通过UCB上界主动选择策略进行微调。

Result: 在多样化实验中，所提方法持续优于现有O2O-RL基线。

Conclusion: 主动选择并微调策略比固定策略或平分交互预算更高效，推动了离线RL在真实高风险系统中的实际部署。
...
### [320] [HiFi-LLP: High-Fidelity, Low-Cost Latency Predictors with Confidence for Robust HW-NAS](https://arxiv.org/abs/2607.11746)
...
TL;DR: 提出高保真低成本的延迟预测器HiFi-LLP，结合置信度指标与混合NAS框架，显著加速硬件感知神经架构搜索。


<details>
  <summary>Details</summary>
Motivation: 现有硬件感知NAS中，硬件在环延迟测量成为瓶颈，而平台特定预测器需大量样本且不准确会误导搜索。

Method: 基于图注意力网络构建延迟预测器HiFi-LLP，并引入置信度指标；提出混合NAS框架，将低置信度预测交由硬件在环处理。

Result: 在LatBench数据集六种设备上，HiFi-LLP的10%精度边界提升最高9个百分点，Spearman秩相关系数达0.996；混合NAS相比典型NAS实现最高8.6倍加速。

Conclusion: HiFi-LLP和混合NAS框架有效平衡了延迟预测的精度与成本，提升了硬件感知NAS的效率。停止标记
...
### [321] [From Global to Factor-Wise Expert Composition in Discrete Diffusion Models](https://arxiv.org/abs/2607.11758)
*Haozhe Huang,Yudong Xu,Abhijoy Mandal,Alán Aspuru-Guzik*

Main category: Inference

TL;DR: 提出FactorDiff框架，通过因子级分解和动态路由专家，在ARC-AGI基准上优于全局标量加权方法。


<details>
  <summary>Details</summary>
Motivation: 现有离散扩散模型在组合生成时采用逐样本混合权重，忽略了潜在的空间或功能专业化，导致容量受限。

Method: 将样本分解为更小的因子（如像素级），并在采样过程中动态地将每个因子路由到最相关的专家。

Result: 在ARC-AGI基准上，简单的因子特定路由在需要逻辑一致性和空间解耦的任务上持续优于复杂的全局标量加权方案。

Conclusion: 因子级分解与动态路由能有效提升扩散模型在复杂推理任务中的组合生成性能。
...
### [322] [An Exact Instrument for State Usage in Selective State-Space Models, and the Input-Driven Migration It Reveals](https://arxiv.org/abs/2607.11796)
*Raktim Bhattacharya*

Main category: Inference

TL;DR: 提出了用于选择性状态空间模型（如Mamba）的精确模式贡献测量方法，发现模型会根据输入动态重新分配状态空间，输入依赖的写映射是主要原因，并基于此实现了输入调度的模式剪枝。


<details>
  <summary>Details</summary>
Motivation: 现有选择性状态空间模型（如Mamba）通过学习的选择机制控制输入耦合到一阶模式，但缺乏精确工具来测量训练模型如何使用这些模式，以及模式贡献如何随输入变化。

Method: 利用对角状态矩阵使每个通道的输出可精确分解为逐模式贡献，构建每（层、通道、窗口）的Gram张量以离线计算任意子集模式丢弃的输出误差；通过冻结信号反事实分析归因迁移来源。

Result: 在Mamba-1家族上验证相对误差2.3×10⁻⁷，预测剪枝误差中位数相对偏差5×10⁻⁷；发现模式承载的信号随上下文迁移，输入依赖的写映射是迁移主因；输入调度剪枝在130M到7B Falcon-Mamba上优于静态方法，半预算下匹配未剪枝模型。

Conclusion: 我们并未部署节省计算或内存，但证明了通过首次遍历窗口模式使用实现输入调度剪枝是可行的，模型状态空间重分配的可解释性为未来优化提供了依据。
...
### [323] [Transformer-Guided Swarm Intelligence for Frugal Neural Architecture Search](https://arxiv.org/abs/2607.11826)
*Romain Amigon*

Main category: Inference

TL;DR: 提出一种节俭的模因神经架构搜索框架，混合强化学习Transformer与人工蜂群算法，在消费级GPU上3小时内发现高效架构，CIFAR-10达84.85%准确率（17.4万参数），并成功用于信用卡欺诈检测。


<details>
  <summary>Details</summary>
Motivation: 传统NAS需要大量计算资源（数千GPU天），阻碍了在消费级硬件上的普及。现有元启发式方法存在冷启动和早熟收敛问题，且模型容易臃肿。

Method: 采用双阶段杂交：自回归Transformer控制器（RL）进行全局宏观搜索，人工蜂群算法进行局部微观优化；通过动态熵机制在RL阶段检测性能停滞时强制拓扑探索；算法中惩罚网络深度以缓解模型膨胀。

Result: 在NVIDIA RTX 3060上：CIFAR-10在3小时内搜索得到17.4万参数网络，准确率84.85%（优于ResNet-20）；信用卡欺诈检测中得到4600参数网络，F1-score达0.71。

Conclusion: 该框架能在消费级硬件上高效生成参数精简、性能良好的深度学习模型，适合边缘部署，并具有跨领域适用性。
...
### [324] [Invariant Learning Dynamics of Transformers in Inductive Reasoning Tasks](https://arxiv.org/abs/2607.11875)
*Tiberiu Musat,Tiago Pimentel,Nicholas Zucchet,Thomas Hofmann*

Main category: Inference

TL;DR: 提出了一个理论框架，将Transformer语言模型归纳推理能力的涌现解释为低维不变流形上的可解释动力学过程。


<details>
  <summary>Details</summary>
Motivation: 现有关于Transformer学习动态的工作大多局限于特定任务，缺乏对泛化归纳推理能力形成机制的通用理论解释。

Method: 定义了一类泛化归纳任务（包括上下文n-gram和多跳推理），通过理论证明注意力模型训练动态可被约束在低维不变流形上，并用少量可解释坐标捕获学习动态。

Result: 揭示了数据统计如何决定上下文学习与权重学习之间的竞争，随机初始化如何影响不同电路获胜，以及该坐标框架可自动检测训练模型已学习的电路。

Conclusion: 通过将电路形成视为低维动力学现象，为预测Transformer学习机制提供了理论基础。
